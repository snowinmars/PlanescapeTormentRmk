...
class DialogueReplacer:
    ...
    def build_replacements(self):
        ...
        self.boolean('Fortress_Party_Roof',           'fortress_party_roof')
        ...
        self.add_replacement('HasItem("Cobble","Post")', 'HasItem("Cobble","Post")  # Checks if "Cobble" is in Quick Item Slot 4 Other possible values include "Weapon1", "Weapon2", "Shield", "Armor", "Helmet", "RingLeft", "RingRight", "Cloak", "Amulet", "Belt", "Boots", "Gloves", "QuickItem1-3", or "Inventory" (general inventory).')
        ...
        self.add_replacement('HPPercent(Protagonist,100)', 'current = self.settings_manager.character_manager.get_property(\'protagonist\', \'current_health\')\nmax = self.settings_manager.character_manager.get_property(\'protagonist\', \'max_health\')\nreturn current == max')
        self.add_replacement('HPPercentGT(Protagonist,49)', 'current = self.settings_manager.character_manager.get_property(\'protagonist\', \'current_health\')\nmax = self.settings_manager.character_manager.get_property(\'protagonist\', \'max_health\')\nreturn current > max / 2')
        self.add_replacement('HPPercentLT(Protagonist,50)', 'current = self.settings_manager.character_manager.get_property(\'protagonist\', \'current_health\')\nmax = self.settings_manager.character_manager.get_property(\'protagonist\', \'max_health\')\nreturn current <= max / 2')
