...
GOLD_REGEX = re.compile(r'(TakePartyGold|DestroyPartyGold|PartyGoldGT|GiveGoldForce)\((.*?)\)')
...
class DialogueTransformer:
    ...
    def _transform_update_journal(self, script, target_npc):
        def rule(match):
            ...
            return f"self.settings_manager.update_journal('{note_id}')    # '{note_id}': '{note_text}'"
