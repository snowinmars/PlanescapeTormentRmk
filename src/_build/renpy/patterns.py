...
""", extractors={'v': lambda m: m.group(1)})
...
""", extractors={'s': lambda m: m.group(1)})
...
action_update_journal_pattern = PatternConfig(
    pattern=re.compile(r"self\.settings_manager\.update_journal\((.*?)\)$"),
    template="""
    def test_{f}(self):
        note_id = {v}
...
""", extractors={'s': lambda m: m.group(1), 'v': lambda m: m.group(2)})
...
condition_is_visited_internal_location_condition_pattern = PatternConfig(
    pattern=re.compile(r"return self\.settings_manager\.location_manager\.is_visited_internal\((.*?)\)$"),
    template="""
    def test_{f}(self):
        self._is_visited_internal_location_condition(
            {v},
            self.logic.{f}
        )
""", extractors={'v': lambda m: m.group(1)})
...
condition_not_is_visited_internal_location_condition_pattern = PatternConfig(
    pattern=re.compile(r"return not self\.settings_manager\.location_manager\.is_visited_internal\((.*?)\)$"),
    template="""
    def test_{f}(self):
        self._not_is_visited_internal_location_condition(
            {v},
            self.logic.{f}
        )
""", extractors={'v': lambda m: m.group(1)})
...
all_patterns = [
    ...
    condition_is_visited_internal_location_condition_pattern,
    condition_not_is_visited_internal_location_condition_pattern
]
