class DeionsLogic:
    def __init__(self, gsm):
        self.gsm = gsm


    def r701_action(self):
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r699_action(self):
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r9616_action(self):
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r705_action(self):
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r707_action(self):
        self.gsm.set_deionarra_value(1)


    def r708_action(self):
        self.gsm.set_deionarra_value(1)


    def r709_action(self):
        self.gsm.set_deionarra_value(1)


    def r712_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'good', -1, 'globalevil_deionarra_1')
        self.gsm.gcm.modify_property_once('protagonist', 'law', -1, 'globalchaotic_deionarra_1')


    def r700_action(self):
        self.gsm.set_deionarra_value(2)


    def r702_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'good', -1, 'globalevil_deionarra_2')
        self.gsm.gcm.modify_property_once('protagonist', 'law', -1, 'globalchaotic_deionarra_2')


    def r747_action(self):
        self.gsm.set_deionarra_value(2)


    def r1313_action(self):
        self.gsm.set_deionarra_value(2)


    def r13255_action(self):
        self.gsm.set_deionarra_value(2)
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r803_action(self):
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r6085_action(self):
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r13256_action(self):
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r780_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'good', -2, 'globalevil_deionarra_3')
        self.gsm.gcm.modify_property_once('protagonist', 'law', -2, 'globalchaotic_deionarra_3')


    def r6093_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'good', 2, 'globalgood_deionarra_1')
        self.gsm.gcm.modify_property_once('protagonist', 'law', 2, 'globallawful_deionarra_1')


    def r805_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'good', -2, 'globalevil_deionarra_3')
        self.gsm.gcm.modify_property_once('protagonist', 'law', -2, 'globalchaotic_deionarra_3')


    def r808_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'good', 2, 'globalgood_deionarra_1')
        self.gsm.gcm.modify_property_once('protagonist', 'law', 2, 'globallawful_deionarra_1')


    def r786_action(self):
        SetGlobal("Prophecy","GLOBAL",1)


    def r6081_action(self):
        self.gsm.set_deionarra_value(2)
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r6082_action(self):
        self.gsm.set_deionarra_value(2)
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r13257_action(self):
        self.gsm.set_deionarra_value(2)
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r810_action(self):
        self.gsm.update_journal('26087')
        # '26087': ' ~Я встретил призрак женщины по имени Дейонарра, и она предсказала мне, что я столкнусь с тремя врагами, но «ни один из них не был бы мне ровней в период полного расцвета моих сил». Они — тени зла, добра и нейтральности, которых породили и извратили законы планов. Она сказала, что я попаду в тюрьму, построенную из «сожалений и скорби», где «даже тени теряют рассудок». Там меня попросят принести ужасную жертву... чтобы обрести покой, я должен «уничтожить то, что удерживает меня в живых, и отринуть свое бессмертие».~ '


    def r6129_action(self):
        self.gsm.set_deionarra_value(1)
        self.gsm.gcm.modify_property_once('protagonist', 'law', -1, 'globalchaotic_deionarra_4')


    def r6131_action(self):
        self.gsm.set_deionarra_value(1)
        self.gsm.gcm.modify_property_once('protagonist', 'law', -1, 'globalchaotic_deionarra_4')


    def r6132_action(self):
        self.gsm.set_deionarra_value(1)
        self.gsm.gcm.modify_property_once('protagonist', 'law', -1, 'globalchaotic_deionarra_4')


    def r6133_action(self):
        self.gsm.set_deionarra_value(1)


    def r6095_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'good', -1, 'globalevil_deionarra_4')


    def r6097_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'good', -1, 'globalevil_deionarra_4')


    def r6100_action(self):
        SetGlobal("Deionarra_Curse","GLOBAL",1)


    def r6101_action(self):
        SetGlobal("Deionarra_Curse","GLOBAL",1)


    def r6148_action(self):
        SetGlobal("Deionarra_Portal","GLOBAL",1) self.gsm.inc_exp_custom('party', 500)


    def r6154_action(self):
        self.gsm.set_deionarra_value(2)
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r6155_action(self):
        self.gsm.set_deionarra_value(2)
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r13258_action(self):
        self.gsm.set_deionarra_value(2)
        SetGlobal("Deio_Wake_Up","GLOBAL",0)


    def r63371_action(self):
        StartCutSceneMode() # ?.startCutScene("1203cd11")


    def r64594_action(self):
        StartCutSceneMode() # ?.startCutScene("1203cd21")


    def r63373_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'law', 3, 'globallawful_deionarra_2')
        self.gsm.gcm.modify_property_once('protagonist', 'good', 1, 'globalgood_deionarra_2')
        SetAnimState(Myself,ANIM_MIMESTAND)


    def r63374_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'law', -3, 'globalchaotic_deionarra_5')
        self.gsm.gcm.modify_property_once('protagonist', 'good', -1, 'globalevil_deionarra_5')


    def r63376_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'law', -1, 'globalchaotic_deionarra_6')
        self.gsm.gcm.modify_property_once('protagonist', 'good', -2, 'globalevil_deionarra_6')


    def r63377_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'law', 1, 'globallawful_deionarra_3')
        self.gsm.gcm.modify_property_once('protagonist', 'good', 1, 'globalgood_deionarra_3')


    def r63380_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'law', -1, 'globalchaotic_deionarra_7')
        self.gsm.gcm.modify_property_once('protagonist', 'good', -1, 'globalevil_deionarra_7')


    def r63381_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'law', 1, 'globallawful_deionarra_4')
        self.gsm.gcm.modify_property_once('protagonist', 'good', 1, 'globalgood_deionarra_4')


    def r63382_action(self):
        self.gsm.gcm.modify_property_once('protagonist', 'law', 2, 'globallawful_deionarra_5')


    def r63384_action(self):
        SetGlobal("Deionarra_Forgives","GLOBAL",1)


    def r63388_action(self):
        SetGlobal("1200_Cut_Scene_2","GLOBAL",1) StartCutSceneMode() SetGlobal("cd_int_1","LOCALS",1) # ?.startCutScene("1200cut1")


    def r63391_action(self):
        SetGlobal("1200_Cut_Scene_2","GLOBAL",1) StartCutSceneMode() SetGlobal("cd_int_1","LOCALS",1) # ?.startCutScene("1200cut1")


    def r63415_action(self):
        self.gsm.update_journal('68117')
        # '68117': ' ~Дейонарра предупредила меня, что за входом в Крепость расположен огромный вестибюль теней. Она сказала, что внутри комнаты есть множество огромных часов, добавив, что прежде с их помощью я смог выбраться из вестибюля. Надо будет найти их и осмотреть при случае. ~ '


    def r63417_action(self):
        self.gsm.update_journal('68117')
        # '68117': ' ~Дейонарра предупредила меня, что за входом в Крепость расположен огромный вестибюль теней. Она сказала, что внутри комнаты есть множество огромных часов, добавив, что прежде с их помощью я смог выбраться из вестибюля. Надо будет найти их и осмотреть при случае. ~ '


    def r63419_action(self):
        TransformPartyItem("Dwedring","DsupRing",1,0,0)


    def r66914_action(self):
        self.gsm.inc_exp_custom('party', 1000)
        SetGlobal("Deionarra_Raise_Dead","GLOBAL",1) ApplySpell(Protagonist,SPECIAL_ADD_RAISE_DEAD) ApplySpell(Protagonist,SPECIAL_ADD_RAISE_DEAD) ApplySpell(Protagonist,SPECIAL_ADD_RAISE_DEAD) self.gsm.update_journal('66917')
        # '66917': ' ~Я не знаю, ужасаться мне или удивляться... когда я пообщался с Дейонаррой в Морге, она сказала, что через свои бесконечные перерождения я обрел некоторую власть над жизнью и смертью. Если я вижу тело, то могу разглядеть слабые следы жизни в нем и воскресить его. По какой-то причине это действует только на тех, кто путешествовал вместе со мной, и только в том случае, если они умерли в моем присутствии... но почему? Быть может, в пути между нами появляется какая-то связь? ~ '


    def r701_condition(self):
        return self.gsm.get_morte_deionarra_quip_1()


    def r699_condition(self):
        return self.gsm.get_in_party_morte() and \
               not self.gsm.get_morte_deionarra_quip_1()


    def r9616_condition(self):
        return not self.gsm.get_in_party_morte() and \
               not self.gsm.get_morte_deionarra_quip_1()


    def r708_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'intelligence') > 11 and \
               self.gsm.gcm.get_character_property('protagonist', 'charisma') < 11


    def r709_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'charisma') > 10


    def r713_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'intelligence') > 11 and \
               self.gsm.gcm.get_character_property('protagonist', 'charisma') < 11


    def r714_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'charisma') > 10


    def r1308_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r6080_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r767_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r1309_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r718_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'intelligence') > 11 and \
               self.gsm.gcm.get_character_property('protagonist', 'charisma') < 11


    def r719_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'charisma') > 10


    def r721_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r1310_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r1311_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r764_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r723_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'intelligence') > 11 and \
               self.gsm.gcm.get_character_property('protagonist', 'charisma') < 11


    def r724_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'charisma') > 10


    def r1312_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r6084_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r747_condition(self):
        return self.gsm.get_morte_deionarra_quip_1()


    def r1313_condition(self):
        return self.gsm.get_in_party_morte() and \
               not self.gsm.get_morte_deionarra_quip_1()


    def r13255_condition(self):
        return not self.gsm.get_in_party_morte() and \
               not self.gsm.get_morte_deionarra_quip_1()


    def r731_condition(self):
        return not self.gsm.get_escape_mortuary() and \
               self.gsm.gcm.get_character_property('protagonist', 'wisdom') < 13


    def r732_condition(self):
        return not self.gsm.get_escape_mortuary() and \
               self.gsm.gcm.get_character_property('protagonist', 'wisdom') > 12


    def r1314_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r6127_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r737_condition(self):
        return not self.gsm.get_escape_mortuary() and \
               self.gsm.gcm.get_character_property('protagonist', 'wisdom') < 13


    def r738_condition(self):
        return not self.gsm.get_escape_mortuary() and \
               self.gsm.gcm.get_character_property('protagonist', 'wisdom') > 12


    def r768_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r1315_condition(self):
        return Global("Deionarra_Curse","GLOBAL",1)


    def r6107_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r6128_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r742_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r1316_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r746_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r792_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r790_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r1318_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r755_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r1319_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r803_condition(self):
        return self.gsm.get_morte_deionarra_quip_1()


    def r6085_condition(self):
        return self.gsm.get_in_party_morte() and \
               not self.gsm.get_morte_deionarra_quip_1()


    def r13256_condition(self):
        return not self.gsm.get_in_party_morte() and \
               not self.gsm.get_morte_deionarra_quip_1()


    def r778_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'charisma') > 10


    def r813_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r1320_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r6081_condition(self):
        return self.gsm.get_morte_deionarra_quip_1()


    def r6082_condition(self):
        return self.gsm.get_in_party_morte() and \
               not self.gsm.get_morte_deionarra_quip_1()


    def r13257_condition(self):
        return not self.gsm.get_in_party_morte() and \
               not self.gsm.get_morte_deionarra_quip_1()


    def r797_condition(self):
        return Global("Deionarra_Raise_Dead","GLOBAL",1)


    def r66911_condition(self):
        return Global("Deionarra_Raise_Dead","GLOBAL",0)


    def r798_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r1321_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r801_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r802_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r1322_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r1323_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r816_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r1324_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r820_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r1325_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r823_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r1326_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r6129_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'charisma') > 10


    def r6131_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'intelligence') > 11 and \
               self.gsm.gcm.get_character_property('protagonist', 'charisma') < 11


    def r6132_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'charisma') > 10


    def r6112_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r6113_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r6114_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r6115_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r6117_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r6118_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r6119_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r6120_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r6139_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'charisma') > 10


    def r6140_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r6141_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r6145_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r6146_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r6147_condition(self):
        return Global("Deionarra_Portal","GLOBAL",1)


    def r6148_condition(self):
        return Global("Deionarra_Portal","GLOBAL",0)


    def r6150_condition(self):
        return self.gsm.gcm.get_character_property('protagonist', 'charisma') > 10


    def r6152_condition(self):
        return Global("Prophecy","GLOBAL",0)


    def r6153_condition(self):
        return Global("Prophecy","GLOBAL",1)


    def r6154_condition(self):
        return self.gsm.get_morte_deionarra_quip_1()


    def r6155_condition(self):
        return self.gsm.get_in_party_morte() and \
               not self.gsm.get_morte_deionarra_quip_1()


    def r13258_condition(self):
        return not self.gsm.get_in_party_morte() and \
               not self.gsm.get_morte_deionarra_quip_1()


    def r63367_condition(self):
        return GlobalGT("Fortress_Party","GLOBAL",1) and \
               not self.gsm.get_fortress_ignus() and \
               not self.gsm.get_fortress_vhailor()


    def r63368_condition(self):
        return GlobalGT("Fortress_Party","GLOBAL",1) and \
               not self.gsm.get_fortress_ignus() and \
               not self.gsm.get_fortress_vhailor()


    def r63369_condition(self):
        return GlobalGT("Deionarra_Death_Truth","GLOBAL",0)


    def r63371_condition(self):
        return Global("Deionarra_Forgives","GLOBAL",1)


    def r64594_condition(self):
        return Global("Deionarra_Forgives","GLOBAL",0)


    def r63394_condition(self):
        return self.gsm.get_deionarra_value() == 0


    def r63395_condition(self):
        return self.gsm.get_deionarra_value() > 0


    def r63397_condition(self):
        return self.gsm.get_deionarra_value() == 0


    def r63398_condition(self):
        return self.gsm.get_deionarra_value() > 0


    def r63401_condition(self):
        return Global("Know_Shadow_Player_Connection","GLOBAL",1)


    def r63402_condition(self):
        return Global("Know_Shadow_Player_Connection","GLOBAL",0)


    def r63406_condition(self):
        return GlobalGT("Fortress_Party","GLOBAL",2)


    def r63407_condition(self):
        return Global("Fortress_Party","GLOBAL",2)


    def r63408_condition(self):
        return Global("Fortress_Party","GLOBAL",1) and \
               self.gsm.get_deionarra_value() > 0


    def r63409_condition(self):
        return Global("Fortress_Party","GLOBAL",1) and \
               self.gsm.get_deionarra_value() == 0


    def r63413_condition(self):
        return GlobalGT("Fortress_Party","GLOBAL",2)


    def r63414_condition(self):
        return Global("Fortress_Party","GLOBAL",2)


    def r63415_condition(self):
        return Global("Fortress_Party","GLOBAL",1)


    def r63419_condition(self):
        return self.gsm.get_deionarra_value() > 0 PartyHasItem("DwedRing")


    def r63420_condition(self):
        return self.gsm.get_deionarra_value() == 0


    def r63421_condition(self):
        return self.gsm.get_deionarra_value() > 0


    def r63423_condition(self):
        return self.gsm.get_deionarra_value() > 0