init 10 python:
    from dlgs.ddustfem_logic import DdustfemLogic
...
label ddustfem_s0:  # - # IF ~  Global("Appearance","GLOBAL",1)
    SPEAKER 'Тленная не обращает на тебя внимания. Должно быть, она спутала тебя с одним из мертвых рабочих.'
...
label ddustfem_s1:  # from 0.0 0.1 0.2 0.3
    SPEAKER 'Тленная подпрыгивает от неожиданности. Затем она поворачивает к тебе голову. Она выглядит потрясенной: должно быть, маскировка у тебя весьма неплохая.'
...
label ddustfem_s2:  # from 1.1 1.2 4.3 5.2 5.3 6.4 19.6 20.4 47.2 47.3 51.4
    SPEAKER 'Тленная отступает на шаг, затем быстро хлопает в ладони три раза. В ответ во всем Морге раздается звон огромного железного колокола.'
...
label ddustfem_s4:  # from 3.0 3.1 3.2 3.3 40.2 40.3
    SPEAKER 'Тленная медленно поднимает свою голову и оборачивается к тебе. Ты потерялся?'
...
label ddustfem_s5:  # from 4.0 16.2 51.1
    SPEAKER 'Я позову стражу, они тебя живо выведут. Погоди минутку.'
...
label ddustfem_s6:  # from 4.1 4.2 51.2 51.3
    SPEAKER 'Если ты не потерялся, то что же ты здесь делаешь?'
...
label ddustfem_s7:  # from 6.0 9.0 20.0
    SPEAKER 'Боюсь, что меня касается. Может, стражники развяжут твой язык. Тленная делает шаг назад, кажется, она собирается позвать стражников.'
...
label ddustfem_s8:  # from 6.1 16.0 20.1
    SPEAKER 'Любишь пошутить? Тогда может поделишься своими шутками со стражниками. Тленная отступает на шаг; кажется, она собирается позвать стражников.'
...
label ddustfem_s10:  # from 9.1
    SPEAKER 'Дхолла можно найти в приемной комнате на этом этаже. Должна предупредить… Дхолл очень занят, а здоровье у него подкошено. Если у тебя к нему несрочное дело, то лучше не беспокоить его.'
...
label ddustfem_s11:  # from 9.2
    SPEAKER 'Дхолл скорее всего в приемной комнате на втором этаже. Он очень занят, а здоровье у него подкошено. Если у тебя к нему несрочное дело, то лучше не беспокоить его.'
...
label ddustfem_s23:  # from 22.0
    SPEAKER 'Она медленно оборачивается, ее взгляд мельком скользит по твоей одежде. Приветствую тебя, посвященный.'
...
label ddustfem_s26:  # from 23.2 27.0 28.2 30.3 31.3 34.2 36.1 39.0 50.0
    SPEAKER 'Тленная терпеливо ждет твоего продолжения.'
...
label ddustfem_s28:  # from 26.1
    SPEAKER 'Это имя… — тленная на секунду умолкает. — Это имя *звучит* знакомо… Кажется, я припоминаю сборщика с таким именем. Писец Дхолл может знать о нем больше.'
...
label ddustfem_s29:  # from 28.0
    SPEAKER 'Сборщики… они собирают тех, кто умер на улицах Сигила, и доставляют их в Морг… — тленная умолкает, хмуря брови. — Ты нездешний. Кто ты?'
...
label ddustfem_s32:  # from 30.1
    SPEAKER 'Дхолл находится в приемной комнате в северо-западной части этого этажа. Должна предупредить… Дхолл очень занят… то время, которое он не занят своими обязанностями, отбирает у него болезнь.'
...
label ddustfem_s33:  # from 30.2
    SPEAKER 'Дхолл скорее всего в приемной комнате на втором этаже. Лучше не отвлекай его слишком сильно — он очень занят… то время, которое он не занят своими обязанностями, отбирает у него болезнь.'
...
label ddustfem_s35:  # from 31.1 34.0
    SPEAKER 'Гитцерай — это… — тленная умолкает, пристально глядя на тебя. — Ты ведь нездешний. Кто ты?'
...
label ddustfem_s37:  # from 36.0
    SPEAKER 'Тленная кивает. Да. Она хмурится, затем пристально смотрит на тебя. Ты нездешний. Кто ты?'
...
# s38 # say1212
label ddustfem_s38:  # -
    SPEAKER 'Ты не из наших. Кто ты? Что ты здесь делаешь? Ты из анархистов? Или шпион другой фракции? — Тленная отступает на шаг. — Стража! Стража!'
...
label ddustfem_s39:  # from 26.2
    SPEAKER 'Дневник? Не встречала такого.'
...
# s40 # say1214
label ddustfem_s40:  # - # Check EXTERN ~DMORTE~ : 81 Check EXTERN ~DMORTE~ : 83
    SPEAKER 'Эта бледная женщина одета в длинную темную мантию. От нее слегка отдает плесенью. Ее лицо ничего не выражает; кажется, она поглощена своими обязанностями.'
...
label ddustfem_s41:  # from 1.0 5.1 7.1 8.1 47.1
    SPEAKER 'Тленная не успевает и слова вымолвить, как твои руки хватают ее голову за виски и резко сворачивают ее влево.'
...
label ddustfem_s42:  # from 41.0 45.0
    SPEAKER 'В шее раздается характерный хруст, и тело тленной безвольно падает в твои объятия.'
...
label ddustfem_s44:  # from 5.0 7.0 8.0 19.4 47.0
    SPEAKER 'Ты недостаточно быстр, и тленная уворачивается от твоего выпада. Отступив на шаг, она быстро хлопает в ладони три раза. В ответ во всем Морге раздается звон огромного железного колокола.'
...
label ddustfem_s45:  # from 19.5
    SPEAKER 'Ты наклоняешься, чтобы шепнуть ей что-то на ухо, тленная тоже наклоняется. Как только она оказывается на расстоянии вытянутой руки, ты хватаешь ее за виски и резко сворачиваешь голову влево.'
...
label ddustfem_s46:  # from 24.3 25.3 29.3 35.3 37.3 49.3 50.1
    SPEAKER 'Тленная явно что-то подозревает. Похоже, она хочет что-то сказать, затем едва заметно качает головой и возвращается к своим обязанностям.'
...
label ddustfem_s47:  # from 24.2 25.2 29.1 29.2 35.1 35.2 37.1 37.2 49.1 49.2
    SPEAKER 'Тленная внимательно тебя осматривает. Ты не из наших. Что ты здесь делаешь? Ты из анархистов? Или шпион другой фракции? Кажется, здесь есть дело для стражников…'
...
label ddustfem_s48:  # from 10.0 11.0 12.0 13.0 14.0 15.0 26.3 27.1 28.3 30.4 31.4 32.1 33.1 34.3 36.2 39.1
    SPEAKER 'Тленная кивает и возвращается к своим делам.'
...
label ddustfem_s49:  # from 24.0 24.1 25.0 25.1
    SPEAKER 'Тленная хмурится. Это имя мне незнакомо.'
...
label ddustfem_s50:  # from 29.0 35.0 37.0 49.0
    SPEAKER 'Тленная продолжает хмуриться, но затем слегка кивает. Ну хорошо. Что я могу сделать для тебя, посвященный?'
...
label ddustfem_s51:  # - # IF ~  Global("Appearance","GLOBAL",0)
    SPEAKER 'Тленная бросает на тебя каменный взгляд. Ты потерялся?'
...
label ddustfem_s52:  # from 51.0
    SPEAKER 'Тленная какое-то время пристально на тебя смотрит, затем кивает. Хорошо. Если тебе нужна помощь, дай мне знать.'
