...
label soego_s1: # from 0.0 0.1 0.2 0.3
    ...
    soego '«Про… прошу прощения? Ты что-то сказал мне?»'
...
label soego_s2: # from 1.0 1.1 3.0 3.3 4.0 4.1
    ...
    soego '«Эй, ты заставляешь меня сделать нелестное признание: из тебя вышел убедительный зомби».'
...
label soego_s7: # from 2.1 5.1 13.3 15.1 42.1 57.0
    ...
    soego '«Ты *выглядишь* так, как будто тебя только что вскрыли. Не представляю, как ты смог пережить эту невыносимую боль… тебе *больно*? Кажется, да».'
...
label soego_s8: # from 2.2 5.2 6.0 13.0 15.2 16.0 17.0 26.0 40.0 41.0 50.0 61.0 62.0
    ...
    soego '«Ну почему же… конечно. Эти залы *способны* запутать посетителей. В этом нет ничего плохого, но тебе нельзя находиться в Морге после девяти ударов колокола. Позволь мне открыть для тебя ворота».'
...
label soego_s12: # from 11.0 # IF WEIGHT #5 /* Triggers after states #: 63 108 79 82 95 even though they appear after this state */ ~  CreatureInArea("AR0201") Global("Gate_Open","GLOBAL",1) Global("Gate_Cut_Scene","AR0201",1)
    soego '«Вот и все. Главные ворота открыты, но вернуться обратно ты не сможешь».'
...
label soego_s23: # from 14.0
    soego '«Некоторые подписывают с тленными контракты, согласно которым они могут использовать их тела, как только те умрут. Возможно, тебя взяли… по недоразумению. Ты кажешься гораздо умнее, чем наши обычные зомби».'
...
label soego_s25: # from 24.0 24.1
    soego '«Оболочки совершают различные поручения по всему Моргу. Они перевозят тела, моют полы, помогают готовить тела к погребению… относительно несложные задачи. К сожалению, они не способны выполнять более сложные инструкции».'
...
label soego_s36: # from 33.1 34.1 35.0
    soego '«Я знаю, что он живет в Улье, в трущобах вокруг Морга, но точнее сказать не могу. Другие сборщики могут знать, если они соизволят говорить с тобой».'
...
label soego_s48: # from 47.0
    soego '«Я не знаю, но она здесь уже достаточно долго. Ее отец может знать, что произошло с ней… он часто приходит сюда из своих апартаментов в Верхнем районе. Это он захотел поместить ее здесь в мемориальном зале».'
...
label soego_s69: # from 67.1 68.0 70.1
    soego '«Полное отсутствие чувств, Истинная Смерть — это истинная жизнь за пределами тени этого существования. Это место, которого должны достичь все эти мертвецы, чтобы стать свободными».'
...
label soego_s71: # from 63.4 64.4 65.3 66.3 67.5 68.4 69.4 70.4 72.6 73.6 74.4 77.2 78.2 79.5 80.1 81.0 101.5 104.3 109.5 110.3 112.1
    soego '«Подожди минуту. Не нападай на здешнюю нежить: если ты не будешь их беспокоить, они не причинят тебе вреда. Будешь враждебно к ним относиться, и они начнут защищаться… а их очень много. И еще: если тебе нужно отдохнуть, ты можешь приходить сюда».'
...
label soego_s74: # from 73.1 101.2 104.0
    soego '«Нет… а должен? Я был немного расстроен, я не собирался уходить оттуда. Однако тебе все равно не стоит возвращаться в Морг: немногие знакомые мне фрактотумы будут рады повстречать тебя».'
...
label soego_s79: # - # IF WEIGHT #2 /* Triggers after states #: 82 95 even though they appear after this state */ ~  CreatureInArea("AR1500") Global("CR_Vic","GLOBAL",1)
    soego '«О, к нам подоспела подмога! Меня, как агента Многоединого, предупредили о твоем приходе. Нам нужно, чтобы ты попытался пробраться в тронный зал Безмолвного Короля и убил его. Сделай это, и Многоединый вознаградит тебя».'
...
label soego_s80: # from 79.4 110.2 112.0
    soego '«Не знаю… Я здесь уже много времени, но так и не смог найти способ попасть в его тронный зал. Может быть, тебе повезет больше, без той ненависти и фанатизма, которые направлены на меня».'
...
label soego_s88: # from 83.0 91.0
    soego '«Упрямец, и все же он поражает своей верностью и преданностью долгу. Здесь он мой сильнейший противник, и он уже многие годы не дает распасться этой цивилизации. Его страсти являются результатом его верности и преданности долгу… замечательные качества, однако направлены не в то русло».'
...
label soego_s89: # from 83.1
    soego '«Акаста»'
    nr 'Животное. Боюсь, что Безмолвный Король'
    soego '«Единственный, кто держит ее под контролем. Если его устранят, упыри разорят катакомбы».'
...
label soego_s90: # from 83.2
    soego '«Тухлая Мери — добрая душа, хоть и медлительная. Я почти ничего не понимаю из того, что она говорит, но она и другие зомби не склонны к насилию».'
...
label soego_s91: # from 83.3 89.0
    soego '«Я никогда не видел Безмолвного Короля. Хотел бы я что-нибудь о нем рассказать, но никогда с ним не встречался. Его тронный зал, предположительно, находится за красными дверями, но я не могу получить туда доступ… Харгримм, скелет-жрец, не даст мне его».'
...
label soego_s92: # from 83.4
    soego '«Думаю, они здесь уже много веков, пекутся о тех, кто скончался в их залах. Подобная преданность долгу больше не нужна… да она почти что преступна».'
...
label soego_s94: # from 82.0 82.1
    soego '«Что? Это правда? Ах, Многоединый наверняка вознаградит тебя за такие вести…»'
...
label soego_s101: # from 73.0
    soego '«Э… твоя память тебе изменяет. Конечно, моя шея слегка побаливала… она немного вывернута. Но сломана? Вряд ли».'
...
label soego_s103: # from 101.1
    soego '«Глупый вопрос! Ты очнулся на препараторской плите в Морге… ты сам мне об этом рассказал. Несомненно, мое ранение было получше того, из-за которого сборщики перепутали тебя с трупом, не так ли?»'
