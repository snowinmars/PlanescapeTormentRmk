init 10 python:
    from dlgs.soego_logic import SoegoLogic
    soegoLogic = SoegoLogic(renpy.store.global_settings_manager)


# ###
# Original:  DLG/SOEGO.DLG
# ###


label soego_init:
    return


label soego_dispose:
    jump show_graphics_menu


# s0 # say1431
label soego_s0:  # -
    SPEAKER 'Перед тобой устало выглядящий мужчина в выцветших черных одеждах. Его худощавое лицо слишком бледное, как будто он давно не спал. Его плечи осунулись, под налитыми кровью глазами видны мешки. Похоже, он не заметил тебя… Должно быть, он спутал тебя с одним из мертвых рабочих.'

    menu:
        'Приветствую.':
            # r0 # reply1432
            jump soego_s1

        'Кто ты?':
            # r1 # reply1433
            jump soego_s1

        'Что это за место?':
            # r2 # reply1434
            jump soego_s1

        'У меня есть несколько вопросов…':
            # r3 # reply1435
            jump soego_s1

        'Оставить его в покое.':
            # r4 # reply1436
            jump soego_dispose


# s1 # say1437
label soego_s1:  # from 0.0 0.1 0.2 0.3
    SPEAKER 'Когда ты обращаешься к тленному, он резко поднимает голову: Про… прошу прощения? Ты что-то сказал мне?'

    menu:
        'Да. У меня есть несколько вопросов…':
            # r5 # reply1438
            $ soegoLogic.r1438_action()
            jump soego_s2

        'Нет. Нет, это не я.':
            # r6 # reply1439
            $ soegoLogic.r1439_action()
            jump soego_s2

        'Стать тихим как покойник.' if soegoLogic.r1440_condition():
            # r7 # reply1440
            jump soego_s3

        'Стать тихим как покойник.' if soegoLogic.r1441_condition():
            # r8 # reply1441
            jump soego_s4

        'Уйти. Быстро.':
            # r9 # reply1442
            jump soego_s5


# s2 # say1443
label soego_s2:  # from 1.0 1.1 3.0 3.3 4.0 4.1
    SPEAKER 'О, силы! Тленный подпрыгивает, затем внимательно смотрит на тебя. Ты замечаешь, что его глаза не налиты кровью, а просто имеют красный оттенок. Эй, ты заставляешь меня сделать нелестное признание: из тебя вышел убедительный зомби, — он делает легкий поклон. — Я Соэго. Могу ли я спросить тебя, что ты делаешь здесь, — он косится на твои шрамы, — в таком виде?'

    menu:
        'Это не твое дело.':
            # r10 # reply1444
            jump soego_s6

        'Я не совсем понимаю, что я здесь делаю. Я очнулся на одной из плит наверху, и моя память… немного туманна.':
            # r11 # reply1445
            jump soego_s7

        'Я заблудился в этих залах и теперь не могу найти выход. Ты можешь мне помочь?' if soegoLogic.r1446_condition():
            # r12 # reply1446
            jump soego_s8

        'Я пытаюсь выбраться отсюда.':
            # r13 # reply1447
            jump soego_s13

        'Мне были нужны перемены в жизни.':
            # r14 # reply1448
            $ soegoLogic.r1448_action()
            jump soego_s16

        'У меня совершенно нет на это времени. Прощай.':
            # r15 # reply4999
            jump soego_s17


# s3 # say1449
label soego_s3:  # from 1.2
    SPEAKER 'Тленный некоторое время изучает тебя, затем трясет головой. Опять почудилось… — вздыхает он и трет глаза. — Эти заклинания от лихорадки действуют слишком сильно…'

    menu:
        'Это не твое воображение. У меня есть другие вопросы…':
            # r16 # reply1450
            $ soegoLogic.r1450_action()
            jump soego_s2

        'Свернуть ему шею, пока он отвлекся.' if soegoLogic.r1451_condition():
            # r17 # reply1451
            jump soego_s19

        'Свернуть ему шею, пока он отвлекся.' if soegoLogic.r1452_condition():
            # r18 # reply1452
            jump soego_s22

        'У меня есть несколько вопросов.':
            # r19 # reply1453
            $ soegoLogic.r1453_action()
            jump soego_s2

        'Тихо уйти.':
            # r20 # reply1454
            jump soego_dispose


# s4 # say1455
label soego_s4:  # from 1.3
    SPEAKER 'Тленный внимательно смотрит на тебя, затем наклоняется к тебе… его губы раскрываются, обнажая ряд грязных и острых зубов. Он, как крыса, начинает обнюхивать тебя.'

    menu:
        'Эй… какого черта ты меня обнюхиваешь?':
            # r21 # reply1456
            $ soegoLogic.r1456_action()
            jump soego_s2

        'Послушай, у меня есть вопросы к тебе…':
            # r22 # reply1457
            $ soegoLogic.r1457_action()
            jump soego_s2

        'Свернуть ему шею, пока он наклоняется вперед.' if soegoLogic.r1458_condition():
            # r23 # reply1458
            jump soego_s19

        'Свернуть ему шею, пока он наклоняется вперед.' if soegoLogic.r1459_condition():
            # r24 # reply1459
            jump soego_s22

        'Уйти. Быстро.':
            # r25 # reply1460
            jump soego_s15


# s5 # say1461
label soego_s5:  # from 1.4
    SPEAKER 'Как только ты собираешься уйти, тленный издает легкое шипение, затем наклоняется вперед и обнюхивает тебя. О, силы! Тленный отступает назад, его глаза широко раскрыты. Ты замечаешь, что его глаза не налиты кровью, а просто имеют красный оттенок. Эй, ты заставляешь меня сделать нелестное признание: из тебя вышел убедительный зомби, — он слегка кивает. — Я Соэго. Могу ли я спросить, что ты здесь делаешь… в таком виде?'

    menu:
        'Это не твое дело.':
            # r26 # reply1462
            jump soego_s6

        'Я не совсем понимаю, что я здесь делаю. Я очнулся на одной из плит наверху, и моя память… немного туманна.':
            # r27 # reply1463
            jump soego_s7

        'Я заблудился и ищу выход. Ты можешь мне помочь?' if soegoLogic.r1464_condition():
            # r28 # reply1464
            jump soego_s8

        'Я пытаюсь выбраться отсюда.':
            # r29 # reply1465
            jump soego_s13

        'Мне были нужны перемены в жизни.':
            # r30 # reply1466
            $ soegoLogic.r1466_action()
            jump soego_s16

        'У меня совершенно нет на это времени. Прощай.':
            # r31 # reply1467
            jump soego_s17


# s6 # say1468
label soego_s6:  # from 2.0 5.0 15.0 41.1 43.0 50.1 115.1
    SPEAKER 'О, боюсь, что это *как раз* мое дело. Глаза Соэго блестят красным, уголки его рта слегка подергиваются, словно в предвкушении. Может… — он широко ухмыляется, показывая ряд острых грязных зубов. — Может, мне стоит позвать охрану? Да… да, пожалуй, я так и сделаю.'

    menu:
        'Постой! Я потерялся… Я просто заблудился в этих залах и теперь не могу найти выход. Ты можешь мне помочь?' if soegoLogic.r1469_condition():
            # r32 # reply1469
            jump soego_s8

        'Стой! Не надо звать стражу! Я просто хочу выбраться отсюда… ты поможешь мне?' if soegoLogic.r1470_condition():
            # r33 # reply1470
            jump soego_s13

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r1471_condition():
            # r34 # reply1471
            jump soego_s19

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r1472_condition():
            # r35 # reply1472
            jump soego_s22

        'Давай, зови их. Буду рад с ними встретиться.':
            # r36 # reply1473
            jump soego_s18


# s7 # say1474
label soego_s7:  # from 2.1 5.1 13.3 15.1 42.1 57.0
    SPEAKER 'Правда? — тленный недоверчиво осматривает тебя. — Ты *выглядишь* так, как будто тебя только что вскрыли. Не представляю, как ты смог пережить эту невыносимую боль… тебе *больно*? Кажется, да.'

    menu:
        'Как я вообще сюда попал?':
            # r37 # reply1475
            jump soego_s54

        'У меня нет времени на это. Прощай.':
            # r38 # reply1476
            jump soego_s17


# s8 # say1477
label soego_s8:  # from 2.2 5.2 6.0 13.0 15.2 16.0 17.0 26.0 40.0 41.0 50.0 61.0 62.0
    SPEAKER 'Соэго кивает, уголки его рта немного подрагивают. Ну почему же… конечно. Эти залы *способны* запутать посетителей. В этом нет ничего плохого, но тебе нельзя находиться в Морге после девяти ударов колокола. Позволь мне открыть для тебя ворота.'

    menu:
        'Спасибо.' if soegoLogic.r1478_condition():
            # r39 # reply1478
            $ soegoLogic.r1478_action()
            jump soego_dispose

        'Спасибо.' if soegoLogic.r1479_condition():
            # r40 # reply1479
            jump soego_s9


# s9 # say1480
label soego_s9:  # from 8.1 56.1 60.1
    SPEAKER 'Соэго тянется к поясу, шарит по одежде, затем начинает шипеть. Ключ! В его глазах появляется яркий красный блеск, а губы складываются в гримасу ярости… выражение лица при этом напоминает звериный оскал. Кто-то украл ключ! Он разворачивается к тебе и рычит: Ты! Это сделал ты!'

    menu:
        'Блеф: Эй… погоди! Зачем мне спрашивать тебя о нем, если я его украл?':
            # r41 # reply1481
            jump soego_s18

        'Ложь: О чем это ты?! Мне-то он зачем!':
            # r42 # reply1482
            $ soegoLogic.r1482_action()
            jump soego_s18

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r1483_condition():
            # r43 # reply1483
            jump soego_s19

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r1484_condition():
            # r44 # reply1484
            jump soego_s22

        'Ну, допустим, я. И что? И что ты мне сделаешь?':
            # r45 # reply1485
            jump soego_s18


# s10 # say1486
label soego_s10:  # - # Check EXTERN ~DMORTE~ : 101
    SPEAKER 'Соэго берет огромный ключ с пояса и идет к главным воротам. Ты не можешь не заметить его странной походки… он наклоняется вперед, пытаясь сохранить равновесие.'

    menu:
        'Какая странная у него походка.' if soegoLogic.r1487_condition():
            # r46 # reply1487
            jump soego_dispose

        'Подождать, пока он открывает ворота.' if soegoLogic.r1488_condition():
            # r47 # reply1488
            jump soego_s11


# s11 # say1489
label soego_s11:  # from 10.1
    SPEAKER 'Дойдя до ворот, Соэго вставляет ключ в замок. Спустя мгновение из замочной скважины раздается неприятный звук… Он наполняет весь главный зал, эхом раскатываясь по мраморным стенам.'

    menu:
        'Подождать, пока он не вернется.':
            # r48 # reply1490
            $ soegoLogic.r1490_action()
            jump soego_s12


# s12 # say1491
label soego_s12:  # from 11.0
    SPEAKER 'Вот и все. Главные ворота открыты, но вернуться обратно ты не сможешь.'

    menu:
        'Могу ли я задать пару вопросу перед тем, как уйду?':
            # r49 # reply1492
            $ soegoLogic.r1492_action()
            jump soego_s26

        'Спасибо, Соэго. Я пошел.':
            # r50 # reply1493
            $ soegoLogic.r1493_action()
            jump soego_dispose


# s13 # say1494
label soego_s13:  # from 2.3 5.3 6.1 15.3 16.1 17.1 26.1 61.1
    SPEAKER 'Выбраться отсюда? — Соэго хмурится. — А как ты умудрился попасть внутрь?'

    menu:
        'Я был здесь на похоронах, отдавал дань уважения. Я уже готов уйти… вот только, кажется, я заблудился. Ты не поможешь мне найти выход?' if soegoLogic.r1495_condition():
            # r51 # reply1495
            jump soego_s8

        'Я и сам не знаю.' if soegoLogic.r1496_condition():
            # r52 # reply1496
            jump soego_s14

        'Я и сам не знаю.' if soegoLogic.r1497_condition():
            # r53 # reply1497
            jump soego_s61

        'Я очнулся на одной из плит наверху, и моя память… немного туманна.':
            # r54 # reply1498
            jump soego_s7

        'У меня нет времени на это. Прощай.':
            # r55 # reply1499
            jump soego_s17


# s14 # say1500
label soego_s14:  # from 13.1
    SPEAKER 'Соэго цокает языком. Очень любопытно, — он снова изучает тебя. — А может, ты один из контрактников?'

    menu:
        'Э, контрактников?':
            # r56 # reply1501
            jump soego_s23

        'У меня совершенно нет на это времени. Прощай.':
            # r57 # reply1502
            jump soego_s17


# s15 # say1503
label soego_s15:  # from 4.4
    SPEAKER 'Едва ты собираешься уйти, как тленный перестает тебя обнюхивать и испускает легкое шипение. О, силы! Тленный отступает назад, его глаза широко раскрыты. Ты замечаешь, что его глаза не налиты кровью, а просто имеют красный оттенок. Эй, ты заставляешь меня сделать нелестное признание: из тебя вышел убедительный зомби, — он слегка кивает. — Я Соэго. Могу ли я спросить, что ты здесь делаешь… в таком виде?'

    menu:
        'Это не твое дело.':
            # r58 # reply1504
            jump soego_s6

        'Я не совсем понимаю, что я здесь делаю. Я очнулся на одной из плит наверху, и моя память… немного туманна.':
            # r59 # reply1505
            jump soego_s7

        'Я заблудился и ищу выход. Ты можешь мне помочь?' if soegoLogic.r1506_condition():
            # r60 # reply1506
            jump soego_s8

        'Я пытаюсь выбраться отсюда.':
            # r61 # reply1508
            jump soego_s13

        'Мне были нужны перемены в жизни.':
            # r62 # reply1509
            $ soegoLogic.r1509_action()
            jump soego_s16

        'У меня совершенно нет на это времени. Прощай.':
            # r63 # reply1510
            jump soego_s17


# s16 # say1511
label soego_s16:  # from 2.4 5.4 15.4
    SPEAKER 'Понятно… Глаза Соэго блестят красным, уголки его рта слегка подрагивают, словно в предвкушении. Может… — он широко ухмыляется, показывая ряд острых грязных зубов. — Может, мне стоит позвать охрану? Да… да, пожалуй, я так и сделаю.'

    menu:
        'Постой! Я потерялся… Я просто заблудился в этих залах и теперь не могу найти выход. Ты можешь мне помочь?' if soegoLogic.r1512_condition():
            # r64 # reply1512
            jump soego_s8

        'Стой! Не надо звать стражу! Я просто хочу выбраться отсюда… ты поможешь мне?' if soegoLogic.r1513_condition():
            # r65 # reply1513
            jump soego_s13

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r1514_condition():
            # r66 # reply1514
            jump soego_s19

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r1515_condition():
            # r67 # reply1515
            jump soego_s22

        'Давай, зови их. Буду рад с ними встретиться.':
            # r68 # reply1516
            jump soego_s18


# s17 # say1517
label soego_s17:  # from 2.5 5.5 7.1 13.4 14.1 15.5 23.2 24.2 25.2 26.9 27.4 28.2 29.3 31.3 32.2 33.4 34.4 35.3 36.3 37.2 114.3 115.4
    SPEAKER 'Когда ты собираешься уходить, как Соэго издает злобное шипение… затем он неожиданно успокаивается и поднимает руки. Нет, нет, боюсь, что ты не можешь уйти. Что-то здесь не так. Мне кажется, что будет лучше, если мы разберемся с этим… — уголки его рта подрагивают, в глазах вспыхивает огонек. — Может, охрана поможет. Да… наверное, мне стоит позвать их.'

    menu:
        'Постой! Я потерялся… Я просто заблудился в этих залах и теперь не могу найти выход. Ты можешь мне помочь?' if soegoLogic.r1518_condition():
            # r69 # reply1518
            jump soego_s8

        'Стой! Не надо звать стражу! Я просто хочу выбраться отсюда… ты поможешь мне?' if soegoLogic.r1520_condition():
            # r70 # reply1520
            jump soego_s13

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r1521_condition():
            # r71 # reply1521
            jump soego_s19

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r1522_condition():
            # r72 # reply1522
            jump soego_s22

        'Давай, зови их. Буду рад с ними встретиться.':
            # r73 # reply1523
            jump soego_s18


# s18 # say1524
label soego_s18:  # from 6.4 9.0 9.1 9.4 16.4 17.4 40.4 40.5 41.6 50.6 53.6 61.4
    SPEAKER 'Соэго делает шаг назад, затем быстро хлопает в ладони три раза. В ответ во всем Морге раздается звон огромного железного колокола.'

    menu:
        'Ну хорошо…':
            # r74 # reply1525
            $ soegoLogic.r1525_action()
            jump soego_dispose


# s19 # say1526
label soego_s19:  # from 3.1 4.2 6.2 9.2 16.2 17.2 40.2 51.0 61.2 114.2 115.3
    SPEAKER 'До того, как он смог произнести слово, твои руки хватают его голову за виски, и ты резко сворачиваешь его голову влево.'

    menu:
        'Нельзя допустить, чтобы ты предупредил своих друзей.':
            # r75 # reply1528
            $ soegoLogic.r1528_action()
            jump soego_s20


# s20 # say1529
label soego_s20:  # from 19.0 # Check EXTERN ~DMORTE~ : 100
    SPEAKER 'В его шее раздается характерный хруст… но вместо того, чтобы безвольно упасть, тленный издает душераздирающий крик и вырывается из твоей хватки!'

    menu:
        'Что?!..' if soegoLogic.r1530_condition():
            # r76 # reply1530
            $ soegoLogic.r1530_action()
            jump soego_dispose

        'Что?!..' if soegoLogic.r1531_condition():
            # r77 # reply1531
            $ soegoLogic.r1531_action()
            jump soego_s21


# s21 # say1532
label soego_s21:  # from 20.1
    SPEAKER 'Кажется, тленный шокирован не меньше тебя. В его глазах безумие, из горла издается булькающие звуки… ты уверен, что свернул ему шею: его голова повернута под неестественным углом, но он все еще жив! Пока ты в оцепенении смотришь на него, он слабо хлопает в ладони три раза. В ответ во всем Морге раздается звон огромного железного колокола.'

    menu:
        'Ну хорошо…':
            # r78 # reply1533
            $ soegoLogic.r1533_action()
            jump soego_dispose


# s22 # say1534
label soego_s22:  # from 3.2 4.3 6.3 9.3 16.3 17.3 40.3 61.3 114.1 115.2
    SPEAKER 'Должно быть, *что-то* насторожило тленного… до того, как ты смог до него дотянуться, он отскакивает назад. В его глазах вспыхивает красный огонек, он скалит зубы. Шипя, он быстро хлопает в ладони три раза. В ответ во всем Морге раздается звон огромного железного колокола.'

    menu:
        'Ну хорошо…':
            # r79 # reply1535
            $ soegoLogic.r1535_action()
            jump soego_dispose


# s23 # say4792
label soego_s23:  # from 14.0
    SPEAKER 'Некоторые подписывают с тленными контракты, согласно которым они могут использовать их тела, как только те умрут. Возможно, тебя взяли… по недоразумению. Ты кажешься гораздо умнее, чем наши обычные зомби.'

    menu:
        'Люди продают вам свои тела после смерти?':
            # r80 # reply4793
            jump soego_s24

        'О. У меня есть другие вопросы…':
            # r81 # reply4794
            jump soego_s26

        'У меня нет времени на это. Прощай.':
            # r82 # reply4795
            jump soego_s17


# s24 # say4796
label soego_s24:  # from 23.0
    SPEAKER 'О да. В обмен на небольшую сумму многие готовы продать свое тело, которое все равно им не понадобится, когда они достигнут Истинной Смерти.'

    menu:
        'И что вы делаете с этими телами?':
            # r83 # reply4797
            jump soego_s25

        'Понятно… Ты не против, если я задам тебе пару вопросов?':
            # r84 # reply4798
            jump soego_s25

        'Спасибо за информацию. Прощай.':
            # r85 # reply4799
            jump soego_s17


# s25 # say4800
label soego_s25:  # from 24.0 24.1
    SPEAKER 'Оболочки совершают различные поручения по всему Моргу. Они перевозят тела, моют полы, помогают готовить тела к погребению… относительно несложные задачи. К сожалению, они не способны выполнять более сложные инструкции.'

    menu:
        'Хорошо, 'контрактник' я или нет, но как я попал сюда, если я не мертв?':
            # r86 # reply4801
            jump soego_s54

        'Понятно… Ты не против, если я задам тебе пару вопросов?':
            # r87 # reply4802
            jump soego_s26

        'Спасибо за информацию. Прощай.':
            # r88 # reply4803
            jump soego_s17


# s26 # say4804
label soego_s26:  # from 12.0 23.1 25.1 27.2 28.0 29.1 31.1 32.0 33.2 34.2 35.1 36.1 37.0 58.0
    SPEAKER 'Соэго кивает: Задавай свои вопросы.'

    menu:
        'Я хочу уйти. Можешь ли ты проводить меня к выходу?' if soegoLogic.r4805_condition():
            # r89 # reply4805
            jump soego_s8

        'Можешь ли ты помочь мне выбраться отсюда?' if soegoLogic.r4806_condition():
            # r90 # reply4806
            jump soego_s13

        'Тебе известно, что на втором этаже есть труп, который на самом деле — замаскированный человек?' if soegoLogic.r4807_condition():
            # r91 # reply4807
            jump soego_s27

        'Можно спросить… с тобой все в порядке? Ты выглядишь уставшим.':
            # r92 # reply4809
            $ soegoLogic.r4809_action()
            jump soego_s31

        'Ты ведь Соэго, верно? Я слышал, что ты немного странноват для тленного. Что ты похож на крысу.' if soegoLogic.r4810_condition():
            # r93 # reply4810
            $ soegoLogic.r4810_action()
            jump soego_s30

        'Ты ведь Соэго, верно? Я слышал, что ты немного странноват для тленного. Что ты похож на крысу.' if soegoLogic.r4811_condition():
            # r94 # reply4811
            jump soego_s29

        'Ты знаешь кого-нибудь по имени Фарод?' if soegoLogic.r4832_condition():
            # r95 # reply4832
            jump soego_s33

        'Я потерял дневник. Тебе ничего такого не попадалось?' if soegoLogic.r4833_condition():
            # r96 # reply4833
            jump soego_s37

        'Неважно. Мне нужно идти. Прощай.' if soegoLogic.r4834_condition():
            # r97 # reply4834
            jump soego_dispose

        'Неважно. Мне нужно идти. Прощай.' if soegoLogic.r4835_condition():
            # r98 # reply4835
            jump soego_s17


# s27 # say4808
label soego_s27:  # from 26.2
    SPEAKER 'Прошу прощения?'

    menu:
        'Наверху есть человек, замаскированный под труп. Я думаю, он шпионит за тленными.' if soegoLogic.r4836_condition():
            # r99 # reply4836
            $ soegoLogic.r4836_action()
            jump soego_s28

        'Наверху есть человек, замаскированный под труп. Я думаю, он шпионит за тленными.' if soegoLogic.r4837_condition():
            # r100 # reply4837
            $ soegoLogic.r4837_action()
            jump soego_s28

        'Забудь об этом. У меня есть несколько вопросов…':
            # r101 # reply4838
            jump soego_s26

        'Неважно. Мне нужно идти. Прощай.' if soegoLogic.r4839_condition():
            # r102 # reply4839
            jump soego_dispose

        'Неважно. Мне нужно идти. Прощай.' if soegoLogic.r916_condition():
            # r103 # reply916
            jump soego_s17


# s28 # say4840
label soego_s28:  # from 27.0 27.1
    SPEAKER 'Что? Как это могло случиться?.. Соэго внезапно переходит на шепот, его губы раскрываются, обнажая ряд острых зубов. *Анархист*, — в его глазах вспыхивает красный огонек. — Анархист. *Здесь*, — неожиданно он вспоминает о твоем присутствии и успокаивается. — Спасибо за предупреждение. Я позабочусь, чтобы стража разобралась с этим делом.'

    menu:
        'Без проблем. У меня есть еще вопросы…':
            # r104 # reply4852
            jump soego_s26

        'Неважно. Мне нужно идти. Прощай.' if soegoLogic.r4853_condition():
            # r105 # reply4853
            jump soego_dispose

        'Неважно. Мне нужно идти. Прощай.' if soegoLogic.r4854_condition():
            # r106 # reply4854
            jump soego_s17


# s29 # say4855
label soego_s29:  # from 26.5
    SPEAKER 'Ты уже готов сказать это, но вдруг останавливаешься. Глядя на Соэго, ты чувствуешь странное покалывающее чувство… почему-то тебе кажется, что не следует ничего говорить.'

    menu:
        'Я слышал о тебе кое-что странное, Соэго. Что ты похож на крысу.':
            # r107 # reply4856
            jump soego_s30

        'Э… я хотел бы у тебя кое-что спросить.':
            # r108 # reply4857
            jump soego_s26

        'Неважно. Мне нужно идти. Прощай.' if soegoLogic.r4858_condition():
            # r109 # reply4858
            jump soego_dispose

        'Неважно. Мне нужно идти. Прощай.' if soegoLogic.r4859_condition():
            # r110 # reply4859
            jump soego_s17


# s30 # say4860
label soego_s30:  # from 26.4 29.0
    SPEAKER 'На минуту Соэго в тишине изучает тебя. В его глазах вспыхивает красный огонек, и он издает легкое шипение. Кажется, ты злоупотребляешь нашим гостеприимством. Неожиданно он делает шаг назад, затем быстро хлопает в ладони три раза. В ответ во всем Морге раздается звон огромного железного колокола.'

    menu:
        'Что за?.. Что ты делаешь?':
            # r111 # reply4861
            $ soegoLogic.r4861_action()
            jump soego_dispose

        'Ну хорошо. Приготовься к смерти, Соэго.':
            # r112 # reply4862
            $ soegoLogic.r4862_action()
            jump soego_dispose


# s31 # say4863
label soego_s31:  # from 26.3
    SPEAKER 'Соэго удается слабо улыбнуться, уголки его рта слегка подрагивают. Я немного приболел… небольшая лихорадка, ничего страшного. Иногда из-за нее трудно… заснуть.'

    menu:
        'Я могу чем-нибудь помочь?':
            # r113 # reply4864
            $ soegoLogic.r4864_action()
            jump soego_s32

        'О. У меня есть другие вопросы…':
            # r114 # reply4865
            jump soego_s26

        'Ясно. Что ж, мне нужно идти. Прощай.' if soegoLogic.r4866_condition():
            # r115 # reply4866
            jump soego_dispose

        'Ясно. Что ж, мне нужно идти. Прощай.' if soegoLogic.r4867_condition():
            # r116 # reply4867
            jump soego_s17


# s32 # say4868
label soego_s32:  # from 31.0
    SPEAKER 'Соэго качает головой: Нет-нет, спасибо за заботу… Я переживу, — он немного хмурится. — Тебе нужно что-либо еще?'

    menu:
        'Да. У меня есть еще вопросы…':
            # r117 # reply4869
            jump soego_s26

        'Нет, не хочу тебя больше беспокоить. Спасибо за информацию.' if soegoLogic.r4870_condition():
            # r118 # reply4870
            jump soego_dispose

        'Нет, не хочу тебя больше беспокоить. Спасибо за информацию.' if soegoLogic.r4871_condition():
            # r119 # reply4871
            jump soego_s17


# s33 # say4872
label soego_s33:  # from 26.6
    SPEAKER 'Фарод? Ну конечно же, я знаю его, — он хмурится, в его глазах появляется красный блеск. — Отвратительный человек. Никакого уважения к мертвым, и еще меньше — к живым. Сборщик.'

    menu:
        'Сборщик?':
            # r120 # reply4873
            jump soego_s34

        'Ты знаешь, где его можно найти?':
            # r121 # reply4874
            jump soego_s36

        'О. У меня есть другие вопросы…':
            # r122 # reply4875
            jump soego_s26

        'Понятно. Кажется, мне пора идти.' if soegoLogic.r4876_condition():
            # r123 # reply4876
            jump soego_dispose

        'Понятно. Кажется, мне пора идти.' if soegoLogic.r4877_condition():
            # r124 # reply4877
            jump soego_s17


# s34 # say4878
label soego_s34:  # from 33.0 36.0
    SPEAKER 'Сборщики находят трупы и доставляют их сюда, в Морг. Затем мы заботимся о том, чтобы тела были похоронены надлежащим образом.'

    menu:
        'А если сборщик находит тело… мое, к примеру… он может принести его сюда и продать его вам?' if soegoLogic.r4879_condition():
            # r125 # reply4879
            jump soego_s35

        'А этот сборщик, Фарод… ты не знаешь, где я могу найти его?':
            # r126 # reply4880
            jump soego_s36

        'О. У меня есть другие вопросы…':
            # r127 # reply4881
            jump soego_s26

        'Ясно. Что ж, мне нужно идти. Прощай.' if soegoLogic.r4882_condition():
            # r128 # reply4882
            jump soego_dispose

        'Ясно. Что ж, мне нужно идти. Прощай.' if soegoLogic.r4883_condition():
            # r129 # reply4883
            jump soego_s17


# s35 # say4884
label soego_s35:  # from 34.0
    SPEAKER 'Да.'

    menu:
        'Хм-м. Выходит, мне крайне важно найти этого Фарода. Ты не знаешь, где я могу найти его?':
            # r130 # reply4885
            jump soego_s36

        'О. У меня есть другие вопросы…':
            # r131 # reply4886
            jump soego_s26

        'Спасибо за информацию. Прощай.' if soegoLogic.r4887_condition():
            # r132 # reply4887
            jump soego_dispose

        'Спасибо за информацию. Прощай.' if soegoLogic.r4888_condition():
            # r133 # reply4888
            jump soego_s17


# s36 # say4889
label soego_s36:  # from 33.1 34.1 35.0
    SPEAKER 'Я знаю, что он живет в Улье, в трущобах вокруг Морга, но точнее сказать не могу. Другие сборщики могут знать, если они соизволят говорить с тобой.'

    menu:
        'Еще раз, чем занимаются сборщики?':
            # r134 # reply4890
            jump soego_s34

        'О. У меня есть другие вопросы…':
            # r135 # reply4891
            jump soego_s26

        'Спасибо за информацию. Прощай.' if soegoLogic.r4892_condition():
            # r136 # reply4892
            jump soego_dispose

        'Спасибо за информацию. Прощай.' if soegoLogic.r4893_condition():
            # r137 # reply4893
            jump soego_s17


# s37 # say4894
label soego_s37:  # from 26.7
    SPEAKER 'Дневник? — Соэго выглядит растерянным. — Нет, мне он не попадался.'

    menu:
        'Ладно, неважно. У меня есть другие вопросы…':
            # r138 # reply4895
            jump soego_s26

        'Все равно спасибо. Прощай.' if soegoLogic.r4896_condition():
            # r139 # reply4896
            jump soego_dispose

        'Все равно спасибо. Прощай.' if soegoLogic.r4897_condition():
            # r140 # reply4897
            jump soego_s17


# s38 # say4898
label soego_s38:  # -
    SPEAKER 'Перед тобой сильно уставший мужчина в черной одежде. Его худощавое лицо очень бледно, похоже, он не высыпается: плечи поникли, под красными глазами огромные мешки. Он настолько погружен в раздумья, что даже не замечает тебя.'

    menu:
        'Приветствую…' if soegoLogic.r66706_condition():
            # r141 # reply66706
            $ soegoLogic.r66706_action()
            jump soego_s39

        'Приветствую…' if soegoLogic.r66707_condition():
            # r142 # reply66707
            $ soegoLogic.r66707_action()
            jump soego_s113

        'Оставить его в раздумьях.':
            # r143 # reply66708
            jump soego_dispose


# s39 # say4904
label soego_s39:  # from 38.0
    SPEAKER 'Приветствую… Человек поворачивается к тебе лицом и слегка кивает. Внезапно ты замечаешь, что его глаза не налиты кровью, а просто имеют красный оттенок. Я Соэго. Чем я могу… — неожиданно он замечает твои шрамы, при этом уголки его рта начинают подрагивать. — Прошу прощения, ты потерялся?'

    menu:
        'Да.':
            # r144 # reply4905
            jump soego_s40

        'Нет.':
            # r145 # reply4906
            jump soego_s41

        'Нет, я не потерялся. У меня есть несколько вопросов…':
            # r146 # reply4907
            jump soego_s41

        'Нет. Я просто искал выход. Прощай.':
            # r147 # reply4908
            jump soego_s41


# s40 # say4909
label soego_s40:  # from 39.0
    SPEAKER 'Тогда ладно… — уголки рта Соэго снова начинают подрагивать, словно в предвкушении. — Позволь мне позвать охрану, она выпроводит тебя. Подожди минуточку. Кажется, вот-вот он позовет охрану.'

    menu:
        'Секундочку! Пожалуйста… не надо звать охрану. Я пришел сюда на похороны, и просто заблудился в этих залах… Ты не можешь указать мне выход наружу?' if soegoLogic.r4910_condition():
            # r148 # reply4910
            jump soego_s8

        'Нет! Я не потерялся. Я оговорился.':
            # r149 # reply4911
            jump soego_s50

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r4912_condition():
            # r150 # reply4912
            jump soego_s19

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r4913_condition():
            # r151 # reply4913
            jump soego_s22

        'Уйти. Быстро.':
            # r152 # reply4914
            jump soego_s18

        'Подождать.':
            # r153 # reply4915
            jump soego_s18


# s41 # say4916
label soego_s41:  # from 39.1 39.2 39.3
    SPEAKER 'Не помню, чтобы я тебя впускал, — Соэго подозрительно смотрит на тебя, его глаза горят красными факелами. — Могу ли я поинтересоваться, что ты здесь делаешь?'

    menu:
        'Я был здесь на похоронах, отдавал дань уважения. Я уже готов уйти… вот только, кажется, я заблудился. Ты не поможешь мне найти выход?' if soegoLogic.r4917_condition():
            # r154 # reply4917
            jump soego_s8

        'Это тебя не касается.':
            # r155 # reply4918
            jump soego_s6

        'Я очнулся на одной из плит в вашей препараторской.':
            # r156 # reply4919
            jump soego_s42

        'Я хочу повидаться кое с кем.':
            # r157 # reply4920
            jump soego_s43

        'Я пришел сюда на похороны, но, похоже, произошла ошибка.' if soegoLogic.r4921_condition():
            # r158 # reply4921
            jump soego_s53

        'Наклониться вперед, будто собираясь прошептать ему что-то, а затем, когда он наклоняется в ответ, свернуть ему шею.':
            # r159 # reply4922
            jump soego_s51

        'Уйти. Быстро.':
            # r160 # reply4923
            jump soego_s18


# s42 # say4924
label soego_s42:  # from 41.2 50.2 115.0
    SPEAKER 'Он выглядит удивленным. Ты… очнулся на одной из плит наверху?'

    menu:
        'Ой, нет. Я оговорился.':
            # r161 # reply4925
            jump soego_s50

        'Да. Я знаю, в это трудно поверить, но это правда. Я очнулся на одной из ваших плит наверху.':
            # r162 # reply4926
            $ soegoLogic.r4926_action()
            jump soego_s7


# s43 # say4927
label soego_s43:  # from 41.3 50.3
    SPEAKER 'Соэго кивает. С кем ты хочешь увидеться? Я буду рад тебе показать дорогу.'

    menu:
        'Это не твое дело.':
            # r163 # reply4928
            jump soego_s6

        'Я хочу повидаться с Дхоллом.' if soegoLogic.r4929_condition():
            # r164 # reply4929
            jump soego_s44

        'Я хочу повидаться с Дейонаррой.' if soegoLogic.r4930_condition():
            # r165 # reply4930
            jump soego_s47

        'Ложь: Э… с Аданом. Он все еще работает здесь, или?..' if soegoLogic.r4931_condition():
            # r166 # reply4931
            $ soegoLogic.r4931_action()
            jump soego_s49

        'Ложь: Э… с Аданом. Он все еще работает здесь, или?..' if soegoLogic.r4932_condition():
            # r167 # reply4932
            jump soego_s49

        'Ой, ни с кем. Я оговорился.':
            # r168 # reply4933
            jump soego_s50


# s44 # say4934
label soego_s44:  # from 43.1
    SPEAKER 'Дхолла? Писаря Дхолла можно найти в приемной комнате на верхнем этаже, — уголки рта Соэго начинают подрагивать. — Он очень занят, а здоровье его подорвано. Если у тебя к нему не срочное дело, то лучше не беспокоить его.'

    menu:
        'Что не так с Дхоллом?':
            # r169 # reply4935
            jump soego_s46

        'В приемной комнате?':
            # r170 # reply4936
            jump soego_s45

        'Хорошо. Я не буду долго задерживать его. Прощай.':
            # r171 # reply4937
            jump soego_s62


# s45 # say4938
label soego_s45:  # from 44.1
    SPEAKER 'Да… в приемную комнату доставляются все тела из города. Их записывают, а затем подготавливают к погребению.'

    menu:
        'Что не так с Дхоллом?':
            # r172 # reply4939
            jump soego_s46

        'Спасибо за разъяснения. Я постараюсь, чтобы мой визит к Дхоллу был коротким. Прощай.':
            # r173 # reply4940
            jump soego_s62


# s46 # say4941
label soego_s46:  # from 44.0 45.0
    SPEAKER 'О, с ним ничего плохого. Дхолл… — Соэго клацает зубами, — очень *стар*. Его ревностная каталогизация мертвых довела его до ручки. Несомненно, смерть незамедлительно последует за его изнурительной болезнью.'

    menu:
        'Хорошо. Я не буду долго задерживать его. Прощай.':
            # r174 # reply4942
            jump soego_s62


# s47 # say4943
label soego_s47:  # from 43.2 53.2
    SPEAKER 'Дейонарру? В северо-западном мемориальном зале погребена женщина с таким именем. Ты ее ищешь?'

    menu:
        'Да… ты можешь сказать, что с ней произошло?':
            # r175 # reply4944
            jump soego_s48

        'Да. Мне нужно ее проведать.':
            # r176 # reply4945
            jump soego_s62


# s48 # say4946
label soego_s48:  # from 47.0
    SPEAKER 'Я не знаю, но она здесь уже достаточно долго. Ее отец может знать, что произошло с ней… он часто приходит сюда из своих апартаментов в Верхнем районе. Это он захотел поместить ее здесь в мемориальном зале.'

    menu:
        'Спасибо за разъяснения. Пойду, проведаю ее.':
            # r177 # reply4947
            jump soego_s62


# s49 # say4948
label soego_s49:  # from 43.3 43.4 53.3 53.4
    SPEAKER 'Адан… Глаза Соэго сужаются, красный блеск, который ты в них видел, казалось бы, усиливается. В этих залах Морга нет никого, живого или мертвого, носящего подобное имя. Его рот подергивается, и, к твоей неожиданности, он начинает принюхиваться.'

    menu:
        'Э… значит, я ошибся.':
            # r178 # reply4949
            jump soego_s50


# s50 # say4950
label soego_s50:  # from 40.1 42.0 43.5 49.0 53.1 57.1
    SPEAKER 'Уголки рта Соэго снова подергиваются, а глаза сверкают. Тогда что ты здесь делаешь?'

    menu:
        'Я был здесь на похоронах, отдавал дань уважения. Я уже готов уйти… вот только, кажется, я заблудился. Ты не поможешь мне найти выход?' if soegoLogic.r4951_condition():
            # r179 # reply4951
            jump soego_s8

        'Это не твое дело.':
            # r180 # reply4952
            jump soego_s6

        'Я очнулся на одной из плит в вашей препараторской.':
            # r181 # reply4953
            jump soego_s42

        'Я хочу повидаться кое с кем.':
            # r182 # reply4954
            jump soego_s43

        'Я пришел на похороны.' if soegoLogic.r4955_condition():
            # r183 # reply4955
            jump soego_s53

        'Наклониться вперед, будто собираясь прошептать ему что-то, а затем, когда он наклоняется в ответ, свернуть ему шею.':
            # r184 # reply4956
            jump soego_s51

        'Убежать от него.':
            # r185 # reply5028
            jump soego_s18


# s51 # say4957
label soego_s51:  # from 41.5 50.5 53.5
    SPEAKER 'Соэго хмурится, когда ты наклоняешься вперед. Ты замечаешь, что он что-то вынюхивает, что-то проверяя. Неожиданно его глаза сужаются. Похоже, он готов позвать стражу.'

    menu:
        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r4958_condition():
            # r186 # reply4958
            jump soego_s19

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r4959_condition():
            # r187 # reply4959
            jump soego_s52


# s52 # say4960
label soego_s52:  # from 51.1
    SPEAKER 'До того, как ты смог дотянуться до Соэго, он отскакивает назад. В его глазах вспыхивает красный огонек, он скалит зубы. Шипя, он быстро хлопает в ладони три раза. В ответ во всем Морге раздается звон огромного железного колокола.'

    menu:
        'Ну хорошо…':
            # r188 # reply4961
            $ soegoLogic.r4961_action()
            jump soego_dispose


# s53 # say4962
label soego_s53:  # from 41.4 50.4
    SPEAKER 'Кого хоронили? Возможно, служба проводится в другом месте Морга.'

    menu:
        'Ты не понял… хоронили МЕНЯ.':
            # r189 # reply4963
            jump soego_s57

        'Прошу прощения… Я оговорился. Мне неизвестно имя усопшего.':
            # r190 # reply4964
            jump soego_s50

        'Ее зовут Дейонарра.' if soegoLogic.r4965_condition():
            # r191 # reply4965
            jump soego_s47

        'Ложь: Его зовут… э, Адан.' if soegoLogic.r4967_condition():
            # r192 # reply4967
            $ soegoLogic.r4967_action()
            jump soego_s49

        'Ложь: Его зовут… э, Адан.' if soegoLogic.r4968_condition():
            # r193 # reply4968
            jump soego_s49

        'Наклониться вперед, будто собираясь прошептать ему что-то, а затем свернуть ему шею.':
            # r194 # reply4969
            jump soego_s51

        'Убежать от него.':
            # r195 # reply4970
            jump soego_s18


# s54 # say4966
label soego_s54:  # from 7.0 25.0
    SPEAKER 'Ну… Соэго отводит глаза в сторону. Кажется, он смущен. Кажется, произошла какая-то ошибка. Или тебя принесли твои кровные родственники, или тленные, или… — Соэго неожиданно умолкает, как будто ему в голову пришла неприятная мысль, — или один из *сборщиков*.'

    menu:
        'Сборщиков?':
            # r196 # reply4971
            jump soego_s55


# s55 # say4972
label soego_s55:  # from 54.0
    SPEAKER 'Да, сборщиков… банды собирателей, которые приносят нам мертвые тела. Они могли решить, что ты мертв, — Соэго хрипит, его глаза приобретают блеск, — они были так ослеплены медью, что не позаботились проверить тело перед тем, как доставить тебя сюда, — Соэго изучает тебя. — Тебе несказанно повезло, что ты выжил… иначе ты мог достичь Истинной Смерти раньше положенного срока.'

    menu:
        'Значит, произошла ошибка… Я хочу уйти отсюда. Немедленно.':
            # r197 # reply4973
            jump soego_s56


# s56 # say4974
label soego_s56:  # from 55.0 59.1
    SPEAKER 'Соэго кивает, уголки его рта начинают подрагивать. Ну… ладно, ладно. Позволь мне открыть главные ворота.'

    menu:
        'Хорошо.' if soegoLogic.r4975_condition():
            # r198 # reply4975
            $ soegoLogic.r4975_action()
            jump soego_dispose

        'Хорошо.' if soegoLogic.r4976_condition():
            # r199 # reply4976
            jump soego_s9


# s57 # say4977
label soego_s57:  # from 53.0
    SPEAKER 'Тебя?'

    menu:
        'Да, *меня*. Я пробудился на одной из ваших плит наверху.':
            # r200 # reply4978
            jump soego_s7

        'Прошу прощения… Я ошибся.':
            # r201 # reply4979
            jump soego_s50


# s58 # say4980
label soego_s58:  # -
    SPEAKER 'При твоем приближении Соэго вынюхивает воздух и поднимает взгляд. Увидев тебя, он хмурится. Я уже открыл для тебя ворота. Почему ты все еще здесь?'

    menu:
        'Перед тем, как уйти, я хочу задать несколько вопросов.':
            # r202 # reply4981
            jump soego_s26

        'Я как раз направляюсь к воротам. Прощай.':
            # r203 # reply4982
            jump soego_dispose


# s59 # say4983
label soego_s59:  # -
    SPEAKER 'При твоем приближении Соэго вынюхивает воздух и поднимает взгляд. Увидев тебя, он слегка кивает. Ты нашел то, что искал?'

    menu:
        'Да, спасибо. Прошу прощения, я, кажется, заблудился в этих залах… Ты не поможешь мне найти выход?' if soegoLogic.r4984_condition():
            # r204 # reply4984
            jump soego_s60

        'Да. Мне пора идти. Ты можешь указать, где выход?' if soegoLogic.r4985_condition():
            # r205 # reply4985
            jump soego_s56

        'Да, я как раз направляюсь к воротам. Прощай.':
            # r206 # reply4986
            jump soego_dispose


# s60 # say4987
label soego_s60:  # from 59.0
    SPEAKER 'Соэго кивает, уголки его рта немого подрагивают. Ну почему же… конечно. Эти залы *могут* запутать посетителей. Позволь мне открыть для тебя ворота.'

    menu:
        'Спасибо.' if soegoLogic.r4988_condition():
            # r207 # reply4988
            $ soegoLogic.r4988_action()
            jump soego_dispose

        'Спасибо.' if soegoLogic.r4989_condition():
            # r208 # reply4989
            jump soego_s9


# s61 # say4990
label soego_s61:  # from 13.2
    SPEAKER 'Очень странно В глазах Соэго вспыхивает красный огонек, уголки рта начинают подрагивать, будто в предвкушении. Может… — он широко ухмыляется, показывая ряд острых грязных зубов. — Может, мне стоит позвать охрану? Да… да, пожалуй, я так и сделаю.'

    menu:
        'Постой! Я потерялся… Я просто заблудился в этих залах и теперь не могу найти выход. Ты можешь мне помочь?' if soegoLogic.r4991_condition():
            # r209 # reply4991
            jump soego_s8

        'Стой! Не надо звать стражу! Я просто хочу выбраться отсюда… ты поможешь мне?' if soegoLogic.r4992_condition():
            # r210 # reply4992
            jump soego_s13

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r4993_condition():
            # r211 # reply4993
            jump soego_s19

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r4994_condition():
            # r212 # reply4994
            jump soego_s22

        'Давай, зови их. Буду рад с ними встретиться.':
            # r213 # reply4995
            jump soego_s18


# s62 # say4996
label soego_s62:  # from 44.2 45.1 46.0 47.1 48.0
    SPEAKER 'Соэго кивает… но его рот снова дергается. Похоже, он этого даже не замечает. Возвращайся, когда воздашь все почести, и я открою для тебя главные ворота. Уже пробило девять ударов — закончи свои дела побыстрее.'

    menu:
        'Пожалуй, отложу это до следующего раза. Ты мог бы меня отсюда выпустить?':
            # r214 # reply4997
            jump soego_s8

        'Спасибо. Я скоро вернусь.':
            # r215 # reply4998
            jump soego_dispose


# s63 # say21653
label soego_s63:  # -
    SPEAKER 'А, еще один из живущих. Большинство из тех, кто заходит так глубоко в катакомбы, убивают упыри. Тебе повезло.'

    menu:
        'Ты Соэго, из Морга. Что ты здесь делаешь?' if soegoLogic.r21655_condition():
            # r216 # reply21655
            $ soegoLogic.r21655_action()
            jump soego_s72

        'Кто ты?' if soegoLogic.r21656_condition():
            # r217 # reply21656
            $ soegoLogic.r21656_action()
            jump soego_s64

        'Где я?' if soegoLogic.r21657_condition():
            # r218 # reply21657
            $ soegoLogic.r21657_action()
            jump soego_s77

        'Почему из меня сделали заключенного?' if soegoLogic.r21658_condition():
            # r219 # reply21658
            $ soegoLogic.r21658_action()
            jump soego_s78

        'Возможно. Прощай.' if soegoLogic.r21660_condition():
            # r220 # reply21660
            $ soegoLogic.r21660_action()
            jump soego_s71


# s64 # say21661
label soego_s64:  # from 63.1 77.0 78.0
    SPEAKER 'Я Соэго, фрактотум тленных. Я занимаюсь здесь миссионерской деятельностью, — он делает полупоклон.'

    menu:
        'Миссионерской?':
            # r221 # reply21662
            jump soego_s65

        'Что здесь делают тленные?' if soegoLogic.r21663_condition():
            # r222 # reply21663
            jump soego_s66

        'Где я?':
            # r223 # reply64595
            jump soego_s77

        'Почему из меня сделали заключенного?':
            # r224 # reply64596
            jump soego_s78

        'Здравствуй и прощай.':
            # r225 # reply21665
            jump soego_s71


# s65 # say21666
label soego_s65:  # from 64.0 72.2 73.2 74.0 101.3 104.1
    SPEAKER 'Да, я пришел в эти катакомбы, когда услышал, что здесь бродит *беспокойная* нежить. Надеюсь, я смогу спасти их.'

    menu:
        'Спасти?':
            # r226 # reply21667
            jump soego_s67

        'Где я?':
            # r227 # reply64597
            jump soego_s77

        'Почему из меня сделали заключенного?':
            # r228 # reply64599
            jump soego_s78

        'Это все, что я хотел узнать. Прощай.':
            # r229 # reply21669
            jump soego_s71


# s66 # say21670
label soego_s66:  # from 64.1 72.3 73.3 74.1 101.4 104.2 109.2
    SPEAKER 'Я здесь один. Я пришел в эти катакомбы, когда услышал, что здесь бродит *беспокойная* нежить. Надеюсь, я смогу спасти их.'

    menu:
        'Спасти?':
            # r230 # reply21671
            jump soego_s67

        'Где я?':
            # r231 # reply64600
            jump soego_s77

        'Почему из меня сделали заключенного?':
            # r232 # reply64601
            jump soego_s78

        'Это все, что я хотел узнать. Прощай.':
            # r233 # reply21673
            jump soego_s71


# s67 # say21674
label soego_s67:  # from 65.0 66.0
    SPEAKER 'Да, чувства связывают их с этой ложной жизнью. Я надеюсь убедить их забыть свои чувства, покинуть эту ложную жизнь и достичь Истинной Смерти.'

    menu:
        'Ложную жизнь?':
            # r234 # reply21675
            jump soego_s68

        'Истинной Смерти?':
            # r235 # reply21676
            jump soego_s69

        'Ты хочешь, чтобы они умерли?':
            # r236 # reply21767
            jump soego_s70

        'Где я?':
            # r237 # reply64602
            jump soego_s77

        'Почему из меня сделали заключенного?':
            # r238 # reply64603
            jump soego_s78

        'Это все, что я хотел узнать. Прощай.':
            # r239 # reply21770
            jump soego_s71


# s68 # say21771
label soego_s68:  # from 67.0 69.0 70.0
    SPEAKER 'Эти… мертвецы… они так близки к Истинной Смерти… но они все еще цепляются за эту жизнь. Ложную жизнь, которая является ложью существования на этом плане.'

    menu:
        'Истинной Смерти?':
            # r240 # reply21772
            jump soego_s69

        'Ты хочешь, чтобы они умерли?':
            # r241 # reply21774
            jump soego_s70

        'Где я?':
            # r242 # reply64604
            jump soego_s77

        'Почему из меня сделали заключенного?':
            # r243 # reply64605
            jump soego_s78

        'Это все, что я хотел узнать. Прощай.':
            # r244 # reply21776
            jump soego_s71


# s69 # say21777
label soego_s69:  # from 67.1 68.0 70.1
    SPEAKER 'Полное отсутствие чувств, Истинная Смерть — это истинная жизнь за пределами тени этого существования. Это место, которого должны достичь все эти мертвецы, чтобы стать свободными.'

    menu:
        'А что это за 'ложная жизнь', про которую ты говорил?':
            # r245 # reply21779
            jump soego_s68

        'Ты хочешь, чтобы они умерли?':
            # r246 # reply21780
            jump soego_s70

        'Где я?':
            # r247 # reply64606
            jump soego_s77

        'Почему из меня сделали заключенного?':
            # r248 # reply64607
            jump soego_s78

        'Это все, что я хотел узнать. Прощай.':
            # r249 # reply21784
            jump soego_s71


# s70 # say21786
label soego_s70:  # from 67.2 68.1 69.1
    SPEAKER 'Я хочу, чтобы они переступили пределы этого плана бытия и освободились от чувств. Это спасет их.'

    menu:
        'А что это за 'ложная жизнь', про которую ты говорил?':
            # r250 # reply21788
            jump soego_s68

        'Ты что-то говорил об 'Истинной Смерти'?':
            # r251 # reply21790
            jump soego_s69

        'Где я?':
            # r252 # reply64608
            jump soego_s77

        'Почему из меня сделали заключенного?':
            # r253 # reply64609
            jump soego_s78

        'Это все, что я хотел узнать. Прощай.':
            # r254 # reply21794
            jump soego_s71


# s71 # say21799
label soego_s71:  # from 63.4 64.4 65.3 66.3 67.5 68.4 69.4 70.4 72.6 73.6 74.4 77.2 78.2 79.5 80.1 81.0 101.5 104.3 109.5 110.3 112.1
    SPEAKER 'Подожди минуту. Не нападай на здешнюю нежить: если ты не будешь их беспокоить, они не причинят тебе вреда. Будешь враждебно к ним относиться, и они начнут защищаться… а их очень много. И еще: если тебе нужно отдохнуть, ты можешь приходить сюда.'

    menu:
        'Погоди… а сейчас можно отдохнуть?' if soegoLogic.r21800_condition():
            # r255 # reply21800
            $ soegoLogic.r21800_action()
            jump soego_s84

        'Погоди… а сейчас можно отдохнуть?' if soegoLogic.r64569_condition():
            # r256 # reply64569
            $ soegoLogic.r64569_action()
            jump soego_s84

        'Уйти.':
            # r257 # reply64570
            $ soegoLogic.r64570_action()
            jump soego_dispose


# s72 # say21806
label soego_s72:  # from 63.0
    SPEAKER 'Память тебе не изменяет. Я больше не работаю в Морге… теперь я занимаюсь здесь миссионерской деятельностью.'

    menu:
        'Но я думал, что сломал тебе шею…' if soegoLogic.r64547_condition():
            # r258 # reply64547
            jump soego_s73

        'Но я думал, что *убил* тебя…' if soegoLogic.r21808_condition():
            # r259 # reply21808
            jump soego_s73

        'Миссионерской?':
            # r260 # reply21809
            jump soego_s65

        'Что здесь делают тленные?' if soegoLogic.r21811_condition():
            # r261 # reply21811
            jump soego_s66

        'Где я?':
            # r262 # reply64610
            jump soego_s77

        'Почему из меня сделали заключенного?':
            # r263 # reply64611
            jump soego_s78

        'Это все, что я хотел узнать. Прощай.':
            # r264 # reply21813
            jump soego_s71


# s73 # say21814
label soego_s73:  # from 72.0 72.1 109.0 109.1
    SPEAKER 'Нанесенное тобой увечье было несмертельным. Я быстро восстановился… и понял, что хочу быть подальше от Морга.'

    menu:
        'Соэго, я свернул тебе шею… разве это не смертельно?' if soegoLogic.r21815_condition():
            # r265 # reply21815
            jump soego_s101

        'Ты не сердишься?':
            # r266 # reply21816
            jump soego_s74

        'Хм… Ты говорил, что ты миссионер?':
            # r267 # reply21817
            jump soego_s65

        'Тогда ладно. Что здесь делают тленные?' if soegoLogic.r21818_condition():
            # r268 # reply21818
            jump soego_s66

        'Ладно… Так где же я?':
            # r269 # reply64612
            jump soego_s77

        'Понятно… Почему из меня сделали заключенного?':
            # r270 # reply64613
            jump soego_s78

        'Забудь; это все, что я хотел узнать. Farewell.':
            # r271 # reply21820
            jump soego_s71


# s74 # say21821
label soego_s74:  # from 73.1 101.2 104.0
    SPEAKER 'Нет… а должен? Я был немного расстроен, я не собирался уходить оттуда. Однако тебе все равно не стоит возвращаться в Морг: немногие знакомые мне фрактотумы будут рады повстречать тебя.'

    menu:
        'Ты говорил, что ты миссионер?':
            # r272 # reply64614
            jump soego_s65

        'Что здесь делают тленные?' if soegoLogic.r21823_condition():
            # r273 # reply21823
            jump soego_s66

        'Где я?':
            # r274 # reply64615
            jump soego_s77

        'Почему из меня сделали заключенного?':
            # r275 # reply64616
            jump soego_s78

        'Это все, что я хотел узнать. Прощай.':
            # r276 # reply21825
            jump soego_s71


# s75 # say21716
label soego_s75:  # -
    SPEAKER 'Null node.'

    jump soego_dispose

# s76 # say21832
label soego_s76:  # -
    SPEAKER 'Null node.'

    jump soego_dispose

# s77 # say21837
label soego_s77:  # from 63.2 64.2 65.1 66.1 67.3 68.2 69.2 70.2 72.4 73.4 74.2 78.1 109.3
    SPEAKER 'Ты в катакомбах Мертвых Народов. Сюда тебя привела стража.'

    menu:
        'А кто ты?':
            # r277 # reply21840
            jump soego_s64

        'Почему из меня сделали заключенного?':
            # r278 # reply21841
            jump soego_s78

        'Это все, что я хотел узнать. Прощай.':
            # r279 # reply21843
            jump soego_s71


# s78 # say21844
label soego_s78:  # from 63.3 64.3 65.2 66.2 67.4 68.3 69.3 70.3 72.5 73.5 74.3 77.1 109.4
    SPEAKER 'Не знаю. Поспрашивай здешних 'жителей'.'

    menu:
        'А кто ты?':
            # r280 # reply21847
            jump soego_s64

        'Где я?':
            # r281 # reply21848
            jump soego_s77

        'Возможно. Прощай.':
            # r282 # reply21850
            jump soego_s71


# s79 # say21851
label soego_s79:  # -
    SPEAKER 'О, к нам подоспела подмога! Меня, как агента Многоединого, предупредили о твоем приходе. Нам нужно, чтобы ты попытался пробраться в тронный зал Безмолвного Короля и убил его. Сделай это, и Многоединый вознаградит тебя.'

    menu:
        'Соэго… Эморик хотел знать, где ты.' if soegoLogic.r66181_condition():
            # r283 # reply66181
            $ soegoLogic.r66181_action()
            jump soego_s110

        'Погоди, ты Соэго? Эморик искал тебя.' if soegoLogic.r21852_condition():
            # r284 # reply21852
            $ soegoLogic.r21852_action()
            jump soego_s110

        'Постой… разве я не сломал тебе шею в Морге?' if soegoLogic.r64623_condition():
            # r285 # reply64623
            $ soegoLogic.r64623_action()
            jump soego_s112

        'Постой… Я думал, что *убил* тебя в Морге…' if soegoLogic.r64624_condition():
            # r286 # reply64624
            $ soegoLogic.r64624_action()
            jump soego_s112

        'Как мне добраться до Безмолвного Короля?' if soegoLogic.r21853_condition():
            # r287 # reply21853
            $ soegoLogic.r21853_action()
            jump soego_s80

        'Понятно. Тогда прощай.' if soegoLogic.r21854_condition():
            # r288 # reply21854
            $ soegoLogic.r21854_action()
            jump soego_s71


# s80 # say21858
label soego_s80:  # from 79.4 110.2 112.0
    SPEAKER 'Не знаю… Я здесь уже много времени, но так и не смог найти способ попасть в его тронный зал. Может быть, тебе повезет больше, без той ненависти и фанатизма, которые направлены на меня.'

    menu:
        'Ненависти и фанатизма?':
            # r289 # reply21860
            jump soego_s81

        'Понятно. Тогда прощай.':
            # r290 # reply21862
            jump soego_s71


# s81 # say21864
label soego_s81:  # from 80.0
    SPEAKER 'Взгляды моей фракции популярны, но не все их принимают. Многие важные персоны этой цивилизации не питают к ним теплых чувств.'

    menu:
        'Понятно. Тогда прощай.':
            # r291 # reply21870
            jump soego_s71


# s82 # say21913
label soego_s82:  # -
    SPEAKER 'А, рад встрече.'

    menu:
        'Безмолвный Король мертв, и уже давно. Нет *никакого* Безмолвного Короля.' if soegoLogic.r24206_condition():
            # r292 # reply24206
            $ soegoLogic.r24206_action()
            jump soego_s94

        'Безмолвный Король мертв, и уже давно. Нет *никакого* Безмолвного Короля.' if soegoLogic.r21915_condition():
            # r293 # reply21915
            $ soegoLogic.r21915_action()
            jump soego_s94

        'Ты Соэго? Эморик тебя искал.' if soegoLogic.r21914_condition():
            # r294 # reply21914
            $ soegoLogic.r21914_action()
            jump soego_s109

        'Я был в твоей комнате и видел твой дневник.' if soegoLogic.r21916_condition():
            # r295 # reply21916
            $ soegoLogic.r21916_action()
            jump soego_s100

        'В одном из коридоров неподалеку я встретил скелета, который находится на грани принятия Истинной Смерти.' if soegoLogic.r21917_condition():
            # r296 # reply21917
            $ soegoLogic.r21917_action()
            jump soego_s97

        'Мне нужно отдохнуть.' if soegoLogic.r21920_condition():
            # r297 # reply21920
            jump soego_s84

        'Нам нужно отдохнуть.' if soegoLogic.r21922_condition():
            # r298 # reply21922
            jump soego_s84

        'У меня есть несколько вопросов…':
            # r299 # reply21924
            jump soego_s83

        'Я просто проходил мимо. Прощай.':
            # r300 # reply21925
            jump soego_dispose


# s83 # say21943
label soego_s83:  # from 82.7 88.0 89.1 90.0 91.1 92.0 94.1 94.3 111.0
    SPEAKER 'Я отвечу, если смогу.'

    menu:
        'Расскажи мне о Харгримме.' if soegoLogic.r21944_condition():
            # r301 # reply21944
            jump soego_s88

        'Расскажи мне об Акасте.' if soegoLogic.r21945_condition():
            # r302 # reply21945
            jump soego_s89

        'Расскажи мне о Тухлой Мери.' if soegoLogic.r21946_condition():
            # r303 # reply21946
            jump soego_s90

        'Расскажи мне о Безмолвном Короле.' if soegoLogic.r21947_condition():
            # r304 # reply21947
            jump soego_s91

        'Что тебе известно об этой 'цивилизации'?' if soegoLogic.r21948_condition():
            # r305 # reply21948
            jump soego_s92

        'Что тебе известно об этой 'цивилизации'?' if soegoLogic.r21949_condition():
            # r306 # reply21949
            jump soego_s93

        'Неважно. Прощай.':
            # r307 # reply21951
            jump soego_dispose


# s84 # say21954
label soego_s84:  # from 71.0 71.1 82.5 82.6
    SPEAKER 'Конечно. Здесь ты будешь в безопасности.'

    menu:
        'Спасибо…':
            # r308 # reply21956
            $ soegoLogic.r21956_action()
            jump soego_dispose


# s85 # say21958
label soego_s85:  # -
    SPEAKER 'Null Node.'

    jump soego_dispose

# s86 # say21963
label soego_s86:  # -
    SPEAKER 'Null Node.'

    jump soego_dispose

# s87 # say21969
label soego_s87:  # -
    SPEAKER 'Null Node.'

    jump soego_dispose

# s88 # say21975
label soego_s88:  # from 83.0 91.0
    SPEAKER 'Упрямец, и все же он поражает своей верностью и преданностью долгу. Здесь он мой сильнейший противник, и он уже многие годы не дает распасться этой цивилизации. Его страсти являются результатом его верности и преданности долгу… замечательные качества, однако направлены не в то русло.'

    menu:
        'У меня есть другие вопросы…':
            # r309 # reply21976
            jump soego_s83

        'Это все, что я хотел узнать. Прощай.':
            # r310 # reply21977
            jump soego_dispose


# s89 # say21978
label soego_s89:  # from 83.1
    SPEAKER 'Акаста — животное. Боюсь, что Безмолвный Король — единственный, кто держит ее под контролем. Если его устранят, упыри разорят катакомбы.'

    menu:
        'Расскажи мне о Безмолвном Короле.':
            # r311 # reply21979
            $ soegoLogic.r21979_action()
            jump soego_s91

        'У меня есть другие вопросы…':
            # r312 # reply21980
            jump soego_s83

        'Это все, что я хотел узнать. Прощай.':
            # r313 # reply21981
            jump soego_dispose


# s90 # say21982
label soego_s90:  # from 83.2
    SPEAKER 'Тухлая Мери — добрая душа, хоть и медлительная. Я почти ничего не понимаю из того, что она говорит, но она и другие зомби не склонны к насилию.'

    menu:
        'У меня есть другие вопросы…':
            # r314 # reply21983
            jump soego_s83

        'Это все, что я хотел узнать. Прощай.':
            # r315 # reply21984
            jump soego_dispose


# s91 # say21985
label soego_s91:  # from 83.3 89.0
    SPEAKER 'Я никогда не видел Безмолвного Короля. Хотел бы я что-нибудь о нем рассказать, но никогда с ним не встречался. Его тронный зал, предположительно, находится за красными дверями, но я не могу получить туда доступ… Харгримм, скелет-жрец, не даст мне его.'

    menu:
        'Расскажи мне о Харгримме.':
            # r316 # reply21986
            $ soegoLogic.r21986_action()
            jump soego_s88

        'У меня есть другие вопросы…':
            # r317 # reply21987
            jump soego_s83

        'Это все, что я хотел узнать. Прощай.':
            # r318 # reply21988
            jump soego_dispose


# s92 # say21989
label soego_s92:  # from 83.4
    SPEAKER 'Думаю, они здесь уже много веков, пекутся о тех, кто скончался в их залах. Подобная преданность долгу больше не нужна… да она почти что преступна.'

    menu:
        'У меня есть другие вопросы…':
            # r319 # reply21990
            jump soego_s83

        'Это все, что я хотел узнать. Прощай.':
            # r320 # reply21991
            jump soego_dispose


# s93 # say21992
label soego_s93:  # from 83.5 # Check EXTERN ~DMORTE~ : 220
    SPEAKER 'Думаю, они здесь уже много веков, пекутся о тех, кто скончался в их залах. Подобная преданность долгу больше не нужна… да она почти что преступна.'

    jump soego_dispose

# s94 # say21993
label soego_s94:  # from 82.0 82.1
    SPEAKER 'Что? Это правда? Ах, Многоединый наверняка вознаградит тебя за такие вести…'

    menu:
        'Многоединый?' if soegoLogic.r25248_condition():
            # r321 # reply25248
            $ soegoLogic.r25248_action()
            jump soego_s111

        'Интересно. У меня есть другие вопросы…' if soegoLogic.r25252_condition():
            # r322 # reply25252
            $ soegoLogic.r25252_action()
            jump soego_s83

        'Возможно. Прощай.' if soegoLogic.r25253_condition():
            # r323 # reply25253
            $ soegoLogic.r25253_action()
            jump soego_dispose

        'Хорошо. У меня есть несколько вопросов…' if soegoLogic.r21994_condition():
            # r324 # reply21994
            $ soegoLogic.r21994_action()
            jump soego_s83

        'Тогда я расскажу ему сам. Прощай.' if soegoLogic.r21995_condition():
            # r325 # reply21995
            $ soegoLogic.r21995_action()
            jump soego_dispose


# s95 # say21997
label soego_s95:  # -
    SPEAKER 'Ах ты… сволочь!'

    menu:
        'Чт…':
            # r326 # reply21998
            $ soegoLogic.r21998_action()
            jump soego_dispose


# s96 # say22003
label soego_s96:  # -
    SPEAKER 'Э… эта дверь ведет в мои личные покои. Прошу туда не входить.'

    menu:
        'Уйти.':
            # r327 # reply22004
            jump soego_dispose


# s97 # say22005
label soego_s97:  # from 82.4
    SPEAKER 'О! Пойду поговорю с ним!'

    menu:
        'Прощай.':
            # r328 # reply22006
            jump soego_dispose


# s98 # say22007
label soego_s98:  # -
    SPEAKER 'Нет, я уже иду.'

    menu:
        'Прощай.':
            # r329 # reply22008
            jump soego_dispose


# s99 # say22009
label soego_s99:  # -
    SPEAKER 'К сожалению, нет. Но все еще образуется.'

    menu:
        'Понятно. Прощай.':
            # r330 # reply22010
            jump soego_dispose


# s100 # say22011
label soego_s100:  # from 82.3
    SPEAKER 'Соэго некоторое время молчит. Все ясно. Неожиданно он начинает превращаться…'

    menu:
        'Какого?..':
            # r331 # reply22012
            $ soegoLogic.r22012_action()
            jump soego_dispose


# s101 # say22014
label soego_s101:  # from 73.0
    SPEAKER 'Э… твоя память тебе изменяет. Конечно, моя шея слегка побаливала… она немного вывернута. Но сломана? Вряд ли.'

    menu:
        'Позволь не согласиться. Что ты такое, Соэго?' if soegoLogic.r22015_condition():
            # r332 # reply22015
            jump soego_s106

        'Позволь не согласиться. Что ты такое, Соэго?' if soegoLogic.r22016_condition():
            # r333 # reply22016
            jump soego_s103

        'Ты не сердишься?':
            # r334 # reply22018
            jump soego_s74

        'Ты говорил, что ты миссионер?':
            # r335 # reply22019
            jump soego_s65

        'Что здесь делают тленные?' if soegoLogic.r22020_condition():
            # r336 # reply22020
            jump soego_s66

        'Это все, что я хотел узнать. Прощай.':
            # r337 # reply22022
            jump soego_s71


# s102 # say22023
label soego_s102:  # -
    SPEAKER 'Он вырывается из твоей хватки со сверхъестественной скоростью. Фыркая и плюясь, он шипит: Глупая тварь, ты напал на агента коллективного разума черепных крыс! Внезапно он начинает пугающе преображаться…'

    menu:
        'Какого?..':
            # r338 # reply22024
            $ soegoLogic.r22024_action()
            jump soego_dispose


# s103 # say22026
label soego_s103:  # from 101.1
    SPEAKER 'Глупый вопрос! Ты очнулся на препараторской плите в Морге… ты сам мне об этом рассказал. Несомненно, мое ранение было получше того, из-за которого сборщики перепутали тебя с трупом, не так ли?'

    menu:
        'Все так, но… неважно.':
            # r339 # reply22027
            jump soego_s104

        'У меня… особый случай.':
            # r340 # reply22028
            jump soego_s105

        'Нет. Что-то здесь не так, и я очень скоро выясню это.':
            # r341 # reply22029
            jump soego_s104


# s104 # say22032
label soego_s104:  # from 103.0 103.2 105.0 105.1 106.1 107.0
    SPEAKER 'Ну и ладно, — он пожимает плечами.'

    menu:
        'Ты не злишься насчет того, что произошло?':
            # r342 # reply22033
            jump soego_s74

        'Ты говорил, что ты миссионер?':
            # r343 # reply22034
            jump soego_s65

        'Так что же тленные делают здесь?' if soegoLogic.r22035_condition():
            # r344 # reply22035
            jump soego_s66

        'Я уже должен идти. Прощай.':
            # r345 # reply22038
            jump soego_s71


# s105 # say22039
label soego_s105:  # from 103.1
    SPEAKER 'Он улыбается. У каждого свои причуды. Уверен, этого ты не станешь отрицать?'

    menu:
        'Все так, но… неважно.':
            # r346 # reply22040
            jump soego_s104

        'Нет. Что-то здесь не так, и я очень скоро выясню это.':
            # r347 # reply22041
            jump soego_s104


# s106 # say22043
label soego_s106:  # from 101.0
    SPEAKER 'Что я такое?.. Что за вопрос такой?'

    menu:
        'Ты слышал меня. Ты ведь не обычный тленный… что ты *такое*, Соэго?':
            # r348 # reply22044
            jump soego_s107

        'Забудь. Неважно.':
            # r349 # reply22045
            jump soego_s104


# s107 # say22047
label soego_s107:  # from 106.0
    SPEAKER 'Соэго бросает на тебя угрюмый взгляд: Я не понимаю, о *чем* ты говоришь.'

    menu:
        'Что-то здесь не так, и я очень скоро выясню это.':
            # r350 # reply22048
            jump soego_s104


# s108 # say22050
label soego_s108:  # -
    SPEAKER 'А, еще один из живущих. Большинство из тех, кто заходит так глубоко в катакомбы, убивают упыри. Тебе повезло.'

    menu:
        'Ты Соэго? Эморик тебя искал.' if soegoLogic.r22051_condition():
            # r351 # reply22051
            $ soegoLogic.r22051_action()
            jump soego_s109

        'Соэго… Эморик хотел знать, где ты.' if soegoLogic.r66173_condition():
            # r352 # reply66173
            $ soegoLogic.r66173_action()
            jump soego_s109


# s109 # say22053
label soego_s109:  # from 82.2 108.0 108.1
    SPEAKER 'Да, это я. Я занимаюсь здесь миссионерской деятельностью для тленных.'

    menu:
        'Хорошо. Но… я думал, что сломал тебе шею…' if soegoLogic.r64617_condition():
            # r353 # reply64617
            jump soego_s73

        'Хорошо. Но… я думал, что *убил* тебя…' if soegoLogic.r64618_condition():
            # r354 # reply64618
            jump soego_s73

        'Здесь есть другие тленные?':
            # r355 # reply22054
            jump soego_s66

        'Где я?':
            # r356 # reply50792
            jump soego_s77

        'Почему из меня сделали заключенного?':
            # r357 # reply50793
            jump soego_s78

        'Я уже должен идти. Прощай.':
            # r358 # reply22056
            jump soego_s71


# s110 # say22057
label soego_s110:  # from 79.0 79.1
    SPEAKER 'Да, это я.'

    menu:
        'Постой… разве я не сломал тебе шею в Морге?' if soegoLogic.r64625_condition():
            # r359 # reply64625
            jump soego_s112

        'Постой… Я думал, что *убил* тебя в Морге…' if soegoLogic.r64626_condition():
            # r360 # reply64626
            jump soego_s112

        'Хорошо. Так как же мне добраться до Безмолвного Короля?' if soegoLogic.r22058_condition():
            # r361 # reply22058
            $ soegoLogic.r22058_action()
            jump soego_s80

        'Хорошо. Прощай.' if soegoLogic.r22060_condition():
            # r362 # reply22060
            jump soego_s71


# s111 # say25249
label soego_s111:  # from 94.0
    SPEAKER 'Да, коллективный разум черепных крыс. Иди на восток от Катакомб плачущих камней. Там ты найдешь дорогу.'

    menu:
        'Интересно. У меня есть другие вопросы…':
            # r363 # reply25250
            jump soego_s83

        'Возможно. Прощай.':
            # r364 # reply25251
            jump soego_dispose


# s112 # say64620
label soego_s112:  # from 79.2 79.3 110.0 110.1
    SPEAKER 'Он отмахивается от твоих слов: Ничто, для меня это ничто. Я уже был благославлен ликантропией — любые раны восстанавливаются очень быстро.'

    menu:
        'Понятно… Так как мне добраться до Безмолвного Короля?':
            # r365 # reply64621
            jump soego_s80

        'Хорошо… Тогда прощай.':
            # r366 # reply64622
            jump soego_s71


# s113 # say66709
label soego_s113:  # from 38.1
    SPEAKER 'Приветствую… Человек поворачивается к тебе лицом и делает небольшой поклон. Внезапно ты замечаешь, что его глаза не налиты кровью, а просто имеют красный оттенок. Я Соэго. Чем могу помочь?'

    menu:
        'Я хотел бы покинуть Морг. Ты не поможешь мне?':
            # r367 # reply66712
            jump soego_s114

        'У меня есть несколько вопросов…':
            # r368 # reply66713
            jump soego_s114

        'Я просто проходил мимо. Прощай.':
            # r369 # reply66714
            jump soego_dispose


# s114 # say66710
label soego_s114:  # from 113.0 113.1
    SPEAKER 'Пока ты произносишь фразу, губы Соэго неожиданно раскрываются, обнажая ряд грязных и острых зубов. Он наклоняется вперед, обнюхивая тебя.'

    menu:
        'Эй… какого черта ты меня обнюхиваешь?':
            # r370 # reply66715
            jump soego_s115

        'Свернуть ему шею, пока он наклоняется вперед.' if soegoLogic.r66716_condition():
            # r371 # reply66716
            jump soego_s22

        'Свернуть ему шею, пока он наклоняется вперед.' if soegoLogic.r66717_condition():
            # r372 # reply66717
            jump soego_s19

        'Неважно… Прощай.':
            # r373 # reply66718
            jump soego_s17


# s115 # say66711
label soego_s115:  # from 114.0
    SPEAKER 'Твоя одежда… эта мантия. Она не так пахнет. Она не твоя, — губы Соэго растягиваются в странной улыбке, глаза горят диким огнем. — Кто ты *такой*?'

    menu:
        'Я… одолжил эту мантию. Я очнулся на одной из плит наверху в препараторской.':
            # r374 # reply66719
            jump soego_s42

        'Ты прав. Эта мантия не моя, но чья она — это не твое дело.':
            # r375 # reply66720
            jump soego_s6

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r66721_condition():
            # r376 # reply66721
            jump soego_s22

        'Свернуть ему шею до того, как он сможет позвать на помощь.' if soegoLogic.r66722_condition():
            # r377 # reply66722
            jump soego_s19

        'Это неважно. Мне пора идти.':
            # r378 # reply66723
            jump soego_s17
