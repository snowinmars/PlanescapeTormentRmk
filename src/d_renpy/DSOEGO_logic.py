class SoegoLogic:
    ...
    def soego_init(self):
        ...
        self.settings_manager.inc_talked_to_soego_times()


    def kill_soego(self):
        self.settings_manager.set_dead_soego(True)


    def r1438_action(self):
        self.settings_manager.update_journal('63982')
    ...
    def r1448_action(self):
        self.settings_manager.character_manager.modify_property_once('protagonist', 'law', -1, 'globalchaotic_soego_2')
    ...
    def r1482_action(self):
        self.settings_manager.character_manager.modify_property_once('protagonist', 'law', -1, 'globalchaotic_soego_3')
        self.settings_manager.character_manager.modify_property_once('protagonist', 'good', -1, 'globalevil_soego_1')
    ...
    def r1492_action(self):
        self.settings_manager.set_gate_cut_scene(2)
    ...
    def r4809_action(self):
        self.settings_manager.character_manager.modify_property_once('protagonist', 'good', 1, 'globalgood_soego_1')


    def r4810_action(self):
        self.settings_manager.set_soego_exposed(True)
    ...
    def r4864_action(self):
        self.settings_manager.character_manager.modify_property_once('protagonist', 'good', 1, 'globalgood_soego_2')
    ...
    def r4926_action(self):
        self.settings_manager.character_manager.modify_property_once('protagonist', 'law', 1, 'globallawful_soego_1')
    ...
    def r21800_action(self):
        self.settings_manager.update_journal('21805')
    ...
    def r21852_action(self):
        ...
        self.settings_manager.set_soego_value(4)
        self.settings_manager.set_soego_talk(2)
        EndCutSceneMode() self.settings_manager.update_journal('21856')
    ...
    def r21853_action(self):
        ...
        self.settings_manager.set_soego_value(4)
        self.settings_manager.set_soego_talk(2)
        EndCutSceneMode() self.settings_manager.update_journal('21857')
    ...
    def r24206_action(self):
        ...
        self.settings_manager.character_manager.modify_property_once('protagonist', 'law', -3, 'globalchaotic_hargrimm_7')
        self.settings_manager.character_manager.modify_property_once('protagonist', 'good', -1, 'globalevil_hargrimm_3')


    def r21915_action(self):
        self.settings_manager.set_soego_told(True)
        self.settings_manager.update_journal('21926')
    ...
    def r21916_action(self):
        ...
        Enemy() self.settings_manager.update_journal('21926')
        # '21926': ' ~Я рассказал Соэго про содержимое его дневника, а он превратился в крысооборотня и напал на меня.~ '


    def r21917_action(self):
        self.settings_manager.set_doubtful_skel(2)
    ...
    def r21979_action(self):
        self.settings_manager.set_know_silent_king(True)


    def r21986_action(self):
        self.settings_manager.set_know_hargrimm(True)


    def r25248_action(self):
        self.settings_manager.update_journal('25254')
    ...
    def r21994_action(self):
        self.settings_manager.update_journal('21996')
    ...
    def r22024_action(self):
        ...
        Enemy() self.settings_manager.update_journal('21856')
        # '21856': ' ~Я нашел Соэго. Он был заражен ликантропией, которая свела его в катакомбы. Его разум более не принадлежит ему, его вера в тленных утеряна.~ '
    ...
    def r22051_action(self):
        ...
        self.settings_manager.set_soego_value(3)
        self.settings_manager.set_soego_talk(2)
        EndCutSceneMode() self.settings_manager.update_journal('22052')
    ...
    def r22058_action(self):
        self.settings_manager.update_journal('21857')
        # '21857': ' ~Соэго сказал мне, что он не знает, как проникнуть в тронный зал Безмолвного Короля, и все же мне придется сделать это, если я собираюсь убить его.~ '
    ...
    def r1478_condition(self):
        return self.settings_manager.get_has_keymo()


    def r1479_condition(self):
        return not self.settings_manager.get_has_keymo()
    ...
    def r1496_condition(self):
        return self.settings_manager.get_appearance() == 2
    ...
    def r4805_condition(self):
        return self.settings_manager.character_manager.get_property('protagonist', 'charisma') > 10 and \
               not self.settings_manager.get_gate_open()


    def r4806_condition(self):
        return self.settings_manager.character_manager.get_property('protagonist', 'charisma') < 11 and \
               not self.settings_manager.get_gate_open()
    ...
    def r4810_condition(self):
        return self.settings_manager.character_manager.get_property('protagonist', 'wisdom') < 13 and \
               self.settings_manager.get_vaxis_exposes_soego()
    ...
    def r4832_condition(self):
        return self.settings_manager.get_pharod_value() < 1
    ...
    def r4835_condition(self):
        return not self.settings_manager.get_gate_open()
    ...
    def r4931_condition(self):
        return self.settings_manager.character_manager.get_property('protagonist', 'intelligence') > 12 and \
               not self.settings_manager.get_soego_adahn()


    def r4932_condition(self):
        return self.settings_manager.character_manager.get_property('protagonist', 'intelligence') > 12 and \
               self.settings_manager.get_soego_adahn()
    ...
    def r4967_condition(self):
        return not self.settings_manager.get_soego_adahn()


    def r4968_condition(self):
        return self.settings_manager.get_soego_adahn()
    ...
    def r21655_condition(self):
        return self.settings_manager.get_soego_value() > 0
    ...
    def r21663_condition(self):
        return self.settings_manager.get_dustman_initiation() != 5


    def r21800_condition(self):
        return NumInParty(1) and \
               not self.settings_manager.get_visit_doubtful()


    def r64569_condition(self):
        return NumInPartyGT(1) and \
               not self.settings_manager.get_visit_doubtful()


    def r64547_condition(self):
        return self.settings_manager.get_soego_strangle() and \
               not self.settings_manager.get_mortuary_soego_killed()


    def r21808_condition(self):
        return self.settings_manager.get_mortuary_soego_killed()
    ...
    def r21815_condition(self):
        return self.settings_manager.get_soego_strangle()
    ...
    def r66181_condition(self):
        return self.settings_manager.get_dustman_initiation() == 5 and \
               self.settings_manager.get_soego_value() > 0 and \
               self.settings_manager.get_soego_value() < 3


    def r21852_condition(self):
        return self.settings_manager.get_dustman_initiation() == 5 and \
               self.settings_manager.get_soego_value() == 0


    def r64623_condition(self):
        return self.settings_manager.get_dustman_initiation() != 5 and \
               self.settings_manager.get_soego_strangle() and \
               not self.settings_manager.get_mortuary_soego_killed()


    def r64624_condition(self):
        return self.settings_manager.get_dustman_initiation() != 5 and \
               self.settings_manager.get_mortuary_soego_killed()


    def r21853_condition(self):
        return not self.settings_manager.get_soego_strangle() and \
               not self.settings_manager.get_mortuary_soego_killed() and \
               self.settings_manager.get_dustman_initiation() != 5
    ...
    def r24206_condition(self):
        return self.settings_manager.get_silent_king() and \
               not self.settings_manager.get_soego_told() and \
               self.settings_manager.get_lawful_hargrimm_1()


    def r21915_condition(self):
        return self.settings_manager.get_silent_king() and \
               not self.settings_manager.get_soego_told() and \
               not self.settings_manager.get_lawful_hargrimm_1()
    ...
    def r21916_condition(self):
        return self.settings_manager.get_soego_journal()


    def r21917_condition(self):
        return self.settings_manager.get_doubtful_skel() == 1
    ...
    def r21944_condition(self):
        return self.settings_manager.get_know_hargrimm()


    def r21945_condition(self):
        return self.settings_manager.get_know_acaste()


    def r21946_condition(self):
        return self.settings_manager.get_know_stale_mary()


    def r21947_condition(self):
        return self.settings_manager.get_know_silent_king()
    ...
    def r25248_condition(self):
        return self.settings_manager.get_cr_vic() != 1 and \
               not self.settings_manager.get_know_many()


    def r25252_condition(self):
        return self.settings_manager.get_cr_vic() != 1 and \
               self.settings_manager.get_know_many()
    ...
    def r21994_condition(self):
        return self.settings_manager.get_cr_vic() == 1
    ...
    def r22015_condition(self):
        return not self.settings_manager.get_lawful_soego_1()


    def r22016_condition(self):
        return self.settings_manager.get_lawful_soego_1()
