...
label vaxis_s3: # from 2.2 5.2
    nr 'Ты уже почти отвернулся, как'
    vaxis '«Зомби»'
    nr 'Начинает что-то бормотать… кажется, он пытается что-то сказать, но с зашитым ртом это сделать трудно.'
    vaxis '«Фто ТЫ? Фто тее нао?»'
...
label vaxis_s4: # from 3.5 6.5 7.8 8.5 10.4 11.4 12.2 13.5 14.4 15.2 16.4 17.2 18.1 19.3 20.1 25.6 27.6 31.6 32.5 34.2 35.6 59.1 74.1 75.3 76.1
    ...
    vaxis '«Никоу ничео не говои пво МЕЯ. Моучи. Не говои НИФЕО твуфявым».'
    nr 'Он прикладывает палец к губам.'
    vaxis '«Фффф».'
    nr 'После этого он проводит пальцем по горлу.'
    vaxis '«Или ты уфнефь нафегда. ПОЯЛ мея?»'
...
label vaxis_s6: # from 2.0 2.1 5.0 5.1
    vaxis '«Зомби»'
    nr 'Пытается что-то сказать сквозь зашитые губы. На его лице странная смесь испуга и злобы.'
    vaxis '«Фто ТЫ? Фего тее нао?»'
...
label vaxis_s7: # from 3.0 3.1 3.2 3.4 6.0 6.1 6.2 6.4
    ...
    vaxis '«Фто ты фдефь делаефь?»'
    nr 'Его глаза недоверчиво сужаются.'
    vaxis '«Фпионифь за Мертфяками?»'
...
label vaxis_s12: # from 7.0 11.0
    ...
    vaxis '«Пвоулемы? Фево ты натвовил?»'
...
label vaxis_s16: # from 13.1
    nr 'Он смотрит на тебя, затем начинает шипеть и отмахиваться от тебя.'
    vaxis '«Пуфтогоовый! Приурок! Профь от мея, пей! Профь!»'
...
label vaxis_s17: # from 7.5 10.1 14.1 16.1 25.3 27.3
    ...
    vaxis '«Ты подеиффя фо мной фвоим фекветом, я подеюфь фвоим ф *тоой*. Фдефь у мея пряфуффа друфья, у тея фдефь *никоо*. Тее фдефь не мефто. Твуфьявые тея уют. Я фбегу».'
...
label vaxis_s18: # from 17.0
    ...
    vaxis '«Ты ПЫТАЕФФЯ фапифать мея ф кьигу мертфых? У мея фдефь пвяфуффа двуфья, у тея фдефь *никоо*. Твониф мея — они тея пвиконфят».'
...
label vaxis_s19: # from 7.6 10.2 14.2 16.2 25.4 27.4
    nr 'На миг он кажется испуганным, потом осматривает тебя, и по его лицу расползается ухмылка.'
    vaxis '«ТЫ пытаеффя фапифать МЕЯ ф кьигу мертфых? У мея фдефь пвячуффа двуфья, у тея фдефь *никоо*. Твонеф мея — они тея пвиконфят».'
...
label vaxis_s20: # from 19.2
    nr 'Глаза зомби превращаются в щелочки, он шипит на тебя:'
    vaxis '«Ты подеилфя фо мной фвоим фекретом, я подеюфь фоим ф *тоой*. Фдефь у мея прячуффа друфья, у тея фдефь *никоо*. Твуфьявые тея уют. Я фбегу».'
...
label vaxis_s21: # from 7.4 10.0 14.0 16.0 19.1 25.2 27.2
    ...
    vaxis '«Не-не-не! Не наа фвать фтражу!»'
    nr 'Он явно напуган.'
    vaxis '«Я-я-я фпионю фа твуфявыми, говою, фео увиву. Ни-нифео больфе».'
...
label vaxis_s22: # from 7.7 10.3 14.3 16.3 17.1 25.5 27.5
    ...
    vaxis '«Я-я-я фпионю за твуфьявыми, говою, фео увиву. Ни-нифео больфе».'
...
label vaxis_s24: # from 3.3 6.3 8.2
    vaxis '«Иффефь мея? Вафем?»'
    nr 'Он искоса смотрит на тебя.'
    vaxis '«У тея соофение двя мея?»'
...
label vaxis_s28: # from 8.3 8.4 11.1 11.2 11.3 26.0 30.0 43.5
    vaxis '«Я фпионю фа твуфьявыми. Говою, чео вифу. Нифео больфе».'
...
label vaxis_s29: # from 21.1 22.1 23.4 28.0 70.1 71.2
    vaxis '«Нифео. Они нифео ни деают. Нифео не нафол. Твупы, твупы, пвофто твупы. Твуфьявые *нифео* ни деают».'
    nr 'Его глаза деловито сужаются.'
    vaxis '«Буу дальфе фледить».'
...
label vaxis_s31: # from 7.1 12.1 13.4 15.1 25.0 27.0 50.0
    ...
    vaxis '«Пофеу я доувен помоать тее?»'
...
label vaxis_s32: # from 31.0
    ...
    vaxis '«Фем *нао* фео-то, но нифто нифео *не дает*. *Дай* мне фео-нить, и, *мовет*, я помоу».'
...
label vaxis_s33: # from 31.2 31.5 32.1 32.4 34.1 35.2 35.5 75.0
    ...
    vaxis '«Хм-м. Ты мовефь убевать фееф поуталы».'
...
label vaxis_s35: # from 30.3 31.1 32.0
    vaxis '«Тее нуно найти *кьюч* дья мея. Нуен желефный кьюч к байвамовофной комуафе».'
...
label vaxis_s38: # from 36.1 36.2 37.0 37.1
    ...
    vaxis '«Ефи тея поимают, не говои никоу обо мне, или я добеусь до тея, когда ты буешь фпать».'
...
label vaxis_s39: # from 43.12
    vaxis '«Я ховоф ф мафкироуке. У мея ефть фрамы. Я наил на сея мноо байфамируфеи фидкофти. Иф мея поуфифя ХООФЫЙ фомби».'
    nr 'Зомби хихикает через зашитые губы, потом стучит себя по голове.'
    vaxis '«Твуфяки тууупые».'
...
# s40 # say4514
label vaxis_s40: # -
    vaxis '«Я фду тея фдефь. Найди кьюч».'
    nr 'Зомби улыбается мертвецким оскалом.'
    vaxis '«Потом я помоу тее».'
...
label vaxis_s44: # from 43.6
    ...
    vaxis '«Я… фвыфал, фто он фиет де-то ф Уле».'
    nr 'Он качает головой.'
    vaxis '«Нефнаю где».'
    nr 'Он снова хмурится.'
    vaxis '«Твуфявые фодят ф ума, они не ЛЮЯТ Фа-ода».'
...
label vaxis_s46: # from 44.1 45.0
    vaxis '«Он фбофик. Пвинофит твупы ф Моуг, пводает их твуфвым. Пвинофит МУОГО твупоф. Твуфявые ненают, откуда он их беет. Дуают, он пифет пьей в кьигу мертфых».'
...
label vaxis_s47: # from 43.7
    vaxis '«Ненают. Какой-то пей обфифтил тея?»'
...
label vaxis_s49: # from 30.1 42.0 43.0 43.1
    nr 'Зомби ворчит.'
    vaxis '«Ты мовешь убевафь феев поуталы».'
...
label vaxis_s51: # from 50.1 50.2 50.3 72.0
    nr 'Зомби кивает.'
    vaxis '«Тее надо выити, пойти ф арку на певом этаже, февео-фападный фал… Тее нувна кофть паица, фогнутоо в кьюк…»'
    nr 'Он поднимает указательный палец и сгибает его в крюк.'
    vaxis '«Ефли у тея кьюч, иди в авку и пуыгай ф тайную гуобицу. И ты выбралфя отфюда. Тайный выфод».'
    nr 'Он энергично кивает.'
    vaxis '«Там ты мовефь ОДОФНУТЬ».'
...
label vaxis_s52: # from 43.2
    vaxis '«Фуфай! Фапомиай!»'
    nr 'В голосе зомби слышна раздраженность.'
    vaxis '«Арка, пеувый этаж, феверо-фападная коуната…»'
    nr 'Он поднимает указательный палец и сгибает его.'
    vaxis '«Тее нувна кофть паица, фогнутая. Ты попаефь ф тайную гуобицу. Тайный выфод. Там ты мовефь ОДОФНУТЬ».'
...
label vaxis_s55: # from 43.10
    vaxis '«Пвоводник. На пеувом этаже. Фто ты хофефь фнать о нем?»'
...
label vaxis_s56: # from 55.0
    vaxis '«Фтвааный. Не твуфявый, не анавфифт. Гуава двугие…»'
    nr 'Пожимает плечами.'
    vaxis '«Как у квыфы. Фтванно».'
...
label vaxis_s61: # from 60.0
    ...
    vaxis '«Тупие твуфявые. Ив мея ХОВОФЫЙ фомби».'
...
label vaxis_s62: # from 60.1 61.0
    ...
    vaxis '«А как ты думаефь? По мие, нет, не офень».'
...
label vaxis_s63: # from 43.3 43.4 60.2 61.1 62.0 64.1 64.2
    ...
    vaxis '«Уху. Мие нуна банка баифама».'
    nr 'Показывает на шрамы на твоей груди.'
    vaxis '«И игоука ф ниткой».'
...
label vaxis_s64: # from 61.2 61.3 62.1 62.2
    ...
    vaxis '«Ты будефь ХОВОФЫМ фомби. Мовно фделать из тея фомби? ХОВОФАЯ мафкиофка».'
...
label vaxis_s66: # from 65.0 65.2 65.3
    nr 'Зомби обильно натирает твое тело бальзамирующей жидкостью, затем широкими стежками зашивает несколько шрамов. Начав с ног, он медленно поднимается наверх, зашивая в конце концов твои губы.'
...
label vaxis_s67: # from 66.0 66.2
    ...
    vaxis '«Офтововно! Вафгоов вафофет фвы, науфив мафкивку. Фомби не говоят. Поняу? Говои медвенно, офтовоно».'
...
label vaxis_s68: # from 67.0
    nr 'Зомби хмурится.'
    vaxis '«Мафкирофка не деужится доуго… бальфам выфыхает, фвы рвутфя».'
    nr 'Он жмет плечами.'
    vaxis '«Фнаружи Морга от нее нифего не офтанетфя. Не бегай! Ефли ты побежифь, то науфифь вфю мафкирофку».'
...
label vaxis_s69: # -
    vaxis '«Зомби»'
    nr 'Хмурится.'
    vaxis '«Фде-то я тея виел. Мы не виелифь раньфе?»'
...
label vaxis_s72: # from 30.2
    ...
    vaxis '«Вфе тихо, твувявыя не февелятся, нифего новоо с пофледнего отфета».'
...
label vaxis_s73: # -
    nr 'Зомби ворчит:'
    vaxis '«Поутал в арке»'
    nr 'Пеувый этав, феверо-фападная коумата, нувен кофтяной паиец дья откуывания,'
    vaxis '«Он кивает. — Удафи».'
...
label vaxis_s77: # from 51.0
    ...
    vaxis '«Доувен быть де-то фдефь… Ифи на фкладах навеуху. Мовет быть, там».'
