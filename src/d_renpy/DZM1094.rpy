init 10 python:
    from dlgs.zm1094_logic import Zm1094Logic
...
label zm1094_s0:  # - # IF ~  Global("Asonje","GLOBAL",0)
    SPEAKER 'У этого ходячего трупа на лбу вырезан номер 1094. Его губы крепко сшиты, от него исходит сильный химический запах свежего формальдегида, окружающего его в виде облака. Несмотря на мертвенно-бледное лицо и впалые безжизненные молочно-белые глаза, совершенно очевидно, что раньше это был красивый молодой человек.'
...
label zm1094_s2:  # from 0.3
    SPEAKER 'Тело на миг вздрагивает, затем замирает: душа еще раз вселяется в когда-то покинутую оболочку. В считанные секунды в глазах зомби появляется некое подобие жизни, и он начинает оглядываться по сторонам с любопытно-озадаченным выражением лица. Все тело теперь окружено мягким золотым свечением.'
...
label zm1094_s4:  # from 3.0
    SPEAKER 'Дружелюбность духа моментально испаряется. Он хмурится, разорванные швы лохмотьями свешиваются с его серых иссохших губ. Хорошо, спрашивай, что хотел. Он отстраняет взгляд: ему явно скучно.'
...
label zm1094_s6:  # from 4.1 11.1
    SPEAKER 'Не могу вспомнить. Что-нибудь еще?'
...
label zm1094_s9:  # from 4.4 11.4
    SPEAKER 'Дух окидывает тебя раздраженным взглядом. Нет, конечно же! Он раздосадовано мотает головой, от этого движения разорванные швы качаются туда-сюда.'
...
label zm1094_s11:  # from 5.0 6.0 7.0 8.0 9.1 10.0 12.0 27.0
    SPEAKER 'Дух шумно вздыхает, наполняя воздух запахом формальдегида из легких трупа. Да-да… спрашивай.'
...
label zm1094_s14:  # from 3.2 13.0
    SPEAKER 'Дух пожимает плечами и улыбается, слегка посмеиваясь. Возможно, ты прав! Что ты хочешь узнать от меня? Он начинает рассеянно вытягивать остатки швов из своих губ и бросать их на пол, один за одним.'
...
label zm1094_s17:  # from 14.3 22.3
    SPEAKER 'Улыбка духа угасает, и он на мгновенье выглядит озадаченным. Странно… Я не знаю! Я точно не знаю, как умер, правда, — он пожимает плечами. — Неважно! Его радостная улыбка возвращается, сверкая даже несмотря на то, что она находится на высохшем лице трупа.'
...
label zm1094_s21:  # from 15.0
    SPEAKER 'Он выглядит удивленным. Как странно! Мне очень жаль. Я ведь должен тебя *как-то* называть, верно? Дух выжидающе смотрит на тебя.'
...
label zm1094_s22:  # from 15.1 16.0 17.0 18.0 19.1 20.0 21.0 23.0 24.0 25.0 26.0
    SPEAKER 'Конечно. Спрашивай! Он довольно улыбается, с интересом ожидая твоего вопроса. Последний шов был убран, и теперь его улыбка не такая зловещая.'
...
label zm1094_s24:  # from 19.0
    SPEAKER 'О Сигиле, что ли? — увидев твой кивок, труп растягивает улыбку до ушей. — О, я не собираюсь портить тебе впечатление! Иди и разведай его сам! Затеряйся в его улочках, тавернах, среди людей… но будь внимателен! Это место не только удивительно, но и опасно. Но ведь это и делает все таким захватывающим, не так ли?'
...
label zm1094_s26:  # - # IF ~  GlobalGT("Asonje","GLOBAL",0) GlobalLT("Asonje","GLOBAL",3)
    SPEAKER 'Снова вернулся, а? Он широко улыбается.'
