init 10 python:
    ...
    zm396Logic = Zm396Logic(renpy.store.global_state_manager)
...
label zm396_s0: # - # IF ~  HasItem("Bandage","ZM396")
    nr 'Этот труп ходит от плиты к плите, перевязывая лежащих на них мертвецов. На левом виске у него выбит номер'
    zm396 '«396»'
    nr '; его губы крепко зашиты. Ты замечаешь, что труп несет в руках несколько бинтов… похоже, они могут пригодиться.'
...
label zm396_s4: # from 3.0 # IF ~  !HasItem("Bandage","ZM396")
    nr 'Этот труп ходит от плиты к плите, перевязывая лежащих на них мертвецов. Он продолжает выполнять свои обязанности, даже без бинтов. На левом виске у него выбит номер'
    zm396 '«396»'
    nr ', а его губы крепко зашиты.'
