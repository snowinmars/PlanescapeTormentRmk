...
def dlg_deivene(manager):
    ...
    DialogStateBuilder().state('DEIVENE.D_s99999999_k1', '# from -') \
        ...
        .with_responses() \
            ...
            .response("Убить хрупкую девушку.", 'DEIVENE.D_s99999999_k1_', '-', '-') \
        .push(manager)

    DialogStateBuilder().state('DEIVENE.D_s99999999_k1_', '# from -') \
        .with_npc_lines() \
            .line(teller, "Она не услышала, как я подошёл. Её пальцы похожи на когти. Она с легкостью погружает их, словно ножи, в грудь трупа, доставая органы.", '-', '-') \
            .line(teller, "Я быстрее. Я ломаю её правую руку, а когда она с криком оборачивается, я ломаю левую. Девушка всё равно пытается сопротивляться, используя сломанные руки на манер плетей.", 's1', 'say3410') \
            .line(teller, "Но через несколько ударов она сдаётся и падает на пол. Удар. Удар.", 's1', 'say3410').with_action(lambda: _kill_eivene(gsm)) \
            .line(teller, "Она затихла. Я провёл пальцем по порезу, который она мне оставила и пнул дважды мёртвое тело.", 's1', 'say3410') \
            .line(teller, "От удара тело перевалилось на спину. Что-то заставило меня взглянуть в её глаза. Они светились...", 's1', 'say3410') \
            .line(teller, "...нежностью?", 's1', 'say3410') \
        .with_responses() \
    ...
    DialogStateBuilder().state('DEIVENE.D_s99999999_k2', '# from -') \
        ...
        .with_responses() \
            .response("Уйти.", EXIT, '-', '-').with_action(lambda: _dispose()) \
            .response("Убить Эи-Вейн.", 'DEIVENE.D_s99999999_k2_', '-', '-') \
        .push(manager)

    DialogStateBuilder().state('DEIVENE.D_s99999999_k2_', '# from -') \
        .with_npc_lines() \
            .line(teller, "Я знаю, что она не услышит, как я подойду. Я знаю, что её пальцы похожи на когти.", '-', '-') \
            .line(teller, "Я знаю, что я быстрее. Я прикасаюсь к её плечу - и Эи-Вейн оборачивается в ту сторону, где меня уже нет.", 's1', 'say3410') \
            .line(teller, "Я проскальзываю за её спиной и ломаю её правую руку, а когда она с криком оборачивается, я ломаю левую.", 's1', 'say3410') \
            .line(teller, "Девушка всё равно пытается сопротивляться, используя сломанные руки на манер плетей.", 's1', 'say3410') \
            .line(teller, "Но через несколько ударов она сдаётся и падает на пол. Удар. Удар.", 's1', 'say3410').with_action(lambda: _kill_eivene(gsm)) \
            .line(teller, "Она затихла. Я провёл пальцем по порезу, который она мне оставила и пнул дважды мёртвое тело.", 's1', 'say3410') \
            .line(teller, "От удара тело перевалилось на спину. Что-то заставило меня взглянуть в её глаза. Они светились...", 's1', 'say3410') \
            .line(teller, "...нежностью?", 's1', 'say3410') \
