...
def dlg_dmorte_two(manager):
    ...
    DialogStateBuilder().state('DMORTE2.D_s20', '# from 19.0 // # s21 got removed because these lines is about gameplay mechanics, that the visual novel does not have') \
        .with_npc_lines() \
    ...
    DialogStateBuilder().state('DMORTE2.D_s27', '# from -') \
        ...
        .with_responses() \
            .response("Да… идем.", EXIT, 'r78', 'reply41251').with_action(lambda: _r41251_action(gsm)) \
    ...
    DialogStateBuilder().state('DMORTE2.D_s28', '# from 27.1') \
        ...
        .with_responses() \
            ...
            .response("Ладно. Я передумал. Давай, идем.", EXIT, 'r81', 'reply41255').with_action(lambda: _r41251_action(gsm)) \
    ...
    DialogStateBuilder().state('DMORTE2.D_s29', '# from 28.0 28.2') \
        ...
        .with_responses() \
            .response("Ладно. Я передумал. Давай, идем.", EXIT, 'r83', 'reply41258').with_action(lambda: _r41251_action(gsm)) \
