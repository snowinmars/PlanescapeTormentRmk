...
def dlg_dzf1072(manager):
    ...
    DialogStateBuilder().state('DZF1072.D_s99999999_k', '# from -') \
        ...
        .with_responses() \
            ...
            .response("Убить зомби.", 'DZF1072.D_s99999999_k_', '-', '-') \
        .push(manager)

    DialogStateBuilder().state('DZF1072.D_s99999999_k_', '# from -') \
        .with_npc_lines() \
            .line(teller, "Её тело лопается под моими ударами. Отвратительно. Отвратительно пустые глаза.", '-', '-') \
            .line(teller, "В них нет ни жизни, ни разума. Я с омерзением отбрасываю тело.", '-', '-').with_action(lambda: _kill_dzf1072(gsm)) \
            .line(teller, "Шлёп.", '-', '-') \
        .with_responses() \
