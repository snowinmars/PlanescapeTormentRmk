...
def dlg_dzm569(manager):
    ...
    DialogStateBuilder().state('DZM569.D_s99999999_k', '# from -') \
        ...
        .with_responses() \
            ...
            .response("Убить зомби.", 'DZM569.D_s99999999_k_', '-', '-') \
        .push(manager)

    DialogStateBuilder().state('DZM569.D_s99999999_k_', '# from -') \
        .with_npc_lines() \
            .line(teller, "Я бью прямо в номер «569», выбитый на кости черепа. Он удивительно легко проламывается. Я бью снова и снова, пока труп не перестаёт на меня смотреть пустыми глазами.", '-', '-') \
            .line(teller, "В них нет ни жизни, ни разума. Я порезался осколками его черепа.", '-', '-').with_action(lambda: _kill_dzm569(gsm)) \
        .with_responses() \
