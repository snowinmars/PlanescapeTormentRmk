...
label ddeathon_s1:  # from 0.0
    teller 'Глаза тленного скользят по тебе.'
    death_names 'Имя.'
    teller 'Когда он произносит это слово, голос его звучит, как звон колокольчика.'
...
label ddeathon_s2:  # from 1.0 1.1 9.0
    ...
    teller 'Нараспев произносит тленный.'
...
label ddeathon_s3:  # from 2.0
    ...
    teller 'Его глаза скользят по монолиту и по стенам.'
...
label ddeathon_s5:  # from 3.6 4.1 7.1 9.1 10.2 11.1 12.1 13.2 14.1 16.1
    teller 'Он кивает, затем вынимает маленькую руку из перевязи на боку. Она атрофирована… по размерам не больше детской.'
...
label ddeathon_s6:  # from 5.2 5.6
    teller 'Медяки падают в руку Смерти-Имен, и он прячет ее назад.'
    teller 'Его глаза, закатившись, неожиданно оживают и взметаются в поисках места на монолите и стенах мемориала.'
...
label ddeathon_s7:  # from 6.0
    ...
    teller 'Последнее слово заставляет тебя почувствовать себя неуютно.'
...
label ddeathon_s9:  # from 4.0 7.0 8.0 10.1 11.0 12.0 13.1 14.0 16.0
    teller 'И снова глаза тленного глядят на тебя.'
    death_names 'Имя.'
    teller 'Когда он произносит это слово, голос его звучит, как звон колокольчика.'
...
label ddeathon_s10:  # from 3.0 3.1 3.3 9.2 9.3 9.5 9.7 9.9
    teller 'Он закатывает глаза. Дико поблескивая, его взгляд пробегается по стенам монумента, с нечеловеческой скоростью просматривая имена. Затем он указывает на одну из стен.'
    death_names 'Похоронено.'
...
label ddeathon_s11:  # from 10.0
    teller 'Это то самое имя, которое ты запросил. Оно выбито на черном камне тонкой вязью, и практически теряется в море имен, окружающих его.'
...
label ddeathon_s14:  # from 13.0
    teller 'Имя Квентина, высеченное на черном камне слегка неровным почерком. К несчастью для Квентина, его имя не разломило монумент пополам, так и не оправдав его надежд.'
...
label ddeathon_s15:  # from 5.4
    teller 'Медяки падают в руку Смерти-Имен, и он прячет ее назад. Его глаза, закатившись, неожиданно оживают и взметаются в поисках места на монолите и стенах мемориала.'
...
label ddeathon_s16:  # from 15.0
    teller 'Он поворачивается к основанию монолита, быстро подбегает к нему, наклоняется и начинает что-то на нем высекать. К несчастью для Квентина, монумент не разломился после его имени..'
    teller 'Мгновение спустя Смерть-имен завершает работу, поднимается и возвращается к тебе.'
    death_names 'Похоронено.'
    teller 'Последнее слово заставляет тебя почувствовать себя неуютно.'
