...
label ddeathon_s0:  # from - # IF ~  Global("Death_of_Names","GLOBAL",0)~ THEN BEGIN 0 // from:
    call ddeathon_init
    SPEAKER 'Перед тобой тленный с кривой улыбкой, застывшей на его лице. Несмотря на улыбку, его глаза безжизненны.'
    SPEAKER 'Правая рука короче левой, и он покачивает ее на перевязи, будто убаюкивая малое дитя.'
...
label ddeathon_s1:  # from 0.0
    SPEAKER 'Глаза тленного скользят по тебе.'
    death_names 'Имя.'
    SPEAKER 'Когда он произносит это слово, голос его звучит, как звон колокольчика.'
...
label ddeathon_s2:  # from 1.0 1.1 9.0
    ...
    SPEAKER 'Нараспев произносит тленный.'
...
label ddeathon_s3:  # from 2.0
    ...
    SPEAKER 'Его глаза скользят по монолиту и по стенам.'
...
label ddeathon_s5:  # from 3.6 4.1 7.1 9.1 10.2 11.1 12.1 13.2 14.1 16.1
    SPEAKER 'Он кивает, затем вынимает маленькую руку из перевязи на боку. Она атрофирована… по размерам не больше детской.'
...
label ddeathon_s6:  # from 5.2 5.6
    SPEAKER 'Медяки падают в руку Смерти-Имен, и он прячет ее назад.'
    SPEAKER 'Его глаза, закатившись, неожиданно оживают и взметаются в поисках места на монолите и стенах мемориала.'
...
label ddeathon_s7:  # from 6.0 # GlobalGT("Death_of_Names","GLOBAL",0)
    SPEAKER 'Он находит на стене место, быстро подходит к нему и наклоняется, затем принимается высекать что-то на стене.'
    SPEAKER 'Спустя несколько минут он завершает работу, поднимается и возвращается к тебе.'
    death_names 'Похоронено.'
    SPEAKER 'Последнее слово заставляет тебя почувствовать себя неуютно.'
...
label ddeathon_s8:  # from -
    call ddeathon_init
    SPEAKER 'Перед тобой Смерть-имен. Он стоит перед монументом с искривленной улыбкой, а его рука висит на перевязи.'
...
label ddeathon_s9:  # from 4.0 7.0 8.0 10.1 11.0 12.0 13.1 14.0 16.0
    SPEAKER 'И снова глаза тленного глядят на тебя.'
    death_names 'Имя.'
    SPEAKER 'Когда он произносит это слово, голос его звучит, как звон колокольчика.'
...
label ddeathon_s10:  # from 3.0 3.1 3.3 9.2 9.3 9.5 9.7 9.9
    SPEAKER 'Он закатывает глаза. Дико поблескивая, его взгляд пробегается по стенам монумента, с нечеловеческой скоростью просматривая имена. Затем он указывает на одну из стен.'
    death_names 'Похоронено.'
...
label ddeathon_s11:  # from 10.0
    SPEAKER 'Это то самое имя, которое ты запросил. Оно выбито на черном камне тонкой вязью, и практически теряется в море имен, окружающих его.'
...
label ddeathon_s14:  # from 13.0
    SPEAKER 'Имя Квентина, высеченное на черном камне слегка неровным почерком. К несчастью для Квентина, его имя не разломило монумент пополам, так и не оправдав его надежд.'
...
label ddeathon_s15:  # from 5.4
    SPEAKER 'Медяки падают в руку Смерти-Имен, и он прячет ее назад. Его глаза, закатившись, неожиданно оживают и взметаются в поисках места на монолите и стенах мемориала.'
...
label ddeathon_s16:  # from 15.0
    SPEAKER 'Он поворачивается к основанию монолита, быстро подбегает к нему, наклоняется и начинает что-то на нем высекать. К несчастью для Квентина, монумент не разломился после его имени..'
    SPEAKER 'Мгновение спустя Смерть-имен завершает работу, поднимается и возвращается к тебе.'
    death_names 'Похоронено.'
    SPEAKER 'Последнее слово заставляет тебя почувствовать себя неуютно.'
