...
label deions_s2:  # from 1.0 1.1
    ...
    deionarra '«Ты!»'
    deionarra '«Что привело сюда *тебя*?!»'
    deionarra '«Захотел лично полюбоваться на причиненные тобой страдания?»'
    deionarra '«Или, быть может, даже после моей смерти ты надеешься получить от меня пользу?.».'
