...
class Dzf1148LogicTest(LogicTest):
    def test_initialization(self):
        logic = Dzf1148Logic(self.settings_manager)
        self.assertIsNotNone(logic.gsm)


    def test_methods_are_bound(self):
        self.target_class = Dzf1148Logic
        self._methods_are_bound()


    def test_dzf1148_init(self):
        logic = Dzf1148Logic(self.settings_manager)
        id = 'mortuary_f2r2'

        self.assertNotEqual(self.settings_manager.glm.get_location(), id)
        self.assertFalse(self.settings_manager.get_meet_dzf1148())

        logic.dzf1148_init()

        self.assertEqual(self.settings_manager.glm.get_location(), id)
        self.assertTrue(self.settings_manager.get_meet_dzf1148(), True)


    def test_kill_dzf1148(self):
        logic = Dzf1148Logic(self.settings_manager)
        who = 'protagonist'
        prop = 'experience'
        delta = 65

        self.assertFalse(self.settings_manager.get_dead_dzf1148())
        expBefore = self.settings_manager.gcm.get_character_property(who, prop)

        logic.kill_dzf1148()

        self.assertTrue(self.settings_manager.get_dead_dzf1148())
