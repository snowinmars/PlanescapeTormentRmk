...
class Dzf891LogicTest(LogicTest):
    def test_initialization(self):
        logic = Dzf891Logic(self.settings_manager)
        self.assertIsNotNone(logic.gsm)


    def test_methods_are_bound(self):
        self.target_class = Dzf891Logic
        self._methods_are_bound()


    def test_dzf891_init(self):
        logic = Dzf891Logic(self.settings_manager)
        id = 'mortuary_f2r2'

        self.assertNotEqual(self.settings_manager.glm.get_location(), id)
        self.assertFalse(self.settings_manager.get_meet_dzf891())

        logic.dzf891_init()

        self.assertEqual(self.settings_manager.glm.get_location(), id)
        self.assertTrue(self.settings_manager.get_meet_dzf891(), True)


    def test_kill_dzf891(self):
        logic = Dzf891Logic(self.settings_manager)
        who = 'protagonist'
        prop = 'experience'
        delta = 65

        self.assertFalse(self.settings_manager.get_dead_dzf891())
        expBefore = self.settings_manager.gcm.get_character_property(who, prop)

        logic.kill_dzf891()

        self.assertTrue(self.settings_manager.get_dead_dzf891())
