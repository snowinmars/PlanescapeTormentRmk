...
class Dzm396LogicTest(LogicTest):
    def test_initialization(self):
        logic = Dzm396Logic(self.settings_manager)
        self.assertIsNotNone(logic.gsm)


    def test_methods_are_bound(self):
        self.target_class = Dzm396Logic
        self._methods_are_bound()


    def test_dzm396_init(self):
        logic = Dzm396Logic(self.settings_manager)
        id = 'mortuary_f2r3'

        self.assertNotEqual(self.settings_manager.glm.get_location(), id)
        self.assertFalse(self.settings_manager.get_meet_dzm396())

        logic.dzm396_init()

        self.assertEqual(self.settings_manager.glm.get_location(), id)
        self.assertTrue(self.settings_manager.get_meet_dzm396(), True)


    def test_kill_dzm396(self):
        logic = Dzm396Logic(self.settings_manager)
        who = 'protagonist'
        prop = 'experience'
        delta = 65

        self.assertFalse(self.settings_manager.get_dead_dzm396())
        expBefore = self.settings_manager.gcm.get_character_property(who, prop)

        logic.kill_dzm396()

        self.assertTrue(self.settings_manager.get_dead_dzm396())
    ...
    def test_get_took_dzm396_bandages(self):
        logic = Dzm396Logic(self.settings_manager)

        self._boolean_straight_condition(
            lambda x: self.settings_manager.set_has_bandages_zm396(x),
            lambda: logic.get_took_dzm396_bandages()
        )
