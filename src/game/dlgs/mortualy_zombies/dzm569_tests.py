...
class Dzm569LogicTest(LogicTest):
    def test_initialization(self):
        logic = Dzm569Logic(self.settings_manager)
        self.assertIsNotNone(logic.gsm)


    def test_methods_are_bound(self):
        self.target_class = Dzm569Logic
        self._methods_are_bound()


    def test_dzm569_init(self):
        logic = Dzm569Logic(self.settings_manager)
        id = 'mortuary_f2r1'

        self.assertNotEqual(self.settings_manager.glm.get_location(), id)
        self.assertFalse(self.settings_manager.get_meet_dzm569())

        logic.dzm569_init()

        self.assertEqual(self.settings_manager.glm.get_location(), id)
        self.assertTrue(self.settings_manager.get_meet_dzm569(), True)


    def test_kill_dzm569(self):
        logic = Dzm569Logic(self.settings_manager)
        who = 'protagonist'
        prop = 'experience'
        delta = 65

        self.assertFalse(self.settings_manager.get_dead_dzm569())
        expBefore = self.settings_manager.gcm.get_character_property(who, prop)

        logic.kill_dzm569()

        self.assertTrue(self.settings_manager.get_dead_dzm569())
