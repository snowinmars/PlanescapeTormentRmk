...
label ddhall_s0:  # from - # IF ~~ THEN BEGIN 0 // from: Manually checked EXTERN ~DMORTE~ : 104 as dmorte_s104
    teller 'Прежде чем Морт успевает завершить свои разглагольствования, писарь начинает безудержно кашлять. Спустя минуту или две кашель прекращается, и дыхание писаря вновь становится неровным хрипом.'
...
label ddhall_s44:  # from 1.0 6.2 7.0
    ...
    dhall_unknown 'Я *никогда* не знал тебя, Неугомонный. Не больше, чем ты знал о себе сам. Он умолкает на секунду. Ты ведь все забыл, не так ли?'
