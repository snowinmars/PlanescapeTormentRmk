...
# Starts:    dmorte_s102 ddhall_s5 ddhall_s40
# ###
...
label ddhall_s1:  # from -
    SPEAKER 'Прежде чем Морт успевает закончить, взгляд серых глаз писаря падает на тебя.'
    dhall_unknown 'Бремя прожитых лет лежит на мне тяжелым грузом, Неугомонный.'
    SPEAKER 'Он откладывает перо.'
...
label ddhall_s2:  # from 21.0
    ...
    SPEAKER 'Дхолл начинает кашлять, но ему удается перевести дыхание.'
    dhall 'Я ошибаюсь?'
...
label ddhall_s3:  # from 9.0
    SPEAKER 'Дхолл пристально смотрит на тебя.'
    dhall 'Ты уверен?'
...
# s4 # say833
label ddhall_s4:  # from 3.0 3.1 # Global("Dhall","GLOBAL",0)
    dhall 'Я…'
    SPEAKER 'У Дхолла начинается очередной приступ кашля. Спустя минуту или две его дыхание становится достаточно спокойным, чтобы он смог продолжить.'
...
label ddhall_s5:  # from - # Check EXTENDS ~DMORTE~ : 102
    ...
    SPEAKER 'Его дыхание неровно и прерывисто, но даже периодический кашель не может замедлить движение его пера.'
...
label ddhall_s6:  # from 5.2
    SPEAKER 'Его серые глаза сверкают, когда он отрывает свой взгляд от книги.'
    dhall 'Подозреваю, что это ты в ответе за нападения в Морге. Это…'
    SPEAKER 'Он тихо кашляет, затем тяжело хрипит.'
...
label ddhall_s7:  # from 5.1
    SPEAKER 'Писарь прекращает вести записи в стоящую перед ним книгу и оглядывается. Его глаза похожи на два гвоздя, забитые в его череп.'
    dhall 'Итак…'
    SPEAKER 'Его голос уставший, как будто он повторял это уже много раз.'
    dhall 'Ты пробудился ото сна и вернулся в свои грезы.'
    SPEAKER 'Он продолжает более уважительным голосом.'
...
label ddhall_s10:  # from 9.1
    ...
    SPEAKER 'Не успев закончить, Дхолл заходится в приступе кашля. Спустя некоторое время он берет себя в руки, и его дыхание снова становится неровным хрипом.'
...
label ddhall_s11:  # from 2.6 2.7 4.1 8.3 9.7 10.6 12.2 14.5 15.3 16.4 19.3 20.3 21.2 22.3 23.3 24.2 25.3 26.3 27.1 28.2 29.4 30.1 31.3 32.7 33.4 34.3 35.3 36.3 37.2 38.3 41.4 42.5 43.4 47.5 48.3 49.3 51.3 52.3 53.2
    SPEAKER 'Когда ты собираешься уйти, Дхолл начинает говорить.'
    ...
    SPEAKER 'Дхолл кашляет, из его горла доносятся хрипы.'
...
label ddhall_s12:  # from 2.3 2.4 42.2 42.3 43.1 43.2
    ...
    SPEAKER 'Дхолл обводит вокруг тебя рукой.'
...
label ddhall_s13:  # from 9.3
    ...
    SPEAKER 'Дхолл заходится в кашле, затем продолжает.'
...
label ddhall_s15:  # from 9.2 10.4 32.5
    SPEAKER 'Дхолл презрительно фыркает, как будто его воротит от воспоминания об этом.'
...
label ddhall_s16:  # from 15.0
    ...
    SPEAKER 'Дхолл снова заходится в приступе кашля, который ему удается побороть только несколько минут спустя.'
...
label ddhall_s17:  # from 16.0
    ...
    SPEAKER 'Дхолл переводит дыхание, затем продолжает.'
...
label ddhall_s19:  # from 17.4 18.0 18.3 29.3 31.2
    SPEAKER 'Голос Дхолла начинает звучать предостерегающе.'
...
label ddhall_s20:  # from 19.0
    SPEAKER 'Дхолл умолкает на минуту. Когда же он начинает говорить, то становится ясно, что он делает это с явной неохотой.'
...
label ddhall_s23:  # from 8.0
    ...
    SPEAKER 'Дхолл, кажется, готов разразиться очередным приступом кашля, но с некоторыми усилиями ему удается сдержаться.'
...
label ddhall_s24:  # from 25.0
    ...
    SPEAKER 'Дхолл начинает кашлять, затем успокаивается.'
...
label ddhall_s25:  # from 9.5 23.0
    ...
    SPEAKER 'Дхолл снова кашляет, затем глубоко вздыхает.'
...
label ddhall_s28:  # from 2.2 42.1
    ...
    SPEAKER 'Дхолл переходит на шепот.'
...
label ddhall_s29:  # from 36.0
    SPEAKER 'Дхолл умолкает в раздумье.'
    dhall 'Скорее всего. Ты что-то потерял… что-то особенно ценное?'
    SPEAKER 'Он хмурится, понижая тон.'
...
label ddhall_s30:  # from 28.0
    SPEAKER 'Дхолл описывает пальцем полукруг в воздухе перед собой.'
...
label ddhall_s32:  # from 8.1 10.0 14.3 15.1 16.2 17.2 18.1 19.1 20.1 22.1 23.1 24.0 25.1 33.2 34.1 37.0 38.1 41.2 47.3 48.1 49.1 51.1 52.1 53.0
    ...
    SPEAKER 'Дхолл от волнения понижает тон.'
...
label ddhall_s34:  # from 14.1 32.4
    ...
    SPEAKER 'Дхолл надрывно кашляет, затем успокаивается.'
...
label ddhall_s35:  # from 16.1 17.1 20.0
    ...
    SPEAKER 'Дхолл успокаивает свое неровное дыхание.'
...
label ddhall_s38:  # from 10.1 14.0
    ...
    SPEAKER 'Дхолл переводит дыхание.'
...
# s39 # say884
label ddhall_s39:  # from - # Global("Dhall","GLOBAL",1)
...
label ddhall_s40:  # from -
    call ddhall_init
    SPEAKER 'Дхолл мельком смотрит на тебя.'
    dhall 'Итак. Ты вернулся…'
    SPEAKER 'Дхолл начинает хрипло дышать, затем у него начинается удушливый кашель. Спустя минуту кашель прекращается, и он, хрипло дыша, продолжает.'
...
label ddhall_s43:  # from 2.1 42.0
    SPEAKER 'Дхолл не отвечает. Он просто молчаливо смотрит на тебя.'
...
label ddhall_s44:  # from 1.0 6.2 7.0
    dhall 'Знаю ли я тебя? Я…'
    SPEAKER 'В голосе писца звучит горечь.'
    dhall 'Я *никогда* не знал тебя, Неугомонный. Не больше, чем ты знал о себе сам. Он умолкает на секунду. Ты ведь все забыл, не так ли?'
...
label ddhall_s45:  # from 44.0
    dhall 'Как всегда, вопрос. И как всегда, неправильный.'
    SPEAKER 'Он делает легкий поклон, но движение вызывает у него неожиданный приступ кашля.'
    dhall 'Я…'
    SPEAKER 'Он умолкает на минуту, переводя дыхание.'
    dhall 'Я… Дхолл.'
...
label ddhall_s46:  # from 45.1
    ...
    SPEAKER 'Дхолл кивает.'
    dhall 'Но, боюсь, в данном вопросе время тебе не враг.'
    SPEAKER 'Он снова берется за перо.'
...
label ddhall_s48:  # from 32.1 33.0 38.0 41.1 47.1
    ...
    SPEAKER 'Дхолл кашляет, затем восстанавливает неровное дыхание.'
...
label ddhall_s52:  # from 51.0
    ...
    SPEAKER 'Дхолл делает вздох… как будто смерть клокочет в его глотке.'
