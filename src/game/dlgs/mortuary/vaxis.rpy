...
label vaxis_s0: # - # IF ~  Global("Vaxis","GLOBAL",0)
    "vaxis_s0{#vaxis_s0}"
    # nr 'Неуклюжий труп смотрит на тебя пустым взглядом. На его лбу вырезан номер{#vaxis_s0_1}'
    # vaxis '«821»{#vaxis_s0_2}'
    # nr ', а его губы крепко зашиты. От тела исходит легкий запах формальдегида.{#vaxis_s0_3}'
...
label vaxis_s3: # from 2.2 5.2
    "vaxis_s3{#vaxis_s3}"
    # nr 'Ты уже почти отвернулся, как{#vaxis_s3_1}'
    # vaxis '«Зомби»{#vaxis_s3_2}'
    # nr 'Начинает что-то бормотать… кажется, он пытается что-то сказать, но с зашитым ртом это сделать трудно.{#vaxis_s3_3}'
    # vaxis '«Фто ТЫ? Фто тее нао?»{#vaxis_s3_4}'
...
label vaxis_s73: # -
    ...
    # vaxis '«Поутал в арке»{#vaxis_s73_2}'
    # nr 'Пеувый этав, феверо-фападная коумата, нувен кофтяной паиец дья откуывания,{#vaxis_s73_3}'
    # vaxis '«Он кивает. — Удафи».{#vaxis_s73_4}'
