...
class EventsManagerTest(LogicTest):
    def test_ctor(self):
        ...
        self.assertIsNotNone(self.events_manager._events_store.events)
        self.assertNotEqual(len(self.events_manager._events_store.events), 0)
    ...
    def test_write_event_when_all_ok(self):
        ...
        before = len(self.events_manager._events_store.events)
        self.events_manager.write_event(text)

        after = len(self.events_manager._events_store.events)
        self.assertEqual(before + delta, after)
        self.assertIsNotNone(self.events_manager._events_store.events[-1].timestamp)
        self.assertEqual(self.events_manager._events_store.events[-1].category, category)
        self.assertEqual(self.events_manager._events_store.events[-1].text, text)


    def test_write_event_when_maxed_out(self):
        max_entries = 2
        events_manager = EventsManager(self.logger)
        events_manager.set_store(EventsStore(max_entries))
        text1 = 'text1'
        category1 = 'category1'
        text2 = 'text2'
        category2 = 'category2'
        ...
        before = len(events_manager._events_store.events)

        events_manager.write_event(text1, category1)

        after1 = len(events_manager._events_store.events)
        self.assertEqual(before + delta, after1)
        self.assertIsNotNone(events_manager._events_store.events[0].timestamp)
        self.assertEqual(events_manager._events_store.events[0].category, category1)
        self.assertEqual(events_manager._events_store.events[0].text, text1)

        events_manager.write_event(text2, category2)

        after2 = len(events_manager._events_store.events)
        self.assertEqual(after1 + delta, after2)
        self.assertIsNotNone(events_manager._events_store.events[1].timestamp)
        self.assertEqual(events_manager._events_store.events[1].category, category2)
        self.assertEqual(events_manager._events_store.events[1].text, text2)

        events_manager.write_event(text3, category3)

        after3 = len(events_manager._events_store.events)
        self.assertEqual(after2, after3)
        self.assertIsNotNone(events_manager._events_store.events[1].timestamp)
        self.assertEqual(events_manager._events_store.events[1].category, category3)
        self.assertEqual(events_manager._events_store.events[1].text, text3)

        events_manager.write_event(text4, category4)

        after4 = len(events_manager._events_store.events)
        self.assertEqual(after3, after4)
        self.assertIsNotNone(events_manager._events_store.events[1].timestamp)
        self.assertEqual(events_manager._events_store.events[1].category, category4)
        self.assertEqual(events_manager._events_store.events[1].text, text4)
    ...
    def test_get_events_when_all_ok(self):
        ...
        self.assertEqual(len(events_manager._events_store.events), 0)
    ...
    def test_clear_when_all_ok(self):
        ...
        after_write = len(self.events_manager._events_store.events)
        self.assertEqual(before + delta, after_write)

        self.events_manager.clear()

        after_clear = len(self.events_manager._events_store.events)
