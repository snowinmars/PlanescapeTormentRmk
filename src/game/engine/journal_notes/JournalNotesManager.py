...
class JournalNotesManager:
    ...
    def set_store(self, journal_notes_store):
        self._journal_notes_store = journal_notes_store
    ...
    def register(self, note_id, content, found=False):
        if note_id in self._journal_notes_store.notes:
            raise KeyError(f"Note '{note_id}' already registrated")

        self._journal_notes_store.notes[note_id] = JournalNote(note_id, content, found)

        return self


    def get(self, note_id):
        if note_id not in self._journal_notes_store.notes:
            raise KeyError(f"Note '{note_id}' was not registrated")

        return self._journal_notes_store.notes.get(note_id)
    ...
    def update_journal(self, note_id):
        ...
        self._report_change_callback(
            'journal_notes_manager_update_journal',
            {
                'note_id': note_id
            }
        )
    ...
    def build_journal(self):
        return list(map(
            ...
            filter(
                lambda x: x[1].found,
                sorted(
                    self._journal_notes_store.notes.items(),
