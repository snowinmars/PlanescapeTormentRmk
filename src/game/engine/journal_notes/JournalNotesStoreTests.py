...
class JournalNotesStoreTests(unittest.TestCase):
    def setUp(self):
        self.store = JournalNotesStore()
        self.journal_note_a = JournalNote('note_a', 'Note a', found=True)
        self.journal_note_b = JournalNote('note_b', 'Note b', found=False)


    def test_reserialize_empty_pickle(self):
        dump = pickle.dumps(self.store)
        expected = b'\x80\x05\x95V\x00\x00\x00\x00\x00\x00\x00\x8c+game.engine.journal_notes.JournalNotesStore\x94\x8c\x11JournalNotesStore\x94\x93\x94)\x81\x94}\x94\x8c\x05notes\x94}\x94sb.'
    ...
    def test_reserialize_empty_json(self):
        ...
        store = JournalNotesStore.fromJson(dump)
        self._assert_empty_store(store)
    ...
    def test_reserialize_filled_pickle(self):
        ...
        dump = pickle.dumps(self.store)
        expected = b'\x80\x05\x95\xfa\x00\x00\x00\x00\x00\x00\x00\x8c+game.engine.journal_notes.JournalNotesStore\x94\x8c\x11JournalNotesStore\x94\x93\x94)\x81\x94}\x94\x8c\x05notes\x94}\x94(\x8c\x06note_a\x94\x8c%game.engine.journal_notes.JournalNote\x94\x8c\x0bJournalNote\x94\x93\x94)\x81\x94}\x94(\x8c\x02id\x94h\x07\x8c\x07content\x94\x8c\x06Note a\x94\x8c\x05found\x94\x88\x8c\x08found_at\x94K\x00ub\x8c\x06note_b\x94h\n)\x81\x94}\x94(h\rh\x12h\x0e\x8c\x06Note b\x94h\x10\x89h\x11K\x00ubusb.'
        self.assertEqual(dump, expected)
