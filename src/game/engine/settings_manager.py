...
class SettingsManager:
    ...
    def update_journal(self, note_id):
        # TODO [snow]: join with real notes
        if note_id not in self._open_journal_note_ids:
            self._log(f"Open journal note '{note_id}'")
            self._open_journal_note_ids.append(note_id)


    def has_journal_note(self, note_id):
        return note_id in self._open_journal_note_ids
