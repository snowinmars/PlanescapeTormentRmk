...
class FromMortuaryF3R3ToMortuaryF2R7(GoToLocationMenuItem):
    ...
    def tooltip(self):
        if self.gsm.locations_manager.is_visited('mortuary_f2r7'):
            return 'Спуститься на второй этаж'
        return 'Спуститься по лестнице'
    def jump(self):
        return NavigationDirective(
            'graphics_menu',
            lambda: self.gsm.locations_manager.set_location('mortuary_f2r7')
        )
...
class InMortuaryF3R3S748(SkeletMenuItem):
    def __init__(self, gsm, x, y):
        super().__init__(gsm, x, y)
    def when(self):
        return not self.gsm.get_dead_s748()
    def tooltip(self):
        if self.gsm.get_talked_to_s748_times() > 0:
...
class InMortuaryF3R3S996(SkeletMenuItem):
    def __init__(self, gsm, x, y):
        super().__init__(gsm, x, y)
    def when(self):
        return not self.gsm.get_dead_s996()
    def tooltip(self):
        if self.gsm.get_talked_to_s996_times() > 0:
...
class InMortuaryF3R3Zm475(ZombieMenuItem):
    def __init__(self, gsm, x, y):
        super().__init__(gsm, x, y)
    def when(self):
        return not self.gsm.get_dead_zm475()
    def tooltip(self):
        if self.gsm.get_talked_to_zm475_times() > 0:
...
class InMortuaryF3R3Zm310(ZombieMenuItem):
    def __init__(self, gsm, x, y):
        super().__init__(gsm, x, y)
    def when(self):
        return not self.gsm.get_dead_zm310()
    def tooltip(self):
        if self.gsm.get_talked_to_zm310_times() > 0:
