...
def build_mortuary_f1r7_menu(location_id, settings_manager):
    builders = []
    ...
    builders.append(MenuBuilder(location_id) \
        ...
        .option(lambda: 'Атаковать труп «732»'
                if settings_manager.get_talked_to_zm732_times() > 0
                else 'Атаковать ходячий труп повешенного') \
        .jump("start_zm732_kill") \
        .when(lambda: not settings_manager.get_dead_zm732()) \
        .style('kill') \
