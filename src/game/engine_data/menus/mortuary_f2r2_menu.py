...
def build_mortuary_f2r2_menu(location_id, settings_manager):
    builders = []

    builders.append(morte_menu(settings_manager, location_id, 510, 880))

    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/zombie.png', lambda: not settings_manager.get_dead_zm965(), 450, 520)
        .auto_position(490, 520)
        .option(lambda: 'Атаковать труп «965»'
                if settings_manager.get_talked_to_zm965_times() > 0
                else 'Атаковать бродящий труп') \
        .jump(lambda: 'start_zm965_kill'
              if settings_manager.get_talked_to_zm965_times() > 0
              else 'start_zm965_kill_first') \
        .when(lambda: not settings_manager.get_dead_zm965()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/zombie.png', lambda: not settings_manager.get_dead_zf594(), 490, 720)
        .auto_position(530, 720)
        .option(lambda: 'Атаковать труп «594»'
                if settings_manager.get_talked_to_zf594_times() > 0
                else 'Атаковать неуклюжий труп') \
        .jump("start_zf594_kill") \
        .when(lambda: not settings_manager.get_dead_zf594()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/zombie.png', lambda: not settings_manager.get_dead_zf626(), 840, 600)
        .auto_position(880, 600)
        .option(lambda: 'Атаковать труп «626»'
                if settings_manager.get_talked_to_zf626_times() > 0
                else 'Атаковать разбитый труп') \
        .jump("start_zf626_kill") \
        .when(lambda: not settings_manager.get_dead_zf626()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        ...
        .option(lambda: 'Пройти в северо-западную приёмную'
                if settings_manager.location_manager.is_visited_location('mortuary_f2r3')
                else "Открыть дверь") \
