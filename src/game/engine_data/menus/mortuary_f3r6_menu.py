...
def build_mortuary_f3r6_menu(location_id, settings_manager):
    builders = []

    builders.append(morte_menu(settings_manager, location_id, 1500, 650))
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/skelet.png', lambda: not settings_manager.get_dead_s748(), 1260, 200)
        .auto_position(1300, 200)
        .option(lambda: 'Атаковать скелет «748»'
                if settings_manager.get_talked_to_s748_times() > 0
                else 'Атаковать скелет со вставной челюстью') \
        .jump("start_s748_kill") \
        .when(lambda: not settings_manager.get_dead_s748()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/skelet.png', lambda: not settings_manager.get_dead_s996(), 1160, 250)
        .auto_position(1200, 250)
        .option(lambda: 'Атаковать скелет «996»'
                if settings_manager.get_talked_to_s996_times() > 0
                else 'Атаковать скелет со словом на лбу') \
        .jump("start_s996_kill") \
        .when(lambda: not settings_manager.get_dead_s996()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/zombie.png', lambda: not settings_manager.get_dead_zm475(), 360, 400)
        .auto_position(400, 400)
        .option(lambda: 'Атаковать труп «475»'
                if settings_manager.get_talked_to_zm475_times() > 0
                else 'Атаковать проржавевший труп') \
        .jump("start_zm475_kill") \
        .when(lambda: not settings_manager.get_dead_zm475()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/zombie.png', lambda: not settings_manager.get_dead_zm310(), 760, 1000)
        .auto_position(800, 1000)
        .option(lambda: 'Атаковать труп «310»'
                if settings_manager.get_talked_to_zm310_times() > 0
                else 'Атаковать безжизненный труп') \
        .jump(lambda: 'start_zm310_kill'
                if settings_manager.get_talked_to_zm310_times() > 0
                else 'start_zm310_kill_first') \
        .when(lambda: not settings_manager.get_dead_zm310()) \
        .style('kill') \
