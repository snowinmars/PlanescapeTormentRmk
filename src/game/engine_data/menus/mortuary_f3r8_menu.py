...
def build_mortuary_f3r8_menu(location_id, settings_manager):
    builders = []

    builders.append(morte_menu(settings_manager, location_id, 800, 800))
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/dustfem.png', lambda: not settings_manager.get_dead_dustfem(), 960, 400)
        .auto_position(1000, 400)
        .option(lambda: 'Атаковать тленную'
                if settings_manager.get_talked_to_dustfem_times() > 0
                else 'Атаковать человека') \
        .jump(lambda: 'start_dustfem_kill'
              if settings_manager.get_talked_to_dustfem_times() > 0
              else 'start_dustfem_kill_first') \
        .when(lambda: not settings_manager.get_dead_dustfem()) \
        .style('kill') \
        ...
        .when(lambda: not settings_manager.get_dead_dustfem() and not settings_manager.get_mortualy_alarmed()) \
        .style('talk')
    )

    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/skelet.png', lambda: not settings_manager.get_dead_s42(), 560, 300)
        .auto_position(600, 300)
        .option(lambda: 'Атаковать скелет «42»'
                if settings_manager.get_talked_to_s42_times() > 0
                else 'Атаковать скелет в комбинезоне') \
        .jump("start_s42_kill") \
        .when(lambda: not settings_manager.get_dead_s42()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/zombie.png', lambda: not settings_manager.get_dead_zf832(), 1260, 400)
        .auto_position(1300, 400)
        .option(lambda: 'Атаковать труп «832»'
                if settings_manager.get_talked_to_zf832_times() > 0
                else 'Атаковать красивый труп') \
        .jump("start_zf832_kill") \
        .when(lambda: not settings_manager.get_dead_zf832()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/zombie.png', lambda: not settings_manager.get_dead_zm613(), 360, 400)
        .auto_position(400, 400)
        .option(lambda: 'Атаковать труп «613»'
                if settings_manager.get_talked_to_zm613_times() > 0
                else 'Атаковать изрезанный труп') \
        .jump("start_zm613_kill") \
        .when(lambda: not settings_manager.get_dead_zm613()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/zombie.png', lambda: not settings_manager.get_dead_zm79(), 560, 750)
        .auto_position(600, 750)
        .option(lambda: 'Атаковать труп «79»'
                if settings_manager.get_talked_to_zm79_times() > 0
                else 'Атаковать труп почти без головы') \
        .jump("start_zm79_kill") \
        .when(lambda: not settings_manager.get_dead_zm79()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/zombie.png', lambda: not settings_manager.get_dead_zf679(), 485, 825)
        .auto_position(525, 825)
        .option(lambda: 'Атаковать труп «679»'
                if settings_manager.get_talked_to_zf679_times() > 0
                else 'Атаковать древний труп') \
        .jump("start_zf679_kill") \
        .when(lambda: not settings_manager.get_dead_zf679()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/skelet.png', lambda: not settings_manager.get_dead_s1221(), 410, 900)
        .auto_position(450, 900)
        .option(lambda: 'Атаковать скелет «1221»'
                if settings_manager.get_talked_to_s1221_times() > 0
                else 'Атаковать вонючий скелет') \
        .jump("start_s1221_kill") \
        .when(lambda: not settings_manager.get_dead_s1221()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        ...
        .when(lambda: not settings_manager.get_has_prybar()) \
        .style('open')
    )
    ...
    builders.append(MenuBuilder(location_id) \
        ...
        .when(lambda: not settings_manager.get_has_dustman_request()) \
        .style('open')
    )
