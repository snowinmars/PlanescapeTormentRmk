...
def build_mortuary_f3r2_menu(location_id, gsm, glm):
    builders = []

    builders.append(morte_menu(gsm, location_id, 500, 800))
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/skelet.png', lambda: not gsm.get_dead_s863(), 1060, 300)
        .auto_position(1100, 300)
        .option(lambda: 'Атаковать скелет «863»'
                if gsm.get_talked_to_s863_times() > 0
                else 'Атаковать скелет ветерана') \
        .jump(lambda: 'start_s863_kill'
                if gsm.get_talked_to_s863_times() > 0 and gsm.get_has_dremind()
                else 'start_s863_kill_first') \
        .when(lambda: not gsm.get_dead_s863()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/zombie.png', lambda: not gsm.get_dead_zm1146(), 960, 300)
        .auto_position(1000, 300)
        .option(lambda: 'Атаковать труп «1146»'
                if gsm.get_talked_to_zm1146_times() > 0
                else 'Атаковать плешивый ходячий труп') \
        .jump(lambda: 'start_zm1146_kill'
                if gsm.get_talked_to_zm1146_times() > 0
                else 'start_zm1146_kill_first') \
        .when(lambda: not gsm.get_dead_zm1146()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        .with_main_texture('images/menu_sprites/zombie.png', lambda: not gsm.get_dead_zf1148(), 1260, 400)
        .auto_position(1300, 400)
        .option(lambda: 'Атаковать труп «1148»'
                if gsm.get_talked_to_zf1148_times() > 0
                else 'Атаковать татуированный труп') \
        .jump("start_zf1148_kill") \
        .when(lambda: not gsm.get_dead_zf1148()) \
        .style('kill') \
    ...
    builders.append(MenuBuilder(location_id) \
        ...
        .when(lambda: not gsm.get_has_mortuary_key()) \
        .style('open')
    )
