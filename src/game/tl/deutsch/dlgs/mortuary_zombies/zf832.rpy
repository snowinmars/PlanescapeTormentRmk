# TODO: Translation updated at 2025-11-26 00:35

# game/dlgs/mortuary_zombies/zf832.rpy:14
translate deutsch zf832_s0_df172e7d:

    # nr "Несмотря на жесткую иссохшую кожу, совершенно очевидно, что раньше это была красивая женщина средних лет."
    nr ""

# game/dlgs/mortuary_zombies/zf832.rpy:15
translate deutsch zf832_s0_f6a7cbc7:

    # nr "Тот, кто препарировал труп, похоже, сжалился над ней: он зашил ей рот аккуратными мелкими стежками и наколол на лбу номер «832» элегантным шрифтом."
    nr ""

# game/dlgs/mortuary_zombies/zf832.rpy:62
translate deutsch zf832_s1_9a22dfe6:

    # nr "Труп продолжает пялиться на тебя."
    nr ""

# game/dlgs/mortuary_zombies/zf832.rpy:80
translate deutsch zf832_s2_d8c406b9:

    # nr "Труп не реагирует. Кажется, он слишком далек от того, чтобы отвечать на твои вопросы."
    nr ""

# game/dlgs/mortuary_zombies/zf832.rpy:98
translate deutsch zf832_s3_d8c406b9:

    # nr "Труп не реагирует. Кажется, он слишком далек от того, чтобы отвечать на твои вопросы."
    nr ""

