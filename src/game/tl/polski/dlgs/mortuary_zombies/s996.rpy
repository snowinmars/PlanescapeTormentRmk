# TODO: Translation updated at 2025-11-26 01:21

# game/dlgs/mortuary_zombies/s996.rpy:14
translate polski s996_s0_6ebef183:

    # nr "Этот скелет довольно стар: кожаные ремешки потрескались и истерлись от времени."
    nr ""

# game/dlgs/mortuary_zombies/s996.rpy:15
translate polski s996_s0_8338a24b:

    # nr "На лбу с особым искусством вырезано слово «ПОКАЙСЯ»; более грубая рука уже позднее высекла на обоих висках число «996»."
    nr ""

# game/dlgs/mortuary_zombies/s996.rpy:94
translate polski s996_s1_b21906a0:

    # nr "Скелет не реагирует."
    nr ""

# game/dlgs/mortuary_zombies/s996.rpy:113
translate polski s996_s2_1edd917f:

    # nr "Скелет не реагирует. Кажется, он слишком далек от того, чтобы отвечать на твои вопросы."
    nr ""

# game/dlgs/mortuary_zombies/s996.rpy:132
translate polski s996_s3_22e74d9a:

    # nr "Кто-то позаботился о том, чтобы связать кости скелета кожаными ремнями, обвивающими тело на манер, напоминающий мускулы и сухожилия."
    nr ""

# game/dlgs/mortuary_zombies/s996.rpy:133
translate polski s996_s3_c1f9d45e:

    # nr "Ремни привязаны к железными скобам, вбитым в суставы скелета."
    nr ""

# game/dlgs/mortuary_zombies/s996.rpy:134
translate polski s996_s3_d20a6998:

    # nr "Кажется, этот скелет довольно долго служил: кости растрескались, многочисленные трещины на них залиты вонючим клеем."
    nr ""

# game/dlgs/mortuary_zombies/s996.rpy:182
translate polski s996_s4_252c838d:

    # nr "Ты тянешь за железные скобы, но тебе не хватает сил, чтобы вытащить их. Они накрепко забиты."
    nr ""

# game/dlgs/mortuary_zombies/s996.rpy:214
translate polski s996_s5_cf0eae7c:

    # nr "Ты тянешь за железные скобы изо всех сил, и через несколько мгновений вырываешь их из суставов. Скелет разваливается, некоторые из его костей продолжают шевелиться."
    nr ""

# game/dlgs/mortuary_zombies/s996.rpy:225
translate polski s996_s6_8d5e3bc9:

    # nr "С помощью ломика ты вырываешь скобы из суставов. Скелет разваливается, хотя некоторые кости продолжают шевелиться."
    nr ""

# game/dlgs/mortuary_zombies/s996.rpy:236
translate polski s996_s7_1edd917f:

    # nr "Скелет не реагирует. Кажется, он слишком далек от того, чтобы отвечать на твои вопросы."
    nr ""

translate polski strings:

    # game/dlgs/mortuary_zombies/s996.rpy:184
    old "«Если бы у меня был подходящий инструмент, я бы смог их вытащить… хммм. Я еще вернусь, Костяшка»."
    new ""

