...
label dustfem_s0: # - # IF ~  Global("Appearance","GLOBAL",1)
    nr 'Die Staubmenschen-Frau scheint dich nicht zu bemerken. Sie hält dich wohl für eine dieser lebenden Leichen, die hier arbeiten.'
...
label dustfem_s1: # from 0.0 0.1 0.2 0.3
    nr 'Die Staubmenschen-Frau zuckt zusammen und wirft den Kopf zu dir herum. Sie wirkt schockiert - deine Verkleidung muß ziemlich gut sein.'

    menu:
        'Nutze ihre Überraschung aus und brich ihr das Genick, bevor sie rufen kann.':
            # a5 # r1172
            jump dustfem_s41
...
label dustfem_s2: # from 1.1 1.2 4.3 5.2 5.3 6.4 19.6 20.4 47.2 47.3 51.4
    nr 'Die Staubmenschen-Frau geht einen Schritt zurück und klatscht dreimal fest in die Hände. Daraufhin ertönt der Klang einer großen Eisenglocke durch die ganze Leichenhalle.'
...
label dustfem_s3: # externs morte_s84
    nr 'Diese blasse Frau trägt eine lange dunkle Robe und riecht leicht muffig. Ihr Gesicht ist ausdruckslos; sie scheint in ihre Arbeit vertieft.'
...
label dustfem_s4: # from 3.0 3.1 3.2 3.3 40.2 40.3
    nr 'Die Staubmenschen-Frau blickt langsam auf und dreht sich zu dir um. "Hast du dich verlaufen?"'
...
label dustfem_s5: # from 4.0 16.2 51.1
    ...
    menu:
        'Brich ihr das Genick, bevor sie rufen kann.' if dustfemLogic.r1235_condition():
            # a18 # r1235
            jump dustfem_s44

        'Brich ihr das Genick, bevor sie rufen kann.' if dustfemLogic.r1236_condition():
            # a19 # r1236
            jump dustfem_s41
...
label dustfem_s6: # from 4.1 4.2 51.2 51.3
    ...
    menu:
        ...
        '"Ich bin wegen einer Bestattung hier, aber da scheint etwas schiefgelaufen zu sein."' if dustfemLogic.r1242_condition():
            # a25 # r1242
            jump dustfem_s16
...
label dustfem_s7: # from 6.0 9.0 20.0
    nr '"Das geht mich leider sehr wohl etwas an. Vielleicht können dir die Wächter ja die Zunge lösen." Die Staubmenschen-Frau geht einen Schritt zurück; sie sieht aus, als würde sie jeden Moment die Wache rufen.'

    menu:
        'Brich ihr das Genick, bevor sie rufen kann.' if dustfemLogic.r1244_condition():
            # a27 # r1244
            jump dustfem_s44

        'Brich ihr das Genick, bevor sie rufen kann.' if dustfemLogic.r1245_condition():
            # a28 # r1245
            jump dustfem_s41
...
label dustfem_s8: # from 6.1 16.0 20.1
    nr '"Du machst wohl Witze? Vielleicht würdest du das gerne den Wächtern erzählen." Die Staubmenschen-Frau geht einen Schritt zurück; sie sieht aus, als würde sie jeden Moment die Wache rufen.'

    menu:
        'Brich ihr das Genick, bevor sie rufen kann.' if dustfemLogic.r1247_condition():
            # a30 # r1247
            jump dustfem_s44

        'Brich ihr das Genick, bevor sie rufen kann.' if dustfemLogic.r1248_condition():
            # a31 # r1248
            jump dustfem_s41
...
label dustfem_s9: # from 6.2 20.2
    ...
    menu:
        ...
        '"Ich bin hier, um Dhall zu treffen."' if dustfemLogic.r1253_condition():
            # a34 # r1253
            jump dustfem_s10

        '"Ich bin gekommen, um Dhall zu sehen."' if dustfemLogic.r1255_condition():
            # a35 # r1255
            jump dustfem_s11

        '"Ich bin gekommen, um Deionarra zu sehen."' if dustfemLogic.r1258_condition():
            # a36 # r1258
            jump dustfem_s13

        '"Ich bin gekommen, um Deionarra zu sehen."' if dustfemLogic.r4336_condition():
            # a37 # r4336
            jump dustfem_s12

        '"Ich bin gekommen, um Soego zu sehen."' if dustfemLogic.r33224_condition():
            # a38 # r33224
            jump dustfem_s15

        '"Ich bin gekommen, um Soego zu sehen."' if dustfemLogic.r33226_condition():
            # a39 # r33226
            jump dustfem_s14

        'Lüge: "Äh… Adahn. Arbeitet er noch hier, oder…?"' if dustfemLogic.r33227_condition():
            # a40 # r33227
            $ dustfemLogic.r33227_action()
            jump dustfem_s21

        'Lüge: "Äh… Adahn. Arbeitet er noch hier, oder…?"' if dustfemLogic.r33229_condition():
            # a41 # r33229
            jump dustfem_s21
...
label dustfem_s17: # from 16.1
    nr '"Mehrere Grabkammern umgeben die Leichenhalle. Sie folgen dem Verlauf der Mauer im Erdgeschoß und im ersten Stock. Kennst du den Namen des Verstorbenen?"'
...
label dustfem_s19: # from 17.1
    ...
    menu:
        ...
        '"Ihr Name ist Deionarra."' if dustfemLogic.r1272_condition():
            # a56 # r1272
            jump dustfem_s12

        'Lüge: "Sein Name ist… äh, Adahn."' if dustfemLogic.r1273_condition():
            # a57 # r1273
            $ dustfemLogic.r1273_action()
            jump dustfem_s21

        'Lüge: "Sein Name ist… äh, Adahn."' if dustfemLogic.r1274_condition():
            # a58 # r1274
            jump dustfem_s21

        'Beug dich nach vorne, als wenn du ihr etwas zuflüstern wolltest, und brich ihr das Genick.' if dustfemLogic.r1275_condition():
            # a59 # r1275
            jump dustfem_s44

        'Beug dich nach vorne, als wenn du ihr etwas zuflüstern wolltest, und brich ihr das Genick.' if dustfemLogic.r1276_condition():
            # a60 # r1276
            jump dustfem_s45
...
label dustfem_s20: # from 9.9 19.0
    ...
    menu:
        ...
        '"Ich bin wegen einer Bestattung hier, aber da scheint etwas schiefgelaufen zu sein."' if dustfemLogic.r1281_condition():
            # a65 # r1281
            jump dustfem_s16
...
label dustfem_s23: # from 22.0
    nr 'Sie dreht sich langsam um. Ihre Augen gleiten auf deine Robe. "Sei gegrüßt, Eingeweihter."'
...
label dustfem_s24: # from 23.0
    ...
    menu:
        'Lüge: "Sein Name ist… äh, Adahn."' if dustfemLogic.r1290_condition():
            # a74 # r1290
            $ dustfemLogic.r1290_action()
            jump dustfem_s49

        'Lüge: "Sein Name ist… äh, Adahn."' if dustfemLogic.r1291_condition():
            # a75 # r1291
            jump dustfem_s49

        '"Mein Name geht dich nichts an. Ich muß mich verabschieden. Leb wohl."' if dustfemLogic.r1292_condition():
            # a76 # r1292
            jump dustfem_s47

        '"Mein Name geht dich nichts an. Ich muß mich verabschieden. Leb wohl."' if dustfemLogic.r1293_condition():
            # a77 # r1293
            jump dustfem_s46
...
label dustfem_s25: # from 23.1
    ...
    menu:
        'Lüge: "Sein Name ist… äh, Adahn."' if dustfemLogic.r1294_condition():
            # a78 # r1294
            $ dustfemLogic.r1294_action()
            jump dustfem_s49

        'Lüge: "Sein Name ist… äh, Adahn."' if dustfemLogic.r1295_condition():
            # a79 # r1295
            jump dustfem_s49

        '"Mein Name geht dich nichts an. Ich muß mich verabschieden. Leb wohl."' if dustfemLogic.r1296_condition():
            # a80 # r1296
            jump dustfem_s47

        '"Mein Name geht dich nichts an. Ich muß mich verabschieden. Leb wohl."' if dustfemLogic.r1297_condition():
            # a81 # r1297
            jump dustfem_s46
...
label dustfem_s29: # from 28.0
    ...
    menu:
        '"Ich bin noch nicht lange eingeweiht. Vergib mir meine Unwissenheit."' if dustfemLogic.r1396_condition():
            # a92 # r1396
            jump dustfem_s50

        '"Ich bin… hier neu. Ich… versuche mich zurechtzufinden."' if dustfemLogic.r1397_condition():
            # a93 # r1397
            jump dustfem_s47

        '"Nun ja… was ist schon ein Name? Bleib bei deinem Glauben, äh, Eingeweihter."' if dustfemLogic.r1398_condition():
            # a94 # r1398
            jump dustfem_s47

        '"Wenn du mir nicht helfen kannst, dann werde ich mir eben jemand suchen, der„s kann. Leb wohl."' if dustfemLogic.r1399_condition():
            # a95 # r1399
            jump dustfem_s46
...
label dustfem_s30: # from 28.1
    ...
    menu:
        ...
        '"Wo finde ich Dhall?"' if dustfemLogic.r4281_condition():
            # a97 # r4281
            jump dustfem_s32

        '"Wo finde ich Dhall?"' if dustfemLogic.r4282_condition():
            # a98 # r4282
            jump dustfem_s33
...
label dustfem_s31: # from 30.0 32.0 33.0
    nr 'Die Staubmenschen-Frau nickt. "Dhall ist krank. Er ist alt, selbst für einen Githzerai. Der Tod ist ihm gewiß nach diesem Siechtum. Er hat wirklich Glück."'
...
label dustfem_s35: # from 31.1 34.0
    nr '"Die Githzerai sind…" Die Staubmenschen-Frau zögert; dann sieht sie dich durchdringend an. "Du bist nicht von hier. Wer bist du?"'

    menu:
        '"Ich bin noch nicht lange eingeweiht. Vergib mir meine Unwissenheit."' if dustfemLogic.r4296_condition():
            # a114 # r4296
            jump dustfem_s50

        '"Ich bin… hier neu. Ich… versuche mich zurechtzufinden."' if dustfemLogic.r4297_condition():
            # a115 # r4297
            jump dustfem_s47

        '"Nun ja… was ist schon ein Name? Bleib bei deinem Glauben, äh, Eingeweihter."' if dustfemLogic.r4298_condition():
            # a116 # r4298
            jump dustfem_s47

        '"Wenn du mir nicht helfen kannst, dann werde ich mir eben jemand suchen, der„s kann. Leb wohl."' if dustfemLogic.r4300_condition():
            # a117 # r4300
            jump dustfem_s46
...
label dustfem_s37: # from 36.0
    nr 'Die Staubmenschen-Frau nickt. "Ja." Sie runzelt die Stirn und sieht dich durchdringend an. "Du bist nicht von hier. Wer bist du?"'

    menu:
        '"Ich bin noch nicht lange eingeweiht. Vergib mir meine Unwissenheit."' if dustfemLogic.r4303_condition():
            # a121 # r4303
            jump dustfem_s50

        '"Ich bin… hier neu. Ich… versuche mich zurechtzufinden."' if dustfemLogic.r4304_condition():
            # a122 # r4304
            jump dustfem_s47

        '"Nun ja… was ist schon ein Name? Bleib bei deinem Glauben, äh, Eingeweihter."' if dustfemLogic.r4305_condition():
            # a123 # r4305
            jump dustfem_s47

        '"Wenn du mir nicht helfen kannst, dann werde ich mir eben jemand suchen, der„s kann. Leb wohl."' if dustfemLogic.r4306_condition():
            # a124 # r4306
            jump dustfem_s46
...
# s38 # say1212
label dustfem_s38: # -
    nr '" Du bist keiner der Unseren, oder? Was führt dich hierher? Bist du ein Anarchist? Oder ein Spitzel eines anderen Bundes?" Die Staubmenschen-Frau geht einen Schritt zurück. "Wache! Wache!"'
    ...
    menu:
        ...
        '"Schhhh! Ich kann dir ja gar keine Antwort geben, wenn du so schreist!"' if dustfemLogic.r4308_condition():
            # a126 # r4308
            $ dustfemLogic.r4308_action()
            jump dustfem_dispose

        '"Schhhh! Ich kann dir ja gar keine Antwort geben, wenn du so schreist!"' if dustfemLogic.r4309_condition():
            # a127 # r4309
...
label dustfem_s40: # -
    ...
    menu:
        '"Sei gegrüßt."' if dustfemLogic.r4312_condition():
            # a130 # r4312
            jump morte_s81  # EXTERN

        '"Sei gegrüßt."' if dustfemLogic.r4313_condition():
            # a131 # r4313
            jump morte_s83  # EXTERN

        '"Sei gegrüßt."' if dustfemLogic.r4314_condition():
            # a132 # r4314
            jump dustfem_s4

        '"Sei gegrüßt."' if dustfemLogic.r4315_condition():
            # a133 # r4315
            jump dustfem_s4
...
label dustfem_s41: # from 1.0 5.1 7.1 8.1 47.1
    nr 'Bevor die Staubmenschen-Frau auch nur ein Wort sagen kann, packst du sie an den Schläfen und drehst ihren Kopf ruckartig nach links.'
...
label dustfem_s42: # from 41.0 45.0
    ...
    menu:
        '"Besser du als ich, Staubie."' if dustfemLogic.r4318_condition():
            # a136 # r4318
            $ dustfemLogic.r4318_action()
            jump dustfem_s43

        '"Besser du als ich, Staubie."' if dustfemLogic.r4319_condition():
            # a137 # r4319
...
label dustfem_s44: # from 5.0 7.0 8.0 19.4 47.0
    nr 'Du bist nicht schnell genug. Die Staubmenschen-Frau weicht deinem Angriffsversuch aus. Sie geht einen Schritt zurück und klatscht dreimal fest in Hände. Daraufhin ertönt der Klang einer großen Eisenglocke durch die Leichenhalle.'
...
label dustfem_s45: # from 19.5
    nr 'Als du dich vorbeugst, um ihr etwas „zuzuflüstern“, beugt die Staubmenschen-Frau ihren Kopf auch nach vorne. Sobald du kannst, packst du sie an den Schläfen und drehst ihren Kopf ruckartig nach links.'
...
label dustfem_s46: # from 24.3 25.3 29.3 35.3 37.3 49.3 50.1
    nr 'Die Staubmenschen-Frau wirkt mißtrauisch. Sie sieht aus, als würde sie jeden Moment etwas sagen, doch dann schüttelt sie nur kurz den Kopf und wendet sich wieder ihrer Arbeit zu'
...
label dustfem_s47: # from 24.2 25.2 29.1 29.2 35.1 35.2 37.1 37.2 49.1 49.2
    ...
    menu:
        'Brich ihr das Genick, bevor sie rufen kann.' if dustfemLogic.r4324_condition():
            # a142 # r4324
            jump dustfem_s44

        'Brich ihr das Genick, bevor sie rufen kann.' if dustfemLogic.r4325_condition():
            # a143 # r4325
            jump dustfem_s41
...
label dustfem_s48: # from 10.0 11.0 12.0 13.0 14.0 15.0 26.3 27.1 28.3 30.4 31.4 32.1 33.1 34.3 36.2 39.1
    nr 'Die Staubmenschen-Frau nickt, bevor sie sich wieder ihrer Arbeit zuwendet.'
...
label dustfem_s49: # from 24.0 24.1 25.0 25.1
    ...
    menu:
        '"Ich bin noch nicht lange eingeweiht. Vergib mir meine Unwissenheit."' if dustfemLogic.r4329_condition():
            # a147 # r4329
            jump dustfem_s50

        '"Ich… bin hier neu. Ich… bin dabei, mich einzuarbeiten."' if dustfemLogic.r4331_condition():
            # a148 # r4331
            jump dustfem_s47

        '"Nun ja… was ist schon ein Name? Bleib bei deinem Glauben, äh, Eingeweihter."' if dustfemLogic.r4332_condition():
            # a149 # r4332
            jump dustfem_s47

        '"Wenn du mir nicht helfen kannst, dann werde ich mir eben jemand suchen, der„s kann. Leb wohl."' if dustfemLogic.r4333_condition():
            # a150 # r4333
            jump dustfem_s46
...
label dustfem_s50: # from 29.0 35.0 37.0 49.0
    nr 'Die Staubmenschen-Frau runzelt weiter die Stirn und sagt mit einem leichten Nicken. "Sehr gut. Was kann ich für dich tun, Eingeweihter?"'
...
label dustfem_s51: # - # IF ~  Global("Appearance","GLOBAL",0)
    ...
    menu:
        '"Nein, ich bin Mitglied des Bundes. Ich seh mir nur ein bißchen die Leichenhalle an."' if dustfemLogic.r66684_condition():
            # a153 # r66684
            jump dustfem_s52

        '"Ja."' if dustfemLogic.r66685_condition():
            # a154 # r66685
            jump dustfem_s5

        '"Nein."' if dustfemLogic.r66686_condition():
            # a155 # r66686
            jump dustfem_s6

        '"Nein, ich hab mich nicht verlaufen. Ich hätte da ein paar Fragen…"' if dustfemLogic.r66687_condition():
            # a156 # r66687
            jump dustfem_s6

        '"Leb wohl."' if dustfemLogic.r66688_condition():
            # a157 # r66688
            jump dustfem_s2
