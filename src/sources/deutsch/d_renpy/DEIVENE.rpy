...
label eivene_s0: # - # IF ~  Global("EiVene","GLOBAL",0)
    ...
    menu:
        ...
        'Laß sie in Ruhe.{#eivene_s0_r3407}':
            # a1 # r3407
            jump eivene_dispose
...
label eivene_s1: # from 0.0
    ...
    menu:
        ...
        '"Ich sagte: Sei gegrüßt."{#eivene_s1_r3413}' if eiveneLogic.r3413_condition():
            # a3 # r3413
            jump eivene_s3

        '"Was ist denn mit deinen Händen los?"{#eivene_s1_r3414}' if eiveneLogic.r3414_condition():
            # a4 # r3414
            jump eivene_s2

        '"Was ist denn mit deinen Händen los?"{#eivene_s1_r3415}' if eiveneLogic.r3415_condition():
            # a5 # r3415
            jump eivene_s19

        'Laß sie in Ruhe.{#eivene_s1_r3416}':
            # a6 # r3416
            jump eivene_dispose
...
label eivene_s2: # from 1.0 1.2
    ...
    menu:
        ...
        'Laß sie in Ruhe.{#eivene_s2_r3419}':
            # a8 # r3419
            jump eivene_dispose
...
label eivene_s5: # from 4.0
    ...
    menu:
        ...
        '"Ich hätte erst noch ein paar Fragen…"{#eivene_s5_r3425}' if eiveneLogic.r3425_condition():
            # a11 # r3425
            $ eiveneLogic.j37702_s5_r3425_action()
            jump eivene_s6

        '"Ich hätte erst noch ein paar Fragen…"{#eivene_s5_r3426}' if eiveneLogic.r3426_condition():
            # a12 # r3426
            $ eiveneLogic.j37702_s5_r3426_action()
            jump eivene_s20

        '"Was ist denn mit deinen Händen los?"{#eivene_s5_r3427}' if eiveneLogic.r3427_condition():
            # a13 # r3427
            $ eiveneLogic.j37702_s5_r3427_action()
            jump eivene_s6

        '"Was ist denn mit deinen Händen los?"{#eivene_s5_r3428}' if eiveneLogic.r3428_condition():
            # a14 # r3428
            $ eiveneLogic.j37702_s5_r3428_action()
            jump eivene_s20

        'Verschwinde.{#eivene_s5_r3429}':
            # a15 # r3429
...
label eivene_s8: # from 7.1
    ...
    menu:
        ...
        'Geh. Und zwar schnell.{#eivene_s8_r3438}':
            # a21 # r3438
            jump eivene_dispose
...
label eivene_s9: # from 7.0 8.0
    ...
    menu:
        ...
        '"Äh, nicht, daß mir das nicht schmeichelt, aber…"{#eivene_s9_r3441}' if eiveneLogic.r3441_condition():
            # a23 # r3441
            jump morte_s59  # EXTERN

        'Spiel weiter den Zombie.{#eivene_s9_r3442}' if eiveneLogic.r3442_condition():
            # a24 # r3442
            jump eivene_s11

        'Spiel weiter den Zombie.{#eivene_s9_r3443}' if eiveneLogic.r3443_condition():
            # a25 # r3443
            jump morte_s59  # EXTERN

        'Stoß sie weg und geh.{#eivene_s9_r3444}':
            # a26 # r3444
            jump eivene_s10
...
label eivene_s12: # from 11.0
    ...
    menu:
        ...
        'Stoß sie weg und geh.{#eivene_s12_r3450}':
            # a30 # r3450
            jump eivene_s10
...
label eivene_s13: # from 12.0
    ...
    menu:
        ...
        'Laß sie ihre Arbeit tun.{#eivene_s13_r3453}' if eiveneLogic.r3453_condition():
            # a32 # r3453
...
label eivene_s14: # from 13.0
    ...
    menu:
        ...
        '"Warte einen Moment." (Du machst eine Handbewegung, als ob du einen Schlüssel umdrehtest.) "Ich brauche einen Schlüssel zum Balsamierungsraum. Hast du einen?"{#eivene_s14_r3457}' if eiveneLogic.r3457_condition():
            # a34 # r3457
            jump eivene_s24

        'Verschwinde.{#eivene_s14_r4350}':
            # a35 # r4350
            jump eivene_dispose
...
label eivene_s15: # - # IF ~  Global("EiVene","GLOBAL",1)
    ...
    menu:
        ...
        'Klopf ihr auf die Schulter, damit sie dich bemerkt.{#eivene_s15_r3463}' if eiveneLogic.r3463_condition():
            # a37 # r3463
            jump eivene_s17

        'Klopf ihr auf die Schulter, damit sie dich bemerkt.{#eivene_s15_r4351}' if eiveneLogic.r4351_condition():
            # a38 # r4351
            jump eivene_s22

        'Verschwinde.{#eivene_s15_r4352}':
            # a39 # r4352
            jump eivene_dispose
...
label eivene_s17: # from 6.0 15.1 25.0 27.0
    ...
    menu:
        ...
        '"Warte einen Moment." (Du machst eine Handbewegung, als ob du einen Schlüssel umdrehtest.) "Ich brauche einen Schlüssel zum Balsamierungsraum. Hast du einen?"{#eivene_s17_r3470}' if eiveneLogic.r3470_condition():
            # a41 # r3470
            $ eiveneLogic.r3470_action()
            jump eivene_s18

        '"Warte einen Moment." (Du machst eine Handbewegung, als ob du einen Schlüssel umdrehtest.) "Ich brauche einen Schlüssel zum Balsamierungsraum. Hast du einen?"{#eivene_s17_r3497}' if eiveneLogic.r3497_condition():
            # a42 # r3497
            jump eivene_s24

        'Verschwinde.{#eivene_s17_r4357}':
            # a43 # r4357
            jump eivene_dispose
...
label eivene_s18: # from 14.0 17.1 22.0
    ...
    menu:
        ...
        '"Was ist denn mit deinen Händen los?"{#eivene_s18_r3495}' if eiveneLogic.r3495_condition():
            # a45 # r3495
            $ eiveneLogic.j38203_s18_r3495_action()
            jump eivene_s21

        'Verschwinde.{#eivene_s18_r3496}':
            # a46 # r3496
...
label eivene_s22: # from 15.2 25.1 27.1
    ...
    menu:
        ...
        '"Warte einen Moment." (Du machst eine Handbewegung, als ob du einen Schlüssel umdrehtest.) "Ich brauche einen Schlüssel zum Balsamierungsraum. Hast du einen?"{#eivene_s22_r3502}' if eiveneLogic.r3502_condition():
            # a48 # r3502
            jump eivene_s24

        'Verschwinde.{#eivene_s22_r4358}':
            # a49 # r4358
            jump eivene_dispose
...
label eivene_s25: # -
    ...
    menu:
        ...
        'Klopf ihr auf die Schulter, damit sie dich bemerkt.{#eivene_s25_r4355}' if eiveneLogic.r4355_condition():
            # a53 # r4355
            jump eivene_s22

        'Verschwinde.{#eivene_s25_r4356}':
            # a54 # r4356
            jump eivene_dispose
...
label eivene_s26: # from 16.0
    ...
    menu:
        ...
        'Echo: "Bewahre diese Dinge gut auf und warte, bis ich zurrücckkomme."{#eivene_s26_r63479}' if eiveneLogic.r63479_condition():
            # a56 # r63479
            jump eivene_s27
...
label eivene_s27: # from 26.0 26.1
    ...
    menu:
        ...
        'Tipp ihr auf die Schulter, mach sie auf dich aufmerksam.{#eivene_s27_r63481}' if eiveneLogic.r63481_condition():
            # a58 # r63481
            jump eivene_s22

        'Geh weg.{#eivene_s27_r63483}':
            # a59 # r63483
            jump eivene_dispose
