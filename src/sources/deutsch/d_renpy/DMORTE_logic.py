class MorteLogic:
    ...
    def j64512_s85_r4676_action(self):
        ...
        #$% .register('64512', 'Anscheinend ist der als Zombie verkleidete Mann ein Mitglied der Anarchisten. Das ist ein Bund, der darauf aus ist, sich Autoritätspersonen zu schnappen und alles niederzureißen, was auch nur im Entferntesten nach dem 'Establishment' riecht.') %$#
    ...
    def r20612_action(self):
        ...
        #$%.register('20538', 'Marta, die Näherin, scheint die Aufgabe zu haben, Leichen, die ihr gegeben werden, die Zähne herauszureißen, ihre Nähte aufzulösen, nach Wertsachen zu suchen, die sie an oder in der Leiche finden kann, und sie dann wieder zusammenzunähen. %$#')
    ...
    def r24932_action(self):
        ...
        #$%.register('24933', 'Ich habe begonnen, meine Erfahrungen aufzuschreiben, um einen klaren Kopf zu behalten, in der Hoffnung, daß ich in die Geschehnisse etwas Sinn bringen kann. Ich bin an einem Ort aufgewacht, der "Leichenhalle" genannt wird... Sieht aus wie eine Art Leichenschauhaus. Ich weiß nicht, wer ich bin, was ich hier tue oder wie ich hierher gekommen bin. Das Erste, was ich nach dem Aufwachen gefunden habe, war ein sprechender Totenkopf namens Morte. Er sagt, daß wir eingeschlossen seien und wahrscheinlich eine der trottenden Leichen im Raum den Schlüssel habe. Er schlug vor, ich solle die Regale im Raum nach einem Skalpell durchsuchen, und es dann in eine der Leichen stoßen, bis sie "stirbt". %$#')
    ...
    def r41921_action(self):
        ...
        #$%.register('67862', 'Ich habe den Mann, der Übelwind verflucht hat, in der Städtischen Festhalle gefunden: Jumble Mördersinn. %$#')
    ...
    def r43910_action(self):
        ...
        #$%.register('39490', 'Ich hab Nemelle erzählt, daß ihre Freundin, Aelwyn, nach ihr suchen würde. Sie belohnte mich mit einem magischen Kuß, der meine maximale Anzahl an Trefferpunkten um drei erhöht hat. %$#')
    ...
    def j39477_s478_r45093_action(self):
        ...
        #$% .register('39477', 'Morte hat etwas von der 'Regel der Drei' erwähnt. Anscheinend glauben manche, daß alles immer in Dreiergruppen passiert, so daß sie der Zahl große Bedeutung beimessen.') %$#
    ...
    def r53629_action(self):
        ...
        #$%.register('53633', 'Morte hat gesagt, daß ich es war, der ihn vor langer Zeit aus der Säule gezogen hat, obwohl er nicht genau wisse, wie. Er meinte auch, daß er dabei das gesamte in der Säule angesammelte Wissen verloren hätte... Was erklärt, warum er nicht *ganz* so nützlich ist, wie er meiner früheren Inkarnation versprochen hatte. %$#')
    ...
    def r53630_action(self):
        ...
        #$%.register('53661', 'Es hat sich herausgestellt, daß Morte aus einer sogenannten Schädelsäule stammt, einem Haufen lebender, vom Körper abgetrennter Schädel auf Avernus, der ersten Unterebene von Baator. Sie besteht aus den Schädeln jener, deren Lügen andere in den Tod geschickt haben. %$#')
    ...
    def r65569_action(self):
        ...
        #$%.register('65573', 'Ich habe Morte mit der zusätzlichen Textzeile auf meinem Rücken konfrontiert, die er "vergessen" hatte mir vorzulesen. Er behauptete, sie sei völlig unwichtig und daß er sie mir deshalb nicht vorgelesen hätte. Ich bin sicher, daß er lügt, aber ich weiß noch nicht warum. %$#')
    ...
    def r65621_action(self):
        ...
        #$%.register('65625', 'Morte hat mir gesagt, daß er eine gewisse "Macht" hat, Leute in Rage zu versetzen, die Litanei der Flüche heißt. Anscheinend hat er im Laufe seines Lebens so viele Beleidigungen gesammelt, daß er genau weiß, wie er Leute so provozieren kann, daß sie ihn angreifen. Wenn er sie wütend macht, sind sie leichter zu treffen, und sie haben einen Strafpunkt auf ihre Trefferpunkte gegen andere. Außerdem werden sie nichts anderes tun als Morte zu jagen und zu versuchen, mit bloßen Händen auf ihn einzutrommeln. Hört sich so an, als ob er sich als ein nützlicher Begleiter erweisen könnte, wenn ich Zauberwerfern begegne. Und da sein Talent nicht magischer Natur ist, könnte es auch bei Wesen wirksam sein, die Zauber mit einem Schulterzucken abschütteln können. Ich nehme an, daß Mortes Fähigkeit immer besser wird, je mehr Beleidigungen er von den Leuten, die ich treffe, hört. %$#')


    def j65637_s667_r65631_action(self):
        self.state_manager.journal_manager.update_journal('65637')
        #$% .register('65637', 'Morte hat mir erzählt, daß er während seines Aufenthalts auf Lothars Regal eine neue Fähigkeit dazugewonnen hat, die "Schädelmob" heißt. Anscheinend kann er ein paarmal am Tag einige Freunde heraufbeschwören, die er bei Lothar getroffen hat. Sie greifen dann einen Gegner Mortes Wahl an, beißen ihn und rennen dann weg. Der Schaden, den sie anrichten, hängt von Mortes Erfahrungsstufe ab.') %$#


    def j65669_s675_r65666_action(self):
        self.state_manager.journal_manager.update_journal('65669')
        #$% .register('65669', 'Morte hat vorgeschlagen, ich solle mal die Städtische Festhalle im Bezirk der Kuratoren aufsuchen und sehen, ob ich dort mehr über die Nachthexe Ravel herausfinden kann.') %$#


    def j65678_s677_r65674_action(self):
        self.state_manager.journal_manager.update_journal('65678')
        #$% .register('65678', 'Morte hat angedeutet, daß jemand im Bezirk der Kuratoren wissen könnte, wo man ein Stück von Ravel bekommen kann. Wenn nicht, könnten wir die Festhalle und die Sinnessteine dort ausprobieren. Und wenn *das* auch nicht hilft, können wir wahrscheinlich immer noch ein paar Prostituierte besuchen... Das wäre auf jeden Fall netter als diese Rumrennerei und das Reden mit all diesen Leuten.') %$#


    def j65712_s686_r65710_action(self):
        self.state_manager.journal_manager.update_journal('65712')
        #$% .register('65712', 'Ich habe Morte damit konfrontiert, daß er mir gesagt hat, er wäre ein Mimir, aber ich habe nichts, womit ich ihn festnageln kann. Ich glaube, ich muß ihn bei einer Lüge ertappen, um die Wahrheit aus ihm herauszubekommen.') %$#
    ...
    def r65821_action(self):
        ...
        #$%.register('65825', 'Ich habe entdeckt, warum Morte die ganze Zeit bei mir geblieben ist... Er fühlt sich schuldig, weil er mich einmal, als er noch lebte, getötet hat. Als ich ihn mit seinen Schuldgefühlen konfrontierte, schien er eine ziemliche Last loszuwerden, und nun scheint er deshalb viel stärker zu sein. %$#')
