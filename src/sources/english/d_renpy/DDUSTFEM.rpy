...
label dustfem_s0: # - # IF ~  Global("Appearance","GLOBAL",1)
    nr 'The Dustman does not appear to notice you. She must mistake you for one of the cadaverous workers.'
...
label dustfem_s1: # from 0.0 0.1 0.2 0.3
    nr 'The Dustman jumps, then snaps her head around to stare at you. She looks shocked - your disguise must be quite good.'

    menu:
        'Take advantage of her surprise and snap her neck before she can call out.':
            # a5 # r1172
            jump dustfem_s41
...
label dustfem_s2: # from 1.1 1.2 4.3 5.2 5.3 6.4 19.6 20.4 47.2 47.3 51.4
    nr 'The Dustman takes a step back, then claps her hands together sharply three times. In response, a great iron bell starts tolling throughout the Mortuary.'
...
label dustfem_s3: # externs morte_s84
    nr 'This pale-faced woman is dressed in long dark robes. She has a slight musty smell about her. Her expression is blank, and she seems absorbed in her duties.'
...
label dustfem_s4: # from 3.0 3.1 3.2 3.3 40.2 40.3
    nr 'The Dustman slowly lifts her head and turns toward you. "Are you lost?"'
...
label dustfem_s5: # from 4.0 16.2 51.1
    ...
    menu:
        'Snap her neck before she can call out.' if dustfemLogic.r1235_condition():
            # a18 # r1235
            jump dustfem_s44

        'Snap her neck before she can call out.' if dustfemLogic.r1236_condition():
            # a19 # r1236
            jump dustfem_s41
...
label dustfem_s6: # from 4.1 4.2 51.2 51.3
    ...
    menu:
        ...
        '"I was here for an interment, but there seems to have been a mistake."' if dustfemLogic.r1242_condition():
            # a25 # r1242
            jump dustfem_s16
...
label dustfem_s7: # from 6.0 9.0 20.0
    nr '"I„m afraid that it is my concern. Mayhap the guards can loosen your tongue." The Dustman takes a step back; she looks like she is about to summon the guards.'

    menu:
        'Snap her neck before she can call out.' if dustfemLogic.r1244_condition():
            # a27 # r1244
            jump dustfem_s44

        'Snap her neck before she can call out.' if dustfemLogic.r1245_condition():
            # a28 # r1245
            jump dustfem_s41
...
label dustfem_s8: # from 6.1 16.0 20.1
    nr '"Do you speak in jest? Perhaps you would like to share it with the guards." The Dustman takes a step back; she looks like she is about to summon the guards.'

    menu:
        'Snap her neck before she can call out.' if dustfemLogic.r1247_condition():
            # a30 # r1247
            jump dustfem_s44

        'Snap her neck before she can call out.' if dustfemLogic.r1248_condition():
            # a31 # r1248
            jump dustfem_s41
...
label dustfem_s9: # from 6.2 20.2
    ...
    menu:
        ...
        '"I am here to see Dhall."' if dustfemLogic.r1253_condition():
            # a34 # r1253
            jump dustfem_s10

        '"I am here to see Dhall."' if dustfemLogic.r1255_condition():
            # a35 # r1255
            jump dustfem_s11

        '"I am here to see Deionarra."' if dustfemLogic.r1258_condition():
            # a36 # r1258
            jump dustfem_s13

        '"I am here to see Deionarra."' if dustfemLogic.r4336_condition():
            # a37 # r4336
            jump dustfem_s12

        '"I am here to see Soego."' if dustfemLogic.r33224_condition():
            # a38 # r33224
            jump dustfem_s15

        '"I am here to see Soego."' if dustfemLogic.r33226_condition():
            # a39 # r33226
            jump dustfem_s14

        'Lie: "Uh… Adahn. Does he still work here, or…?"' if dustfemLogic.r33227_condition():
            # a40 # r33227
            $ dustfemLogic.r33227_action()
            jump dustfem_s21

        'Lie: "Uh… Adahn. Does he still work here, or…?"' if dustfemLogic.r33229_condition():
            # a41 # r33229
            jump dustfem_s21
...
label dustfem_s19: # from 17.1
    ...
    menu:
        ...
        '"The name is Deionarra."' if dustfemLogic.r1272_condition():
            # a56 # r1272
            jump dustfem_s12

        'Lie: "The name is… uh, Adahn."' if dustfemLogic.r1273_condition():
            # a57 # r1273
            $ dustfemLogic.r1273_action()
            jump dustfem_s21

        'Lie: "The name is… uh, Adahn."' if dustfemLogic.r1274_condition():
            # a58 # r1274
            jump dustfem_s21

        'Lean in as if to whisper something to her, then snap her neck.' if dustfemLogic.r1275_condition():
            # a59 # r1275
            jump dustfem_s44

        'Lean in as if to whisper something to her, then snap her neck.' if dustfemLogic.r1276_condition():
            # a60 # r1276
            jump dustfem_s45
...
label dustfem_s20: # from 9.9 19.0
    ...
    menu:
        ...
        '"I was here for an interment, but there seems to have been a mistake."' if dustfemLogic.r1281_condition():
            # a65 # r1281
            jump dustfem_s16
...
label dustfem_s23: # from 22.0
    nr 'She slowly turns, and her eyes flicker to your robes. "Greetings, fellow initiate."'
...
label dustfem_s24: # from 23.0
    ...
    menu:
        'Lie: "The name is… uh, Adahn."' if dustfemLogic.r1290_condition():
            # a74 # r1290
            $ dustfemLogic.r1290_action()
            jump dustfem_s49

        'Lie: "The name is… uh, Adahn."' if dustfemLogic.r1291_condition():
            # a75 # r1291
            jump dustfem_s49

        '"My name is not your concern. I must take my leave. Farewell."' if dustfemLogic.r1292_condition():
            # a76 # r1292
            jump dustfem_s47

        '"My name is not your concern. I must take my leave. Farewell."' if dustfemLogic.r1293_condition():
            # a77 # r1293
            jump dustfem_s46
...
label dustfem_s25: # from 23.1
    ...
    menu:
        'Lie: "The name is… uh, Adahn."' if dustfemLogic.r1294_condition():
            # a78 # r1294
            $ dustfemLogic.r1294_action()
            jump dustfem_s49

        'Lie: "The name is… uh, Adahn."' if dustfemLogic.r1295_condition():
            # a79 # r1295
            jump dustfem_s49

        '"My name is not your concern. I must take my leave. Farewell."' if dustfemLogic.r1296_condition():
            # a80 # r1296
            jump dustfem_s47

        '"My name is not your concern. I must take my leave. Farewell."' if dustfemLogic.r1297_condition():
            # a81 # r1297
            jump dustfem_s46
...
label dustfem_s29: # from 28.0
    ...
    menu:
        '"I„m a recent initiate. Forgive my ignorance."' if dustfemLogic.r1396_condition():
            # a92 # r1396
            jump dustfem_s50

        '"I„m… new here. I“m… trying to get my bearings."' if dustfemLogic.r1397_condition():
            # a93 # r1397
            jump dustfem_s47

        '"Yeah, well… what„s in a name? Keep the faith, uh, fellow initiate."' if dustfemLogic.r1398_condition():
            # a94 # r1398
            jump dustfem_s47

        '"If you can„t help me, then I will find someone else who can. Farewell."' if dustfemLogic.r1399_condition():
            # a95 # r1399
            jump dustfem_s46
...
label dustfem_s30: # from 28.1
    ...
    menu:
        ...
        '"Where can I find Dhall?"' if dustfemLogic.r4281_condition():
            # a97 # r4281
            jump dustfem_s32

        '"Where can I find Dhall?"' if dustfemLogic.r4282_condition():
            # a98 # r4282
            jump dustfem_s33
...
label dustfem_s31: # from 30.0 32.0 33.0
    nr 'The Dustman nods. "Dhall is ill. He is old, even by githzerai standards. Death will no doubt follow the wasting sickness he has contracted. He is fortunate, indeed."'
...
label dustfem_s35: # from 31.1 34.0
    nr '"The githzerai are…" The Dustman pauses, then stares at you intently. "You are not from around here. Who are you?"'

    menu:
        '"I„m a recent initiate. Forgive my ignorance."' if dustfemLogic.r4296_condition():
            # a114 # r4296
            jump dustfem_s50

        '"I„m… new here. I“m… trying to get my bearings."' if dustfemLogic.r4297_condition():
            # a115 # r4297
            jump dustfem_s47

        '"Yeah, well… what„s in a name? Keep the faith, uh, fellow initiate."' if dustfemLogic.r4298_condition():
            # a116 # r4298
            jump dustfem_s47

        '"If you can„t help me, then I will find someone else who can. Farewell."' if dustfemLogic.r4300_condition():
            # a117 # r4300
            jump dustfem_s46
...
label dustfem_s37: # from 36.0
    nr 'The Dustman nods. "Yes." She frowns, then studies you intently. "You are not from around here. Who are you?"'

    menu:
        '"I„m a recent initiate. Forgive my ignorance."' if dustfemLogic.r4303_condition():
            # a121 # r4303
            jump dustfem_s50

        '"I„m… new here. I“m… trying to get my bearings."' if dustfemLogic.r4304_condition():
            # a122 # r4304
            jump dustfem_s47

        '"Yeah, well… what„s in a name? Keep the faith, uh, fellow initiate."' if dustfemLogic.r4305_condition():
            # a123 # r4305
            jump dustfem_s47

        '"If you can„t help me, then I will find someone else who can. Farewell."' if dustfemLogic.r4306_condition():
            # a124 # r4306
            jump dustfem_s46
...
# s38 # say1212
label dustfem_s38: # -
    nr '"You are not one of us, are you? What are you doing here? Are you a member of the Anarchists? Or a spy of another faction?" The Dustman takes a step back. "Guards! Guards!"'
    ...
    menu:
        ...
        '"Shhhh! I can„t answer you over all that shouting!"' if dustfemLogic.r4308_condition():
            # a126 # r4308
            $ dustfemLogic.r4308_action()
            jump dustfem_dispose

        '"Shhhh! I can„t answer you over all that shouting!"' if dustfemLogic.r4309_condition():
            # a127 # r4309
...
label dustfem_s40: # -
    ...
    menu:
        '"Greetings."' if dustfemLogic.r4312_condition():
            # a130 # r4312
            jump morte_s81  # EXTERN

        '"Greetings."' if dustfemLogic.r4313_condition():
            # a131 # r4313
            jump morte_s83  # EXTERN

        '"Greetings."' if dustfemLogic.r4314_condition():
            # a132 # r4314
            jump dustfem_s4

        '"Greetings."' if dustfemLogic.r4315_condition():
            # a133 # r4315
            jump dustfem_s4
...
label dustfem_s41: # from 1.0 5.1 7.1 8.1 47.1
    nr 'Before the Dustman can utter a word, your hands clamp onto her temples, and you twist her head sharply to the left.'
...
label dustfem_s42: # from 41.0 45.0
    ...
    menu:
        '"Better you than me, Dustie."' if dustfemLogic.r4318_condition():
            # a136 # r4318
            $ dustfemLogic.r4318_action()
            jump dustfem_s43

        '"Better you than me, Dustie."' if dustfemLogic.r4319_condition():
            # a137 # r4319
...
label dustfem_s44: # from 5.0 7.0 8.0 19.4 47.0
    nr 'You are not fast enough, and the Dustman dodges as you lunge for her. Taking a step back, she claps her hands together sharply three times. In response, a great iron bell starts tolling throughout the Mortuary.'
...
label dustfem_s45: # from 19.5
    nr 'As you lean in to „whisper“ to her, the Dustman leans in as well. As she comes within arm„s reach, your hands clamp onto her temples, and you twist her head sharply to the left.'
...
label dustfem_s46: # from 24.3 25.3 29.3 35.3 37.3 49.3 50.1
    nr 'The Dustman seems suspicious. She looks like she„s about to say something, then shakes her head slightly and returns to her duties.'
...
label dustfem_s47: # from 24.2 25.2 29.1 29.2 35.1 35.2 37.1 37.2 49.1 49.2
    ...
    menu:
        'Snap her neck before she can call out.' if dustfemLogic.r4324_condition():
            # a142 # r4324
            jump dustfem_s44

        'Snap her neck before she can call out.' if dustfemLogic.r4325_condition():
            # a143 # r4325
            jump dustfem_s41
...
label dustfem_s48: # from 10.0 11.0 12.0 13.0 14.0 15.0 26.3 27.1 28.3 30.4 31.4 32.1 33.1 34.3 36.2 39.1
    nr 'The Dustman nods, then returns to her duties.'
...
label dustfem_s49: # from 24.0 24.1 25.0 25.1
    ...
    menu:
        '"I„m a recent initiate. Forgive my ignorance."' if dustfemLogic.r4329_condition():
            # a147 # r4329
            jump dustfem_s50

        '"I„m… new here. I“m… trying to learn the routine."' if dustfemLogic.r4331_condition():
            # a148 # r4331
            jump dustfem_s47

        '"Yeah, well… what„s in a name? Keep the faith, uh, fellow initiate."' if dustfemLogic.r4332_condition():
            # a149 # r4332
            jump dustfem_s47

        '"If you can„t help me, then I will find someone else who can. Farewell."' if dustfemLogic.r4333_condition():
            # a150 # r4333
            jump dustfem_s46
...
label dustfem_s50: # from 29.0 35.0 37.0 49.0
    nr 'The Dustman„s frown remains, but she nods slightly. "Very well. How may I be of service to you, initiate?"'
...
label dustfem_s51: # - # IF ~  Global("Appearance","GLOBAL",0)
    ...
    menu:
        '"No, I am a member of the faction. I am just touring the Mortuary."' if dustfemLogic.r66684_condition():
            # a153 # r66684
            jump dustfem_s52

        '"Yes."' if dustfemLogic.r66685_condition():
            # a154 # r66685
            jump dustfem_s5

        '"No."' if dustfemLogic.r66686_condition():
            # a155 # r66686
            jump dustfem_s6

        '"No, I„m not lost. I had some questions…"' if dustfemLogic.r66687_condition():
            # a156 # r66687
            jump dustfem_s6

        '"Farewell."' if dustfemLogic.r66688_condition():
            # a157 # r66688
            jump dustfem_s2
