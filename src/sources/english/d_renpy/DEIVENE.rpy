...
label eivene_s0: # - # IF ~  Global("EiVene","GLOBAL",0)
    ...
    menu:
        ...
        'Leave her alone.{#eivene_s0_r3407}':
            # a1 # r3407
            jump eivene_dispose
...
label eivene_s1: # from 0.0
    ...
    menu:
        ...
        '"I said, Greetings."{#eivene_s1_r3413}' if eiveneLogic.r3413_condition():
            # a3 # r3413
            jump eivene_s3

        '"What„s wrong with your hands?"{#eivene_s1_r3414}' if eiveneLogic.r3414_condition():
            # a4 # r3414
            jump eivene_s2

        '"What„s wrong with your hands?"{#eivene_s1_r3415}' if eiveneLogic.r3415_condition():
            # a5 # r3415
            jump eivene_s19

        'Leave her alone.{#eivene_s1_r3416}':
            # a6 # r3416
            jump eivene_dispose
...
label eivene_s2: # from 1.0 1.2
    ...
    menu:
        ...
        'Leave her alone.{#eivene_s2_r3419}':
            # a8 # r3419
            jump eivene_dispose
...
label eivene_s5: # from 4.0
    ...
    menu:
        ...
        '"I had some questions first…"{#eivene_s5_r3425}' if eiveneLogic.r3425_condition():
            # a11 # r3425
            $ eiveneLogic.j37702_s5_r3425_action()
            jump eivene_s6

        '"I had some questions first…"{#eivene_s5_r3426}' if eiveneLogic.r3426_condition():
            # a12 # r3426
            $ eiveneLogic.j37702_s5_r3426_action()
            jump eivene_s20

        '"What„s wrong with your hands?"{#eivene_s5_r3427}' if eiveneLogic.r3427_condition():
            # a13 # r3427
            $ eiveneLogic.j37702_s5_r3427_action()
            jump eivene_s6

        '"What„s wrong with your hands?"{#eivene_s5_r3428}' if eiveneLogic.r3428_condition():
            # a14 # r3428
            $ eiveneLogic.j37702_s5_r3428_action()
            jump eivene_s20

        'Leave.{#eivene_s5_r3429}':
            # a15 # r3429
...
label eivene_s9: # from 7.0 8.0
    ...
    menu:
        ...
        '"Uh, it„s not that I“m not flattered, but…"{#eivene_s9_r3441}' if eiveneLogic.r3441_condition():
            # a23 # r3441
            jump morte_s59  # EXTERN

        'Keep playing zombie.{#eivene_s9_r3442}' if eiveneLogic.r3442_condition():
            # a24 # r3442
            jump eivene_s11

        'Keep playing zombie.{#eivene_s9_r3443}' if eiveneLogic.r3443_condition():
            # a25 # r3443
            jump morte_s59  # EXTERN

        'Push her away, leave.{#eivene_s9_r3444}':
            # a26 # r3444
            jump eivene_s10
...
label eivene_s12: # from 11.0
    ...
    menu:
        ...
        'Push her away, leave.{#eivene_s12_r3450}':
            # a30 # r3450
            jump eivene_s10
...
label eivene_s13: # from 12.0
    ...
    menu:
        ...
        'Let her work.{#eivene_s13_r3453}' if eiveneLogic.r3453_condition():
            # a32 # r3453
...
label eivene_s14: # from 13.0
    ...
    menu:
        ...
        '"Wait a minute." (You make the motion of a key turning with your hand.) "I need an embalming key. Do you have one?"{#eivene_s14_r3457}' if eiveneLogic.r3457_condition():
            # a34 # r3457
            jump eivene_s24

        'Leave.{#eivene_s14_r4350}':
            # a35 # r4350
            jump eivene_dispose
...
label eivene_s15: # - # IF ~  Global("EiVene","GLOBAL",1)
    ...
    menu:
        ...
        'Tap her, get her attention.{#eivene_s15_r3463}' if eiveneLogic.r3463_condition():
            # a37 # r3463
            jump eivene_s17

        'Tap her, get her attention.{#eivene_s15_r4351}' if eiveneLogic.r4351_condition():
            # a38 # r4351
            jump eivene_s22

        'Leave.{#eivene_s15_r4352}':
            # a39 # r4352
            jump eivene_dispose
...
label eivene_s17: # from 6.0 15.1 25.0 27.0
    ...
    menu:
        ...
        '"Wait a minute." (You make the motion of a key turning with your hand.) "I need an embalming key. Do you have one?"{#eivene_s17_r3470}' if eiveneLogic.r3470_condition():
            # a41 # r3470
            $ eiveneLogic.r3470_action()
            jump eivene_s18

        '"Wait a minute." (You make the motion of a key turning with your hand.) "I need an embalming key. Do you have one?"{#eivene_s17_r3497}' if eiveneLogic.r3497_condition():
            # a42 # r3497
            jump eivene_s24

        'Leave.{#eivene_s17_r4357}':
            # a43 # r4357
            jump eivene_dispose
...
label eivene_s18: # from 14.0 17.1 22.0
    ...
    menu:
        ...
        '"What„s wrong with your hands?"{#eivene_s18_r3495}' if eiveneLogic.r3495_condition():
            # a45 # r3495
            $ eiveneLogic.j38203_s18_r3495_action()
            jump eivene_s21

        'Leave.{#eivene_s18_r3496}':
            # a46 # r3496
...
label eivene_s22: # from 15.2 25.1 27.1
    ...
    menu:
        ...
        '"Wait a minute." (You make the motion of a key turning with your hand.) "I need an embalming key. Do you have one?"{#eivene_s22_r3502}' if eiveneLogic.r3502_condition():
            # a48 # r3502
            jump eivene_s24

        'Leave.{#eivene_s22_r4358}':
            # a49 # r4358
            jump eivene_dispose
...
label eivene_s25: # -
    ...
    menu:
        ...
        'Tap her, get her attention.{#eivene_s25_r4355}' if eiveneLogic.r4355_condition():
            # a53 # r4355
            jump eivene_s22

        'Leave.{#eivene_s25_r4356}':
            # a54 # r4356
            jump eivene_dispose
...
label eivene_s26: # from 16.0
    ...
    menu:
        ...
        'Echo: "Keep these things safe and wait for my return."{#eivene_s26_r63479}' if eiveneLogic.r63479_condition():
            # a56 # r63479
            jump eivene_s27
...
label eivene_s27: # from 26.0 26.1
    ...
    menu:
        ...
        'Tap her, get her attention.{#eivene_s27_r63481}' if eiveneLogic.r63481_condition():
            # a58 # r63481
            jump eivene_s22

        'Leave.{#eivene_s27_r63483}':
            # a59 # r63483
            jump eivene_dispose
