...
label morte1_s0: # - # IF WEIGHT #1 /* Triggers after states #: 26 even though they appear after this state */ ~  !InParty("Morte") Global("Morte","GLOBAL",0)
    nr '"Hé, chef, ça va ? Tu fais l„mort ou t“essaies d„bluffer les Hommes-Poussière ? J“ai vraiment cru qu„t“étais clamsé."~ [MRT001]'
...
label morte1_s1: # from 0.0
    nr '"Euh… qui *je* suis ? Et pourquoi *tu* ne commencerais pas ? Qui tu es, toi ?"'
...
label morte1_s2: # from 1.0 3.0 4.0
    nr '"Tu ne te souviens pas de ton *nom* ? Hé ben ! La PROCHAINE fois que tu passes la nuit dans ce bled, vas-y doucement sur la bibe. J„m“appelle Morte. Moi aussi, j„suis piégé ici."'
...
label morte1_s6: # from 5.0
    ...
    menu:
        '"„La Morgue“ ? Qu„est-ce… suis-je mort ?"':
            # a8 # r39808
            jump morte1_s7
...
label morte1_s7: # from 6.0
    ...
    menu:
        '"Des cicatrices ? Profondes comment ?"':
            # a9 # r39810
            jump morte1_s8
...
label morte1_s8: # from 7.0
    nr '"Ben… les trucs gravés sur ta poitrine ne sont pas TROP moches… mais ceux que tu as dans le dos…" Morte s„interrompt. "En gros, on dirait que tu as une galerie entière de tatouages sur le dos, chef. Comme si ça voulait dire quelque chose…"'

    menu:
        '"Des tatouages dans le dos ? Qu„est-ce qu“ils disent ?"':
            # a10 # r39812
            jump morte1_s9
...
label morte1_s10: # from 9.0
    ...
    menu:
        '"Je comprends maintenant pourquoi j„ai mal au dos ; il y a un bon sang d“roman écrit dessus. Et ce journal que je suis supposé avoir sur moi… est-ce que j„en avais un quand tu m“as vu allongé là ?"':
            # a12 # r39816
            jump morte1_s11
...
label morte1_s11: # from 10.0
    nr '"Non… t„étais nu comme un ver quand t“es arrivé ici. De toute façon, t„as déjà un journal écrit sur le corps."'

    menu:
        '"Et Pharod ? Tu le connais ?"':
            # a13 # r39818
            jump morte1_s12
...
label morte1_s12: # from 11.0
    nr '"Ça m„dit rien… mais ceci dit, j“connais pas grand monde. Enfin, y a bien un bige qui doit savoir où trouver Pharod… euh, une fois qu„on sera sorti d“ici, j„veux dire."'

    menu:
        '"Et comment on va *faire* pour sortir d„ici ?"':
            # a14 # r39820
            jump morte1_s13
...
label morte1_s13: # from 12.0
    nr '"Bon, toutes les portes sont fermées, nous avons donc besoin de la clé. Il y a des chances qu„un des cadavres errants ici l“ait."'
...
label morte1_s14: # from 13.0
    nr '"Ouais, les gardiens de la Morgue utilisent les corps morts comme main-d„œuvre bon marché. Les cadavres sont aussi muets que des pierres, ils sont inoffensifs et n“attaqueront que si tu les provoques."'
...
label morte1_s15: # from 14.0
    nr '"Quoi, tu penses que ça va les vexer ? Ils sont MORTS. Mais le point positif, c„est que si tu les tues, ils pourront au moins se reposer avant que leurs gardiens les ressuscitent pour les remettre au travail."'
...
label morte1_s16: # from 14.1 15.0
    nr '"Bon, avant de faire ça, arme-toi. Je crois avoir vu un scalpel sur une des étagères par ici."  ^NREMARQUE : Fouille les étagères dans la pièce et trouve une arme pour attaquer les zombis. <SEARCH_WEAPON>^-'
...
label morte1_s18: # - # IF WEIGHT #2 /* Triggers after states #: 26 even though they appear after this state */ ~  Global("Mortuary_Walkthrough","GLOBAL",0) !PartyHasItem("Scalpel") Global("ZM782_Dead_KAPUTZ","GLOBAL",0)
    nr '"Il devrait y avoir un scalpel sur l„une de ces étagères. Je le trouverais avant d“en découdre avec un de ces cadavres."  ^NREMARQUE : Fouille les étagères dans la pièce et trouve une arme pour attaquer les zombis. <SEARCH_WEAPON>^-'
...
label morte1_s21: # from 20.0
    ...
    menu:
        '"Quand je vais attaquer ce cadavre, tu as intérêt à être là, sinon c„est toi qui vas recevoir le coup de scalpel suivant."':
            # a26 # r39841
            jump morte1_s22
...
label morte1_s23: # from 19.1 20.1 21.1 22.0
    nr '"Le moment est venu de renvoyer ces cadavres dans l„au-delà…"  ^NREMARQUE : <ATTACKNOTE>^-'
...
label morte1_s24: # - # IF WEIGHT #4 /* Triggers after states #: 26 even though they appear after this state */ ~  Global("Mortuary_Walkthrough","GLOBAL",0) !PartyHasItem("KeyPr") Global("ZM782_Dead_KAPUTZ","GLOBAL",1)
    nr '"D„accord, on dirait que tu t“es occupé du bon cadavre. Maintenant il faut que tu trouves la clé. Elle aurait dû être sur lui. Une fois qu„on l“aura, on pourra sortir d„ici."  ^NREMARQUE : <DEADPILE>^-'
...
label morte1_s28: # from 27.0 27.2
    nr '"Est-ce que tu essaies de me vexer, chef ? Qu„est-ce qu“il y a, c„est quelque chose que j“ai dit ? Le fait que je n„aie pas de bras ? Quoi ?"'
...
label morte1_s30: # - # IF WEIGHT #6 ~  Global("Mortuary_Walkthrough","GLOBAL",1)
    ...
    menu:
        '"Rien pour le moment, Morte. Je voulais juste savoir si je pouvais toujours compter sur toi."':
            # a40 # r39864
            jump morte1_dispose
...
label morte1_s31: # externs zm825_s3 zm825_s0 zm569_s3 zm569_s0
    nr '"Euh, chef… Ils ne peuvent pas t„entendre, d“accord ? Ils sont morts."'

    menu:
        '"Mais toi aussi tu es mort. Et ça ne t„empêche pas de me parler."':
            # a41 # r42299
            jump morte1_s32
...
label morte1_s32: # from 31.0
    nr '"Ouais, mais *je suis* spécial. Ce n„est pas la mort qui pourrait m“ôter mon goût de vivre. Ces cadavres, là…" Morte roule les yeux. "Ils ne devaient déjà pas avoir beaucoup de personnalité à l„origine."'
...
label morte1_s33: # from 32.0
    nr '"Écoute, chef… Te regarder essayer de parler avec ces cadavres, c„est pas très bon pour mon moral. Laissons ça aux azimutés, d“accord ?"'
...
label morte1_s34: # externs zm782_s0
    nr '"On dirait que c„est le suppliant chanceux, chef. Regarde… Il tient la clé dans sa main."'
