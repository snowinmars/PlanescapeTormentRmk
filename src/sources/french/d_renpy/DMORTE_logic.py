class MorteLogic:
    ...
    def j64512_s85_r4676_action(self):
        ...
        #$% .register('64512', 'Apparemment, l'homme déguisé en zombi fait partie de la faction des Anarchistes, qui lutte contre toutes les formes d'autorité et de structure sociale.') %$#
    ...
    def r20612_action(self):
        ...
        #$%.register('20538', 'Marta la Couturière semble chargée de récupérer les cadavres qui lui sont donnés, de leur arracher les dents, de découdre leurs points de suture et de chercher tous les objets de valeur susceptibles de se trouver sur ou dans les corps, pour ensuite les recoudre. %$#')
    ...
    def r24932_action(self):
        ...
        #$%.register('24933', 'J'ai commencé à écrire mes expériences pour y voir plus clair et dans l'espoir de comprendre ce qui se passait. Je me suis réveillé dans un endroit que l'on appelle la "Morgue"… C'est effectivement une sorte de morgue. Je ne sais pas qui je suis, ni ce que je fais ici, ni comment je suis arrivé. La première chose que j'ai découverte en me réveillant, c'est un crâne bavard du nom de Morte. Il m'a dit que nous étions coincés là, et que l'un des cadavres qui traînaient dans la salle devait avoir la clé. Il m'a proposé de fouiller les étagères de la salle pour trouver un scalpel, puis de le plonger dans l'un des corps jusqu'à ce qu'il "meure". %$#')
    ...
    def r41921_action(self):
        ...
        #$%.register('67862', 'À la Salle des Fêtes, j'ai rencontré Méli Sensdumeurtre, l'homme qui avait maudit Pestilent. %$#')
    ...
    def r43910_action(self):
        ...
        #$%.register('39490', 'J'ai dit à Nemelle que son amie Aelwyn la cherchait. Elle m'a remercié d'un baiser magique, qui m'a fait gagner trois points de vie supplémentaires. %$#')
    ...
    def j39477_s478_r45093_action(self):
        ...
        #$% .register('39477', 'Morte m'a vaguement parlé de la 'Règle des Trois'. Apparemment, certaines personnes pensent que les événements s'enchaînent toujours par trois. Elles accordent donc une grande importance à ce chiffre.') %$#
    ...
    def r53629_action(self):
        ...
        #$%.register('53633', 'Morte m'a dit que c'est moi qui l'avait arraché au Pilier des Crânes, il y a bien longtemps de cela, mais il ignore comment j'ai fait. Il m'a également expliqué que tout le savoir du Pilier l'avait quitté à partir du moment où il en avait été séparé… ce qui expliquerait pourquoi il n'est pas tout à fait aussi utile (!) que ce qu'il avait promis à mon incarnation passée. %$#')
    ...
    def r53630_action(self):
        ...
        #$%.register('53661', 'Il semblerait que Morte soit issu du Pilier des Crânes, colonne de têtes vivantes existant sur l'Averne, le premier niveau de Baator. Il est constitué des têtes de ceux dont les mensonges ont mené d'autres gens à leur perte. %$#')
    ...
    def r65569_action(self):
        ...
        #$%.register('65573', 'J'ai interrogé Morte au sujet de la phrase qu'il a 'oublié' de lire sur mon dos. Il m'a dit qu'elle n'avait pas d'importance, et que c'était pour cette raison qu'il ne me l'avait pas lue. Je suis sûr qu'il me ment, mais j'ignore pourquoi. %$#')
    ...
    def r65621_action(self):
        ...
        #$%.register('65625', 'Morte m'a dit qu'il possédait le 'pouvoir' de mettre les gens en colère ; il l'appelle sa 'Litanie d'injures'. Apparemment, il a mémorisé tellement d'insultes ou cours de sa vie et de sa mort qu'il sait comment provoquer n'importe qui. Les individus qui se jettent sur lui en étant fous de rage sont moins précis et plus faciles à toucher ; de plus, ils ne font rien d'autre que courir après Morte et tenter de l'aplatir à mains nues. C'est le genre de pouvoir qui peut s'avérer très utile contre les jeteurs de sorts. De plus, n'étant pas magique, il affecte normalement les créatures dotées de résistance à la magie. J'imagine que le 'don' de Morte ne peut que s'améliorer encore quand il entend de nouvelles injures. %$#')


    def j65637_s667_r65631_action(self):
        self.state_manager.journal_manager.update_journal('65637')
        #$% .register('65637', 'Morte m'a dit qu'il avait acquis une nouvelle capacité alors qu'il se trouvait sur l'étagère de Lothar ; il lui donne le nom de 'Gang de crânes'. Apparemment, il est capable, un certain nombre de fois par jour, d'appeler les amis qu'il s'est faits chez Lothar, et ceux-ci se jettent alors sur l'adversaire de son choix pour le mordre. Les dégâts qu'ils infligent dépendent du niveau de Morte.') %$#


    def j65669_s675_r65666_action(self):
        self.state_manager.journal_manager.update_journal('65669')
        #$% .register('65669', 'Morte m'a conseillé de me rendre à la Salle des Fêtes du Quartier des Gratte-Papier pour en apprendre davantage au sujet de Ravel, la guenaude noire.') %$#


    def j65678_s677_r65674_action(self):
        self.state_manager.journal_manager.update_journal('65678')
        #$% .register('65678', 'Morte m'a dit qu'un habitant du Quartier des Gratte-Papiers saurait peut-être où trouver un morceau de Ravel. Si cela échoue, nous pourrons toujours essayer la Salle des Fêtes et ses pierres sensorielles. Et si nous devions faire chou blanc une seconde fois, nous pourrions aller interroger les prostituées… c'est franchement plus agréable que d'arrêter tous les passants pour leur poser des questions.') %$#


    def j65712_s686_r65710_action(self):
        self.state_manager.journal_manager.update_journal('65712')
        #$% .register('65712', 'J'ai parlé à Morte du fait qu'il avait prétendu être un mimir, mais je n'avais rien de précis à lui reprocher. Il va me falloir le prendre à mentir si je veux lui faire dire la vérité.') %$#
    ...
    def r65821_action(self):
        ...
        #$%.register('65825', 'J'ai découvert pourquoi Morte me suit depuis tout ce temps. C'est parce qu'il se sent coupable de m'avoir tué une fois, du temps où il était toujours vivant. Nous en avons parlé et il a eu l'air soulagé. J'ai l'impression qu'il est plus fort désormais. %$#')
