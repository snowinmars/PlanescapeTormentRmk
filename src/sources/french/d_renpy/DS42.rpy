...
label s42_s0: # - # IF ~  True()
    ...
    menu:
        ...
        '"Excuse-moi, tu n„aurais pas vu de squelettes errants dans le coin ?"{#s42_s0_r6613}':
            # a1 # r6613
            $ s42Logic.r6613_action()
            jump s42_s1

        '"Il faut que je te demande : pourquoi ce sarrau ? Je veux dire, ce n„est pas comme si tu avais quelque chose à cacher."{#s42_s0_r6614}' if s42Logic.r6614_condition():
            # a2 # r6614
            $ s42Logic.r6614_action()
            jump s42_s1

        '"Il faut que je te demande : pourquoi ce sarrau ? Je veux dire, ce n„est pas comme si tu avais quelque chose à cacher."{#s42_s0_r6615}' if s42Logic.r6615_condition():
            # a3 # r6615
            jump s42_s1

        'Utilise Histoires-Os-Conter sur le cadavre.{#s42_s0_r6616}' if s42Logic.r6616_condition():
            # a4 # r6616
            jump s42_s2

        'Examine attentivement le squelette.{#s42_s0_r6617}':
            # a5 # r6617
            $ s42Logic.r6617_action()
            jump s42_s3

        'Essaie de déboulonner les articulations du squelette.{#s42_s0_r6618}' if s42Logic.r6618_condition():
            # a6 # r6618
            $ s42Logic.r6618_action()
            jump morte_s110  # EXTERN

        'Essaie de déboulonner les articulations du squelette.{#s42_s0_r6619}' if s42Logic.r6619_condition():
            # a7 # r6619
            jump s42_s6

        'Essaie de déboulonner les articulations du squelette.{#s42_s0_r6620}' if s42Logic.r6620_condition():
            # a8 # r6620
            jump s42_s6

        '"Qu„est-ce que tu dis de ce squelette, Morte ? Ça t“ira comme corps ?"{#s42_s0_r6621}' if s42Logic.r6621_condition():
            # a9 # r6621
            jump s42_s1

        'Laisse le squelette tranquille.{#s42_s0_r6622}' if s42Logic.r6622_condition():
            # a10 # r6622
            jump morte_s111  # EXTERN

        'Laisse le squelette tranquille.{#s42_s0_r6623}' if s42Logic.r6623_condition():
            # a11 # r6623
            jump s42_dispose

        'Laisse le squelette tranquille.{#s42_s0_r6624}' if s42Logic.r6624_condition():
            # a12 # r6624
            jump s42_dispose
...
label s42_s1: # from 0.0 0.1 0.2 0.3 0.9 3.0 3.3
    ...
    menu:
        ...
        'Imite le geste… Attends de voir ce qui se passe.{#s42_s1_r6626}' if s42Logic.r6626_condition():
            # a14 # r6626
            jump s42_s9
        ...
        'Laisse le squelette tranquille.{#s42_s1_r6628}':
            # a16 # r6628
            jump s42_dispose
...
label s42_s2: # from 0.4
    ...
    menu:
        ...
        'Laisse le squelette tranquille.{#s42_s2_r6630}' if s42Logic.r6630_condition():
            # a18 # r6630
            jump s42_dispose

        'Laisse le squelette tranquille.{#s42_s2_r6631}' if s42Logic.r6631_condition():
            # a19 # r6631
            jump s42_dispose
...
label s42_s3: # from 0.5 10.2
    ...
    menu:
        ...
        'Essaie de déboulonner les articulations du squelette.{#s42_s3_r6632}' if s42Logic.r6632_condition():
            # a21 # r6632
            $ s42Logic.r6632_action()
            jump morte_s110  # EXTERN

        'Essaie de déboulonner les articulations du squelette.{#s42_s3_r6633}' if s42Logic.r6633_condition():
            # a22 # r6633
            jump s42_s6

        '"Je peux emprunter quelques sangles et boulons ?"{#s42_s3_r6634}' if s42Logic.r6634_condition():
            # a23 # r6634
            jump s42_s1

        'Laisse le squelette tranquille.{#s42_s3_r6635}' if s42Logic.r6635_condition():
            # a24 # r6635
            $ s42Logic.r6635_action()
            jump morte_s111  # EXTERN

        'Laisse le squelette tranquille.{#s42_s3_r6636}' if s42Logic.r6636_condition():
            # a25 # r6636
            jump s42_dispose

        'Laisse le squelette tranquille.{#s42_s3_r6637}' if s42Logic.r6637_condition():
            # a26 # r6637
            jump s42_dispose
...
label s42_s4: # from 1.0 12.0
    ...
    menu:
        ...
        'Laisse le squelette tranquille.{#s42_s4_r6639}':
            # a28 # r6639
            jump s42_dispose
...
label s42_s5: # from 4.0 9.0
    ...
    menu:
        ...
        'Laisse le squelette tranquille.{#s42_s5_r6641}':
            # a30 # r6641
            jump s42_dispose
...
label s42_s7: # from 5.0
    ...
    menu:
        ...
        'Examine l„objet.{#s42_s7_r6644}' if s42Logic.r6644_condition():
            # a33 # r6644
            jump s42_s8
...
label s42_s9: # from 1.1 12.1
    ...
    menu:
        ...
        'Laisse le squelette tranquille.{#s42_s9_r6647}':
            # a36 # r6647
            jump s42_dispose
...
label s42_s10: # from 1.2 12.2
    ...
    menu:
        ...
        '"Euh… bonjour ?"{#s42_s10_r6649}' if s42Logic.r6649_condition():
            # a38 # r6649
            jump s42_s13

        'Examine attentivement le squelette.{#s42_s10_r6650}':
            # a39 # r6650
            $ s42Logic.r6650_action()
            jump s42_s3

        'Laisse le squelette tranquille.{#s42_s10_r6651}':
            # a40 # r6651
            jump s42_dispose
...
label s42_s12: # from 10.0
    ...
    menu:
        ...
        'Imite le geste… Attends de voir ce qui se passe.{#s42_s12_r6654}' if s42Logic.r6654_condition():
            # a43 # r6654
            jump s42_s9
        ...
        'Laisse le squelette tranquille.{#s42_s12_r6656}':
            # a45 # r6656
            jump s42_dispose
