...
label s863_s0: # from 10.0 # IF ~  !HasItem("DRemind","S863")
    ...
    menu:
        ...
        '"Désolé d„avoir pris ce parchemin, mais tu n“aurais sans doute pas pu l„apporter avant quelque temps."{#s863_s0_r35561}' if s863Logic.r35561_condition():
            # a1 # r35561
            jump s863_s1

        '"Il faut que je te demande : ces os brisés, c„est le résultat d“un combat ou d„une chute ?"{#s863_s0_r35562}' if s863Logic.r35562_condition():
            # a2 # r35562
            $ s863Logic.r35562_action()
            jump s863_s1

        '"Il faut que je te demande : ces os brisés, c„est le résultat d“un combat ou d„une chute ?"{#s863_s0_r35563}' if s863Logic.r35563_condition():
            # a3 # r35563
            jump s863_s1

        'Utilise Histoires-Os-Conter sur le squelette.{#s863_s0_r35564}' if s863Logic.r35564_condition():
            # a4 # r35564
            jump s863_s2

        'Examine attentivement le squelette.{#s863_s0_r35569}':
            # a5 # r35569
            $ s863Logic.r35569_action()
            jump s863_s3

        'Essaie de déboulonner les articulations du squelette.{#s863_s0_r35602}' if s863Logic.r35602_condition():
            # a6 # r35602
            $ s863Logic.r35602_action()
            jump morte_s400  # EXTERN

        'Essaie de déboulonner les articulations du squelette.{#s863_s0_r35603}' if s863Logic.r35603_condition():
            # a7 # r35603
            jump s863_s4

        'Essaie de déboulonner les articulations du squelette.{#s863_s0_r35604}' if s863Logic.r35604_condition():
            # a8 # r35604
            jump s863_s5

        'Essaie de déboulonner les articulations du squelette.{#s863_s0_r35605}' if s863Logic.r35605_condition():
            # a9 # r35605
            jump s863_s6

        'Essaie de déboulonner les articulations du squelette.{#s863_s0_r35606}' if s863Logic.r35606_condition():
            # a10 # r35606
            jump s863_s4

        'Essaie de déboulonner les articulations du squelette.{#s863_s0_r35607}' if s863Logic.r35607_condition():
            # a11 # r35607
            jump s863_s5

        'Essaie de déboulonner les articulations du squelette.{#s863_s0_r35608}' if s863Logic.r35608_condition():
            # a12 # r35608
            jump s863_s6

        '"Qu„est-ce que tu dis de ce squelette, Morte ? Ça t“ira comme corps ?"{#s863_s0_r35609}' if s863Logic.r35609_condition():
            # a13 # r35609
            jump morte_s396  # EXTERN

        'Laisse le squelette tranquille.{#s863_s0_r35610}' if s863Logic.r35610_condition():
            # a14 # r35610
            $ s863Logic.r35610_action()
            jump morte_s394  # EXTERN

        'Laisse le squelette tranquille.{#s863_s0_r35611}' if s863Logic.r35611_condition():
            # a15 # r35611
            jump s863_dispose

        'Laisse le squelette tranquille.{#s863_s0_r35612}' if s863Logic.r35612_condition():
            # a16 # r35612
            jump s863_dispose
...
label s863_s1: # from 0.0 0.1 0.2 0.3
    ...
    menu:
        ...
        '"C„était sympa de parler avec toi, Sac d“os. Prends soin de toi."{#s863_s1_r35559}' if s863Logic.r35559_condition():
            # a18 # r35559
            jump s863_dispose

        '"C„était sympa de parler avec toi, Sac d“os. Prends soin de toi."{#s863_s1_r35560}' if s863Logic.r35560_condition():
            # a19 # r35560
            jump s863_dispose
...
label s863_s2: # from 0.4
    ...
    menu:
        ...
        'Laisse le squelette tranquille.{#s863_s2_r35567}' if s863Logic.r35567_condition():
            # a21 # r35567
            jump s863_dispose

        'Laisse le squelette tranquille.{#s863_s2_r35568}' if s863Logic.r35568_condition():
            # a22 # r35568
            jump s863_dispose
...
label s863_s3: # from 0.5
    ...
    menu:
        ...
        'Essaie de déboulonner les articulations du squelette.{#s863_s3_r35593}' if s863Logic.r35593_condition():
            # a24 # r35593
            jump s863_s4

        'Essaie de déboulonner les articulations du squelette.{#s863_s3_r35594}' if s863Logic.r35594_condition():
            # a25 # r35594
            jump s863_s5

        'Essaie de déboulonner les articulations du squelette.{#s863_s3_r35595}' if s863Logic.r35595_condition():
            # a26 # r35595
            jump s863_s6

        '"Je peux emprunter quelques sangles et boulons ?"{#s863_s3_r35596}' if s863Logic.r35596_condition():
            # a27 # r35596
            jump s863_s4

        '"Je peux emprunter quelques sangles et boulons ?"{#s863_s3_r35597}' if s863Logic.r35597_condition():
            # a28 # r35597
            jump s863_s5

        '"Je peux emprunter quelques sangles et boulons ?"{#s863_s3_r35598}' if s863Logic.r35598_condition():
            # a29 # r35598
            jump s863_s6

        'Laisse le squelette tranquille.{#s863_s3_r35599}' if s863Logic.r35599_condition():
            # a30 # r35599
            $ s863Logic.r35599_action()
            jump morte_s394  # EXTERN

        'Laisse le squelette tranquille.{#s863_s3_r35600}' if s863Logic.r35600_condition():
            # a31 # r35600
            jump s863_dispose

        'Laisse le squelette tranquille.{#s863_s3_r35601}' if s863Logic.r35601_condition():
            # a32 # r35601
            jump s863_dispose
...
label s863_s4: # from 0.7 0.10 3.1 3.4
    ...
    menu:
        ...
        '"Peut-être qu„avec l“outil adéquat, je pourrais les enlever… Hmmmm. Je vais peut-être revenir, Sac d„os."{#s863_s4_r35578}' if s863Logic.r35578_condition():
            # a34 # r35578
            jump s863_dispose

        '"Peut-être qu„avec l“outil adéquat, je pourrais les enlever… Hmmmm. Je vais peut-être revenir, Sac d„os."{#s863_s4_r35579}' if s863Logic.r35579_condition():
            # a35 # r35579
            jump s863_dispose

        'Laisse le squelette tranquille.{#s863_s4_r35580}' if s863Logic.r35580_condition():
            # a36 # r35580
            $ s863Logic.r35580_action()
            jump morte_s394  # EXTERN

        'Laisse le squelette tranquille.{#s863_s4_r35581}' if s863Logic.r35581_condition():
            # a37 # r35581
            jump s863_dispose

        'Laisse le squelette tranquille.{#s863_s4_r35582}' if s863Logic.r35582_condition():
            # a38 # r35582
            jump s863_dispose
...
label s863_s8: # - # IF ~  HasItem("DRemind","S863")
    ...
    menu:
        ...
        'Laisse le squelette tranquille.{#s863_s8_r64264}':
            # a42 # r64264
            jump s863_dispose
...
label s863_s10: # from 9.0
    ...
    menu:
        ...
        'Laisse le squelette tranquille.{#s863_s10_r64269}':
            # a45 # r64269
            jump s863_dispose
