...
label giantsk_s0: # - # IF ~  True()
    nr '당신 앞에는 화려한 청동 갑옷을 입은 자이언트 스켈레톤이 서 있다. 갑옷은 스켈레톤의 골격에 볼트로 고정되어 있으며, 그 가슴받이의 표면 전체에 걸쳐 일련의 정교한 심벌들이 새겨져 있다. 당신은 그 뼈들의 출처가 궁금하다. 당신이 알기로 그토록 거대한 골격을 지닌 인간은 존재하지 않기 때문이다. 그것이 손에 들고 있는 거대한 칼의 무게는 마차 한 대의 무게에 필적할 것처럼 보인다.{#giantsk_s0_1}'
...
label giantsk_s1: # from 0.0 0.1 0.9 # IF ~  False()
    nr '스켈레톤은 당신 질문에 대답하기에는 너무 오래 전에 죽은 듯하다. 아니면 그의 머리는 당신 얘기를 듣기에는 너무 높은 곳에 있든지.{#giantsk_s1_1}'
...
label giantsk_s2: # from 0.4 0.6 1.2 1.4 3.0 4.1 4.3 5.3 5.5 6.2 6.4 7.3 7.5 8.2 8.4 9.4 9.6
    nr '당신이 스켈레톤을 만지자 철제 종소리가 시체안치소 안에 울려 퍼지기 시작한다… 그리고 스켈레톤은 번개같은 속도로 깨어나, 칼을 들어 공격한다!{#giantsk_s2_1}'
...
label giantsk_s3: # from 0.5 0.7 1.3 1.5 4.2 4.4 5.4 5.6 6.3 6.5 7.4 7.6 8.3 8.5 9.5 9.7
    nr '그렇게 하려던 순간 당신은 갑자기 멈춘다… 그리고 당신의 시선은 스켈레톤의 갑옷으로 쏠린다. 그것의 가슴받이에 새겨진 심벌들에는 당신으로 하여금 멈추고 생각하게 만드는 무엇인가가 있다. 만약 이 스켈레톤들이 수호자라면, 그들을 건드릴 경우… 그들이 깨어날지도 모른다.{#giantsk_s3_1}'
...
label giantsk_s4: # from 0.2 1.0 3.1 7.1 15.1 16.3
    nr '스켈레톤의 정교한 청동 갑옷은 스켈레톤의 흉곽과 어깨뼈에 일련의 철제 볼트들로 고정되어 있다. 갑옷 안의 골격을 살펴보던 중 당신은 전술한 철제 볼트와 동일한 볼트가 스켈레톤의 어깨, 팔꿈치, 골반, 그리고 무릎 관절에도 박혀 있다는 사실을 발견한다. 그리고 스켈레톤의 팔과 다리에는 다량의 두꺼운 가죽끈과 마디가 있는 로프가 마치 근육 및 건과 흡사한 형태로 감겨져 있다.{#giantsk_s4_1}'
...
label giantsk_s5: # from 4.0
    nr '갑옷은 오래된 것임에도 불구하고 그 보존 상태가 양호하다. 그것은 환히 반짝이고 있으며, 가슴받이에 새겨진 심벌들은 불빛 아래서 마치 흐르는 것처럼 움직여, 당신이 그것들에게 집중을 하려고 할 때마다 약간씩 그 형체가 변화한다.{#giantsk_s5_1}'
...
label giantsk_s6: # from 5.0 5.1
    nr '심벌들을 바라보면서 당신은 거의 무의식적으로 시선의 긴장이 풀리도록 한다. 잠시 후 심벌들은 변화하는 것을 멈추고, 가슴받이를 아래위로 왕복하는 룬들의 패턴이 모습을 드러낸다. 기묘하게도 서로 연결된 룬들의 패턴은 당신에게 사슬을 연상시킨다… 그리고 당신은 그 생각으로부터 이 룬들은 일종의 수호 마법이란 사실을 갑자기 기억해낸다.{#giantsk_s6_1}'
...
label giantsk_s7: # from 5.2 6.1 7.2
    nr '당신은 한동안 룬들을 연구하지만 그 마법을 해독할 수가 없다. 그것은 매우 복잡한 듯하며 당신은 집중하는 데 어려움을 겪고 있다.{#giantsk_s7_1}'
...
label giantsk_s8: # from 6.0
    nr '당신은 룬들이 가슴받이 표면 전체에 걸쳐 전개되는 패턴을 연구한다. 가장 기본적인 레벨의 룬들은 하급 갑옷의 주문이나, 갑옷의 가장자리 부분에 있는 여러 개의 해골 모양의 룬들과 원형 기호들은 당신으로 하여금 갑옷에는 여러 종류의 상급 사령술과 상급 보호 마법 역시 걸려 있는 것이 아닌가 하고 생각하게 만든다. 스켈레톤을 만지면 그것이 깨어나… 자신을 방어할 가능성이 높다.{#giantsk_s8_1}'
...
label giantsk_s9: # from 0.3 1.1 8.0 8.1
    nr '가슴받이에 새겨진 룬들의 패턴을 훼손하면 마법을 깨트릴 수 있다는 생각이 드나, 쉽지는 않을 듯하다… 패턴은 복잡하며, 엉뚱한 부분을 훼손할 경우 스켈레톤이 깨어날 수도 있다.{#giantsk_s9_1}'
...
label giantsk_s10: # from 9.1 16.0
    nr '당신이 가슴받이를 장식하는 룬들을 훼손하기 시작하자 철제 종소리가 시체안치소 안에 울려 퍼지기 시작한다… 그리고 스켈레톤은 번개같은 속도로 깨어나, 칼을 들어 공격한다!{#giantsk_s10_1}'
...
label giantsk_s11: # from 9.2 16.1
    nr '그 작업은 처음에는 고통스러울 정도로 어려우나. 서서히 당신은 일에 정신을 집중할 수 있게 되며, 룬들은 당신의 공세 앞에 무릎을 꿇기 시작한다. 몇 분 안에 자이언트 스켈레톤에 걸려 있던 모든 마법은 제거된다. 그것은 무너지면서 굉음과 함께 바닥에 쓰러진다!{#giantsk_s11_1}'
...
label giantsk_s12: # from 11.0
    nr '당신은 잠시 기다린다, 하지만 다행히 아무도 그 소리에 반응하지 않는다. 당신은 빠른 동작으로 바닥에 널린 스켈레톤의 잔해들을 살펴본다. 그것들은 대부분 쓸모가 있기에는 너무 낡았거나 무겁다. 하지만 당신은 그 중에서 스켈레톤의 가슴받이의 일부분을 발견하는데, 그것에는 깨어진 마법들 중 하나의 내용 대부분이 새겨져 있다. 당신에게는 그것이 쓸모가 있을 거라는 생각이 든다.{#giantsk_s12_1}'
...
label giantsk_s13: # from 9.3 16.2
    nr '이번에는 마법을 해제하는 일이 한층 쉽다, 그리고 룬들은 당신의 공세 앞에 금새 무릎을 꿇는다. 불과 몇 분 안에 자이언트 스켈레톤에 걸려 있던 모든 마법은 제거된다. 이전에 무슨 일이 일어났는지를 기억하고 있는 당신은 스켈레톤이 쓰러지기 전에 그것을 잡은 다음, 툴툴거리며 천천히 바닥에 내려놓는다.{#giantsk_s13_1}'
...
label giantsk_s14: # from 13.0
    nr '당신은 재빠르게 스켈레톤의 잔해를 뒤지기 시작한다, 그리고 이번에도 가슴받이의 일부를 발견한다… 첫 번째 것과 마찬가지로 이것에도 깨어진 마법의 일부가 새겨져 있다. 그것은 쓸모가 있을지도 모른다.{#giantsk_s14_1}'
...
label giantsk_s15: # from 7.0
    nr '당신이 책에 있는 그림들과 가슴받이 표면에 새겨진 심벌들을 비교한다. 가장 기본적인 레벨의 룬들은 하급 갑옷의 주문이나, 갑옷의 가장자리 부분에 있는 여러 개의 해골 모양의 룬들과 원형 기호들은 당신으로 하여금 갑옷에는 여러 종류의 상급 사령술과 상급 보호 마법 역시 걸려 있는 것이 아닌가 하고 생각하게 만든다. 스켈레톤을 만지면 그것이 깨어나… 자신을 방어할 가능성이 높다.{#giantsk_s15_1}'
...
label giantsk_s16: # from 9.0 15.0
    nr '당신이 책으로부터 이해한 바에 따르면, 갑옷의 마법은 가슴받이에만 걸려 있고, 스켈레톤이 다시 일어나 움직이게 하는 것은 사령술이나, 그것에게 주변 환경에 대한 기본적인 인지 능력을 부여하고 있는 것은 보호의 마법이다. 만약 당신이 스켈레톤의 룬을 훼손하면 그 행위가 스켈레톤에 의해 공격으로 간주될 것이다… 하지만 사전에 그것이 당신의 존재를 인지할 수 없게 한다면…{#giantsk_s16_1}'
