...
label morte2_s0: # - # IF WEIGHT #0 ~  Global("Mortuary_Walkthrough","GLOBAL",1) InParty("Morte")
    nr '"잠깐… 충고를 하겠어, 대장. 여기서부터는 조용히 움직이자고. 필요 이상으로 시체들을 훼손할 필요는 없어… 특히 여자 시체들은. 또한 그들을 „죽이면“ 관리자들을 여기로 불러오는 결과를 야기할 수도 있어."{#morte2_s0_}'
...
label morte2_s1: # from 0.0 3.0 7.0
    nr '"그들은 자신들을 „더스트맨“이라고 부르고 있어. 그들을 알아보는 건 쉽지. 예외없이 검은 법복을 입고 있는데다가 꼭 사후경직을 일으킨 것 같은 얼굴들을 하고 있으니까. 그들은 악귀같은 죽음 숭배자들이야. 그들은 모두 다 죽어야 한다고 믿고 있어… 그것도 빠르면 빠를수록 좋다고."{#morte2_s1_}'
...
label morte2_s2: # from 1.0
    nr '"내가 말할 때 뭘 하고 있었어?! 더스트맨은 모두가 다 죽어야 하며, 그것도 빠르면 빠를수록 좋다고 믿고 있다고 내가 이미 말했잖아. 대장이 보기엔 지금까지 우리가 본 시체들이 죽어서 더 행복해진 것 같아?"{#morte2_s2_}'
...
label morte2_s3: # from 0.1 2.0 7.1
    nr '"다원 우주에서는 매일 누군가 반드시 죽어, 대장. 이 비틀거리는 얼간이들은 이 곳의 관리자들에게 사후의 시체에 대한 권리를 판 한심한 작자들의 잔해라고."{#morte2_s3_}'
...
label morte2_s4: # from 0.2 2.1 3.1
    nr '"뭐- *진담*이야? 이봐, 대장, 이 죽은 아가씨들은 우리들처럼 산전수전 다 겪은 친구들에게 주어진 마지막 기회라고. 우리는 *기사도* 정신을 발휘해야 해… 열쇠를 찾으려고 그녀들을 칼로 베거나 사지를 절단하는 짓 따위는 해서는 안돼."{#morte2_s4_}'
...
label morte2_s5: # from 4.0
    nr '"대장, 그들은 죽었어, 그리고 우리들도 죽었어… 내가 무슨 말을 하려는지 알겠어? 그래?"{#morte2_s5_}'
...
label morte2_s6: # from 5.0 5.1
    nr '"대장, 우린 이미 이 절뚝거리는 아가씨들과 대화를 시작할 화제거리도 가지고 있다고. 우리들은 *모두* 적어도 한 번씩은 죽어 봤어. 그러니 우린 공통의 화제가 있는 셈이지. 그들은 우리처럼 죽은 경험이 있는 사내들을 환영할 거라고."{#morte2_s6_}'
...
label morte2_s7: # from 6.0
    nr '"그래… 좋아. *대장*은 죽지 않았을지도 모르지만 *난* 죽었어. 그리고 나라면 여기 있는 체격 좋은 멋진 시체 아가씨들과 기꺼이 관을 함께 쓰겠어." 모트는 기대에 부푼 듯 이빨을 딱딱거리기 시작한다. "물론 그러기 위해서는 우선 관리인들이 그들을 포기해야 하는데 그럴 가능성은 희박하지…"{#morte2_s7_}'
...
label morte2_s8: # from 0.3 2.2 3.2 7.2 12.7 13.2 14.2 15.2 16.2 17.1 18.1 19.2 20.1 21.1 22.1
    nr '"이봐, 대장. 대장은 죽음과 키스를 한 탓에 아직도 정신이 좀 오락가락한 것 같아. 그러니 내가 두 가지 충고를 하겠어. 첫째, 질문할 게 있으면 내게 *물어.* 알겠어?"   ^NNOTE: <SPEAKTO>^-{#morte2_s8_}'
...
label morte2_s9: # from 8.0
    nr '"둘째, 대장이 겉보기의 *반*만큼이라도 건망증이 심하다면, 뭐든지 기록을 하는 습관을 기르라고. 조금이라도 중요할 것 같은 얘기를 듣게 되면 잊지 않도록 즉각 기록을 해."{#morte2_s9_}'
...
label morte2_s10: # from 9.0
    nr '"그럼 새 일지를 쓰기 시작하라고. 그 따윈 별 문제도 아냐. 여긴 대장이 평생 쓰고 남을 정도의 양피지와 잉크가 있으니까."{#morte2_s10_}'
...
label morte2_s11: # from 10.0
    nr '"그것을 써서 대장의 여정을 파악하라고. 만약 대장이 중요한 사안들, 예들 들어 대장이 누구인가.,.. 또는 더 중요하게 *내*가 누구인가에 대해 가물가물하게 되면 일지를 읽어 기억을 상기시키라고. 알겠어?"  ^NNOTE: 일지를 보려면 퀵 메뉴에 있는 일지 버튼을 누른다. 일지는 게임 진행 동안 자동으로 갱신된다.^-{#morte2_s11_}'
...
label morte2_s12: # from 13.1 14.1 15.1 16.1 17.0 18.0 19.1 20.0 21.0 22.0 23.1 24.2 25.1 26.0 # IF WEIGHT #1 ~  !Global("Mortuary_Walkthrough","GLOBAL",0) !Global("Mortuary_Walkthrough","GLOBAL",1) !Global("Mortuary_Walkthrough","GLOBAL",3) InParty("Morte")
    nr '"대체 뭘 그렇게 골똘히 생각하는 거야, 대장?"{#morte2_s12_}'
...
label morte2_s13: # from 12.0
    nr '"이봐, 대장. 설마 벌써 까먹은 건 아니겠지?"{#morte2_s13_}'
...
label morte2_s14: # from 13.0
    nr '"그 소리를 많이 듣게 될 것 같군." 모트는 헛기침을 한다. "어디 보자…"  „네가 스틱스 강물을 몇 통이나 마신 듯한 기분인 것은 알지만, 정신을 차리고 집중해야 한다. 네 소지품들 중에는 이 중대사에 대해서 일부나마 밝혀 줄 일지가 한 권 있을 것이다. 만약 파로드가 이미 죽지 않았다면 그가 나머지 부분에 대해 설명해줄 수 있을 것이다.“{#morte2_s14_}'
...
label morte2_s15: # from 14.0
    nr '"그래, 그래, 기다리라고." 모트는 잠시 멈춘다. "좋아, 이게 마지막이야…"  „절대로 일지를 잃어서는 안 된다, 아니면 우린 다시 스틱스에 빠지는 신세가 될 테니까. 알겠나? 그리고 내 말을 믿어라 - 뭘 하든 간에 네가 누구이며, 어떤 일이 네게 일어나고 있으며, 어디서 왔는지에 대해 말해서는 안 된다. 그렇지 않으면 화장터로 직행하게 될 테니. 내가 말하는 대로 해라: 일지를 읽은 후 파로드를 찾아라.“{#morte2_s15_}'
...
label morte2_s16: # from 15.0
    nr '"아니… 여기 왔을 때 대장은 깨끗하게 털린 상태였어. 이미 말했듯이 대장 몸에는 충분하고도 남을 정도의 일지가 적혀 있다고."{#morte2_s16_}'
...
label morte2_s17: # from 16.0
    nr '"아니. 그래도 이 파로드란 자가 어디 있는지 아는 사람은 있을 거야… 물론 그건 우리가 이 곳에서 탈출한 후의 얘기지만."{#morte2_s17_}'
...
label morte2_s18: # from 12.1
    nr '"그것은 „시체안치소“라고 불리지… 임신한 거미만큼이나 매력적인 모양새의 커다란 검은 건물이라고."{#morte2_s18_}'
...
label morte2_s19: # from 12.2 12.3
    nr '"그들은 자신들을 „더스트맨“이라고 부르고 있어. 그들을 알아보는 건 쉽지. 예외없이 검은 법복을 입고 있는데다가 꼭 사후경직을 일으킨 것 같은 얼굴들을 하고 있으니까. 그들은 악귀같은 죽음 숭배자들이야. 그들은 모두 다 죽어야 한다고 믿고 있어… 그것도 빠르면 빠를수록 좋다고."{#morte2_s19_}'
...
label morte2_s20: # from 19.0
    nr '"내가 말할 때 뭘 하고 있었어?! 더스트맨은 모두가 다 죽어야 하며, 그것도 빠르면 빠를수록 좋다고 믿고 있다고 내가 이미 말했잖아. 대장이 보기엔 지금까지 우리가 본 시체들이 죽어서 더 행복해진 것 같아?"{#morte2_s20_}'
...
label morte2_s21: # from 12.6
    nr '"어… 그것들을 쓰라고. 지혈 등의 용도로 말이야."  ^NNOTE: <BANDAGES2>^-{#morte2_s21_}'
...
label morte2_s22: # from 12.4
    nr '"다원 우주에서는 매일 누군가 반드시 죽어, 대장. 이 얼간이들은 이 곳의 관리자들에게 사후의 시체에 대한 권리를 판 한심한 작자들의 잔해라고."{#morte2_s22_}'
...
label morte2_s23: # from 12.5
    nr '"뭐- *진담*이야? 이봐, 대장, 이 죽은 아가씨들은 우리들처럼 산전수전 다 겪은 친구들에게 주어진 마지막 기회라고. 우리는 *기사도* 정신을 발휘해야 해… 열쇠를 찾으려고 그녀들을 칼로 베거나 사지를 절단하는 짓 따위는 해서는 안돼."{#morte2_s23_}'
...
label morte2_s24: # from 23.0
    nr '"대장, 그들은 죽었어, 그리고 우리들도 죽었어… 내가 무슨 말을 하려는지 알겠어? 그래?"{#morte2_s24_}'
...
label morte2_s25: # from 24.0 24.1
    nr '"대장, 우린 이미 이 절뚝거리는 아가씨들과 대화를 시작할 화제거리도 가지고 있다고. 우리들은 *모두* 적어도 한 번씩은 죽어 봤어. 그러니 우린 공통의 화제가 있는 셈이지. 그들은 우리처럼 죽은 경험이 있는 사내들을 환영할 거라고."{#morte2_s25_}'
...
label morte2_s26: # from 25.0
    nr '"그래… 좋아. *대장*은 죽지 않았을지도 모르지만 *난* 죽었어. 그리고 나라면 여기 있는 체격 좋은 멋진 시체 아가씨들과 기꺼이 관을 함께 쓰겠어." 모트는 기대에 부푼 듯 이빨을 딱딱거리기 시작한다. "물론 그러기 위해서는 우선 관리인들이 그들을 포기해야 하는데 그럴 가능성은 희박하지…"{#morte2_s26_}'
...
label morte2_s27: # - # IF WEIGHT #3 /* Triggers after states #: 31 even though they appear after this state */ ~  !InParty("Morte")
    nr '"난 대장이 돌아올 줄 알고 있었어! 드디어 대장에겐 내가 필요하다는 사실을 깨달은 거야?"{#morte2_s27_}'
...
label morte2_s28: # from 27.1
    nr '"흠. 글쎄, 내가 여기서 얼마나 더 기다릴 수 있을 지는 나도 모르겠어. 그러니 난 대장에게 마지막으로 단 한 번의 기회를 주겠어. 정말 내 현명한 충고와 기민한 재치를 필요로 하지 않는 거야?"{#morte2_s28_}'
...
label morte2_s29: # from 28.0 28.2
    nr '"일부러 내 감정을 상하게 하려는 거야, 대장? 내가 무슨 말이라도 잘못했어? 아니면 내게 팔이 없어서야? 뭐야?"{#morte2_s29_}'
...
label morte2_s30: # from 29.1
    nr '"좋아, 하지만 난 여기서 영원히 기다리진 않을 거니까 생각이 바뀌면 당장 돌아오라고."{#morte2_s30_}'
...
label morte2_s31: # - # IF WEIGHT #2 ~  Global("Mortuary_Walkthrough","GLOBAL",3) InParty("Morte")
    nr '"신들이시여! 그건 정말 골때리는 책이군!"{#morte2_s31_}'
...
label morte2_s32: # from 31.0
    nr '"추측을 하자면 그 책은 이 곳에 실려오는 모든 불행한 작자들의 이름을 기록하는 책인 것 같아."{#morte2_s32_}'
...
label morte2_s33: # from 32.0
    nr '"어… 그럴 수도 있겠지. 알아보려면 저기 있는 저 떠다니는 더스티와 얘기를 해야 할 거야. 그게 좋은 생각인지는 모르겠지만."{#morte2_s33_}'
