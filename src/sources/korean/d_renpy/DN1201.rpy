...
label n1201_s0: # from 1.6 3.0 # IF ~  True()
    nr '이 악취가 나는 쪽지에 적힌 글 밑에는 기묘하게 생긴 도표가 그려져 있다. 그것은 당신에게 쪽지의 구석이 중심을 향하도록 접으라고 지시하고 있는 듯하다. 각 구석에는 일련의 기묘한 부호들이 그려져 있다. 우측 상단에는 하나, 우측 하단에는 두 개, 좌측 하단에는 세 개, 그리고 좌측 상단에는 네 개의 부호들이 각각 그려져 있다.{#n1201_s0_}'
...
label n1201_s1: # from 0.0 0.1 0.2 0.3 1.0 1.1 1.2 1.3 1.4
    nr '당신은 모서리가 중심에 닿도록 쪽지의 구석을 안쪽으로 접는다.{#n1201_s1_}'
...
label n1201_s2: # from 1.5
    nr '당신이 왼쪽 상단 구석을 중심 쪽으로 접자 오른쪽 상단 구석이 펼쳐져 원래 위치로 돌아간다.{#n1201_s2_}'
...
label n1201_s3: # from 2.1
    nr '당신이 왼쪽 하단 구석을 안쪽으로 접자 잠시 후 다른 두 구석들이 바깥쪽으로 펼쳐진다. 아무 일도 일어나지 않는다.{#n1201_s3_}'
...
label n1201_s4: # from 2.0
    nr '당신이 오른쪽 상단 구석을 다시 중심 쪽으로 접자, 왼쪽 하단 구석이 그 동작을 그대로 따라 하고, 다른 구석들도 연달아 안쪽으로 접힌다. 당신은 그 광경을 잠시 지켜본다, 그리고 종이의 각 구석들이 일어서서 쪽지를 작은 4면 피라미드로 만든다.{#n1201_s4_}'
...
label n1201_s5: # from 4.0
    nr '당신이 피라미드의 면을 벗기자 종이는 먼지가 되어 사라진다. 그 안에는 작은 삼각형 모양의 귀걸이가 들어 있다. 그것은 빛을 받아 밝게 반짝인다.{#n1201_s5_}'
