...
label s863_s0: # from 10.0 # IF ~  !HasItem("DRemind","S863")
    nr '이 스켈레톤은 엄청난 수난을 겪은 듯하다. 전투 때문인지 아니면 계단에서 여러 번 굴러 떨어져서인지는 알 수 없으나, 양쪽 팔과 다리가 부서져 금속 막대와 가죽끈으로 수선되었다. 해골의 이마에는 "863"이란 번호가 새겨져 있으나 후두부는 함몰되어 텅 빈 속이 들여다 보인다.{#s863_s0_1}'
...
label s863_s1: # from 0.0 0.1 0.2 0.3
    nr '스켈레톤은 아무런 응답도 하지 않는다.{#s863_s1_1}'
...
label s863_s2: # from 0.4
    nr '스켈레톤은 아무런 응답을 하지 않는다. 당신의 질문에 대답할 수 있을 정도의 정신은 이미 그것에게는 남아있지 않는 듯하다.{#s863_s2_1}'
...
label s863_s3: # from 0.5
    nr '누군가가 이 스켈레톤의 뼈들을 가죽끈들로 감았는데, 끈들은 실제의 근육과 근을 연상시키는 패턴으로 감겨졌다. 가죽끈들은 스켈레톤의 관절에 박힌 금속 볼트에 고정되어 있다. 이 스켈레톤은 오랫동안 사용된 것 같다. 그 뼈들 중 상당수가 깨졌으며 부서진 부분들은 봉함제와 악취가 나는 접착제로 봉합되어 있다.{#s863_s3_1}'
...
label s863_s4: # from 0.7 0.10 3.1 3.4
    nr '당신은 철제 볼트를 뽑아 내려고 시도하나, 당신은 그것들을 뽑아 낼 정도로 힘이 세지 못하다, 그것들은 매우 튼튼히 박혀 있다.{#s863_s4_1}'
...
label s863_s5: # from 0.8 0.11 3.2 3.5
    nr '당신은 온몸의 힘을 다 기울여 철제 볼트를 뽑아 내려고 노력한다, 그리고 수 초간의 분투 끝에 마침내 관절로부터 볼트를 뽑아 낸다. 스켈레톤은 붕괴하나, 그 뼈들 중 일부는 아직도 씰룩거린다.{#s863_s5_1}'
...
label s863_s6: # from 0.9 0.12 3.3 3.6
    nr '당신은 쇠 지렛대를 이용하여 관절로부터 볼트를 뽑아 낸다. 스켈레톤은 붕괴하나, 그 뼈들 중 일부는 아직도 씰룩거린다.{#s863_s6_1}'
...
label s863_s7: # - # IF ~  False()
    nr '스켈레톤은 아무런 응답을 하지 않는다. 당신의 질문에 대답할 수 있을 정도의 정신은 이미 그것에게는 남아있지 않는 듯하다.{#s863_s7_1}'

    menu:

# s8 # say64262
label s863_s8: # - # IF ~  HasItem("DRemind","S863")
    nr '이 스켈레톤은 많은 전투를 겪었던지, 아니면 계단에서 여러 번 굴러 떨어진 것 같다. 양쪽 팔과 다리가 부서져 금속 막대와 가죽끈으로 수선되었다. 해골의 이마에는 "863"이란 번호가 새겨져 있으나 후두부는 함몰되어 텅 빈 속이 들여다 보인다. 당신은 누군가가 이를 이용하기로 작심하고 그 안에 양피지를 말아서 넣어 놓은 것을 발견한다.{#s863_s8_1}'
...
label s863_s9: # from 8.0
    nr '당신은 두루마리를 노동자의 해골로부터 꺼낸다. 묘하게도 해골의 공동은 매시지를 저장하기 위해 일부러 만든 것처럼 보인다. 두루마리가 우연히 떨어져 나오는 것을 방지하기 위해 두루마리는 해골 안에 달린 고리에 가는 실로 매여 있다.{#s863_s9_1}'
...
label s863_s10: # from 9.0
    nr '당신은 실을 풀고 두루마리를 훑어본다. 그것은 시체안치소의 관리인들 중 하나가 쓴 독촉장인 것 같다. 그 내용으로 감안할 때, 이 스켈레톤은 일종의 심부름꾼인 듯하다. 당신은 스켈레톤을 다시 흘긋 본다, 그리고 그가 철판 앞에서 멈춘 것은 그가 그것을 어떻게 지나가야 하는지 모르기 때문이라는 사실을 깨닫는다.  ^NNOTE: <READSTUFF>^-{#s863_s10_1}'
