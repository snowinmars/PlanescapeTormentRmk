...
label zf1072_s0: # - # IF ~  True()
    nr '이 시체로부터 포름알데히드의 냄새가 유달리 강하게 난다… 최근에 방부제를 뿌린 듯하며, 그렇게 한 데는 그럴 만한 이유가 있다. 이 시체는 부패가 상당히 진행된 상태이기 때문이다. 그녀의 턱은 이미 없어졌으며, 그녀의 해골로부터 살점이 일부 떨어져 나간 탓에 뼈에 새겨진 "1072"라는 번호가 보인다.{#zf1072_s0_1}'
...
label zf1072_s1: # from 0.0 0.1 0.2
    nr '시체는 당신 목소리에 아무런 반응도 하지 않는다. 턱이 없기 때문인지도 모르겠다. 아니면 아무런 할 말이 없기 때문일 수도 있다.{#zf1072_s1_1}'
...
label zf1072_s2: # from 0.3
    nr '시체는 움직이지 않는다. 당신의 질문에 대답할 정도의 정신은 이제 그것에게는 남아있지 않은 것 같다.{#zf1072_s2_1}'
...
label zf1072_s3: # - # IF ~  False()
    nr '시체는 움직이지 않는다. 당신의 질문에 대답할 정도의 정신은 이제 그것에게는 남아있지 않은 것 같다.{#zf1072_s3_1}'
