...
label zf444_s0: # - # IF ~  True()
    nr '이 여자 시체의 몰골은 말이 아니다. 방부제로 처리된 그녀의 딱딱한 피부 전체를 쥐에게 물린 것으로 추정되는 수백 개의 작은 상처가 뒤덮고 있다. 상처 주변이 오그라든 것으로 미루어 볼 때 상처들은 시체가 처리되기 전에 생긴 듯하다. 그녀의 입술은 꿰매져 있으며, 얼굴에는 "444"란 번호가 흑청색 잉크로 적혀 있다.{#zf444_s0_1}'
...
label zf444_s2: # from 0.3
    nr '시체는 아무런 응답을 하지 않는다. 당신의 질문에 대답할 수 있을 정도의 정신은 이미 그것에게는 남아있지 않는 듯하다.{#zf444_s2_1}'
...
label zf444_s3: # - # IF ~  False()
    nr '시체는 아무런 응답을 하지 않는다. 당신의 질문에 대답할 수 있을 정도의 정신은 이미 그것에게는 남아있지 않는 듯하다.{#zf444_s3_1}'
