...
label zf832_s0: # - # IF ~  True()
    nr '비록 이 시체의 피부가 지금은 건조하고 가죽 같지만, 생전에 그녀는 아름다운 중년여성이었음이 틀림없다. 이 시체를 처리한 사람도 그녀를 동정했던지 그녀의 입술을 표가 별로 나지 않도록 깨끗하게 꿰맸으며, 이마의 "832"란 번호 역시 우아한 글씨체로 새겼다.{#zf832_s0_}'
...
label zf832_s2: # from 0.3
    nr '시체는 아무런 응답을 하지 않는다. 당신의 질문에 대답할 수 있을 정도의 정신은 이미 그것에게는 남아있지 않는 듯하다.{#zf832_s2_}'
...
label zf832_s3: # - # IF ~  False()
    nr '시체는 아무런 응답을 하지 않는다. 당신의 질문에 대답할 수 있을 정도의 정신은 이미 그것에게는 남아있지 않는 듯하다.{#zf832_s3_}'
