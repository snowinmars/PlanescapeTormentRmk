...
label zf891_s0: # - # IF ~  True()
    nr '이 유별나게 엽기적으로 생긴 여자 좀비에게는 귀, 코, 그리고 입술이 없다. 이 시체를 처리한 사람은 그녀의 입을 꿰매기 위해 주변의 피부를 잡아당기느라 고생을 했을 것 같다. 아직도 남은 틈새 사이로 누렇고 구부정한 이빨들이 보인다. 그녀의 이마에는 "891"이란 번호가 새겨져 있다.{#zf891_s0_1}'
...
label zf891_s2: # from 0.3
    nr '시체는 아무런 응답을 하지 않는다. 당신의 질문에 대답할 수 있을 정도의 정신은 이미 그것에게는 남아있지 않는 듯하다.{#zf891_s2_1}'
...
label zf891_s3: # - # IF ~  False()
    nr '시체는 아무런 응답을 하지 않는다. 당신의 질문에 대답할 수 있을 정도의 정신은 이미 그것에게는 남아있지 않는 듯하다.{#zf891_s3_1}'
