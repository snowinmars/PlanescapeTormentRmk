...
label zm1201_s0: # - # IF ~  Global("1201_Note_Retrieved","GLOBAL",0)
    nr '시체의 이마에는 잉크로 "1201"이란 번호가 적혀 있으며, 잉크는 눈을 거쳐 볼과 턱에까지 흘러내렸다. 잉크가 흘러내려 간 자국을 따라가다가 당신은 잉크가 시체의 입술을 꿰맨 자국 틈새로까지 들어가 시체의 입 안에 낀 쪽지와 같은 것의 구석에서 멈추었다는 사실을 깨닫는다.{#zm1201_s0_}'
...
label zm1201_s1: # from 0.0
    nr '쪽지는 좀비 입 안의 점액으로 범벅이 되어 있다. 만약 당신 시체의 입술을 꿰매고 있는 실들 사이로 쪽지를 억지로 꺼내려고 한다면 종이가 갈기갈기 찢어질 것 같다. 시체를 칼로 베어서 꺼내는 방법도 쪽지를 파손할 위험이 크다. 쪽지를 꺼내기 전에 꿰맨 실들부터 조심스럽게 제거할 방법부터 찾아야 할 것 같다.{#zm1201_s1_}'
...
label zm1201_s2: # from 1.0
    nr '당신은 시체의 입을 꿰매고 있는 실들을 능숙하게 잘라낸다, 그러자 시체의 턱이 아래로 쳐지면서 입이 열린다. 당신은 시체의 입으로부터 쪽지를 조심스럽게 꺼낸다… 종이의 상태에도 불구하고 그 위에 적힌 글은 아직도 알아볼 수가 있다.  ^NNOTE: <READSTUFF>^-{#zm1201_s2_}'
...
label zm1201_s3: # from 0.1 5.0 5.1 5.2
    nr '시체의 유백색 눈은 당신을 공허하게 쳐다보고 있다.{#zm1201_s3_}'
...
label zm1201_s4: # from 0.2 5.3
    nr '시체는 움직이지 않는다. 당신의 질문에 대답할 정도의 정신은 이제 그것에게는 남아있지 않은 것 같다.{#zm1201_s4_}'
...
label zm1201_s5: # from 2.0 # IF ~  Global("1201_Note_Retrieved","GLOBAL",1)
    nr '체의 이마에는 잉크로 "1201"이란 번호가 적혀 있으며, 잉크는 눈을 거쳐 볼과 턱에까지 흘러내려 마치 울고 있는 것처럼 보인다. 턱이 내려가 입이 열린 상태이며, 입의 한쪽 구석으로부터 약간의 점액이 흘러나오고 있다.{#zm1201_s5_}'
