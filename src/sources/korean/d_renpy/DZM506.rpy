...
label zm506_s0: # from 3.2 # IF ~  Global("506_Thread","GLOBAL",0)
    nr '꿰맨 곳 투성이인 이 시체는 두 철판 사이를 천천히 비틀거리면서 오가고 있다. 그의 이마… 목 옆쪽… 오른팔… 에는 "506"이란 번호가 실로 꿰매져 있다… 이 시체의 벗겨지고 있는 피부는 너무나도 빈번하게 꿰매진 탓에 마치 기괴한 거리 지도처럼 보인다.{#zm506_s0_}'
...
label zm506_s1: # from 0.1 4.0 4.1 5.0 5.1 5.2
    nr '시체는 아무 것도 느끼지 못하고 앞만 바라본다.{#zm506_s1_}'
...
label zm506_s2: # from 0.2 5.3
    nr '시체는 움직이지 않는다. 당신의 질문에 대답할 정도의 정신은 이제 그것에게는 남아있지 않은 것 같다.{#zm506_s2_}'
...
label zm506_s3: # from 0.0
    nr '꿰맨 자국은 시체의 전신을 감싸고 있다. 팔에서 시작하여, 가슴과 목을 거쳐, 축축한 흰머리 속에까지 이르고 있다. 당신은 꿰맨 자국들의 교차로를 따라가다가 누군가가 시체의 이마에 바늘을 쑤셔 넣은 것을 발견한다… 바늘에는 머리의 측면을 꿰매고 있는 실이 꿰어져 있다. 만약 실을 자를 수 있는 도구가 있다면 바늘을 빼낼 수 있을 듯하다.{#zm506_s3_}'
...
label zm506_s4: # from 3.0
    nr '당신은 메스로 실을 깔끔하게 잘라낸다, 그리고 바늘을 실과 함께 뽑아 낸다. 그러자 이마를 덮고 있던 피부가 벗겨져 백묵처럼 하얀 두개골이 드러난다. 놀랍게도 그 곳에는 "78"이란 번호가 새겨져 있다.{#zm506_s4_}'
...
label zm506_s5: # from 4.2 # IF ~  Global("506_Thread","GLOBAL",1)
    nr '꿰맨 곳 투성이인 이 시체는 두 철판 사이를 천천히 비틀거리면서 오가고 있다. 그의 몸 대부분의 부위에는 506"이란 번호가 실로 꿰매져 있으나, 이마의 피부가 벗겨져 드러난 두개골에는 "78"이란 번호가 새겨져 있다.{#zm506_s5_}'
