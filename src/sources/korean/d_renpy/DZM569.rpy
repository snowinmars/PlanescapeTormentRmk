...
label zm569_s0: # - # IF ~  True()
    nr '이 비틀거리는 시체는 죽은지 여러 해가 지난 것 같다. 이마의 피부가 이미 벗겨져 분필처럼 하얀 뼈가 드러나 보인다. 누군가가 드러난 뼈에 "569"라는 번호를 새겨 놓았다.{#zm569_s0_}'
...
label zm569_s2: # from 0.4
    nr '시체는 움직이지 않는다. 당신의 질문에 대답할 정도의 정신은 이제 그것에게는 남아있지 않은 것 같다.{#zm569_s2_}'
...
label zm569_s3: # from 0.5
    nr '이 시체는 열쇠를 가지고 있지 않는 것 같다… 그리고 설령 가지고 있더라도 그것을 사용할 수 있을 것 같지는 않다. 누가 망치로 내려친 듯 그의 손가락은 모두 부서져 있다. 당신은 그의 왼쪽 어깨에 붕대가 잔뜩 감겨 있다는 사실을 눈치챈다… 시체를 먼저 해치우고 나면 그 붕대를 쓸 수 있을 것 같다.{#zm569_s3_}'
