...
label zm782_s0: # - # IF ~  True()
    nr '당신이 다가가자 이 시체는 동작을 멈추고 당신을 공허한 눈으로 바라본다. 그의 이마에는 "782" 이라고 새겨져 있으며, 그의 입은 꿰매져 있다. 그의 몸으로부터는 포름알데히드 냄새가 약간 난다.{#zm782_s0_}'
...
label zm782_s1: # from 0.1
    nr '시체는 아무런 응답을 하지 않는다.{#zm782_s1_}'
...
label zm782_s2: # from 0.2 0.3
    nr '이 시체가 바로 열쇠를 가지고 있는 시체인 것 같다. 그는 열쇠를 왼손 엄지와 집게손가락으로 꽉 쥐고 있는데 경직되어 벌어질 생각을 하지 않는다. 열쇠를 빼내려면 시체의 손을 잘라야만 할 것 같다.{#zm782_s2_}'
