...
label zm965_s0: # - # IF ~  NearbyDialog("Dmorte")
    nr '이 시체는 삼각형을 그리며 움직이고 있다. 시체는 삼각형의 한 모퉁이에 도달하면 일단 잠시 멈춘다, 그리고 방향을 바꾸어 다음 모퉁이로 향한다. 그것의 해골 옆머리에는 "965"라는 문신이 새겨져 있다. 당신이 다가가자 시체는 걸음을 멈추고 당신을 쳐다본다.'
...
label zm965_s1: # externs morte_s481 morte_s480 # IF ~  !NearbyDialog("Dmorte")
    ...
    menu:
        '"그런데… 당신은 왜 삼각형을 그리며 걷는 거요?"' if zm965Logic.r34923_condition():
            # a0 # r34923
            $ zm965Logic.r34923_action()
            jump zm965_s2

        '"그런데… 당신은 왜 삼각형을 그리며 걷는 거요?"' if zm965Logic.r45070_condition():
            # a1 # r45070
            jump zm965_s2

        '"난 당신이 좀비가 아니라는 것을 아오. 당신은 아무도 속이지 못할 거요."' if zm965Logic.r45071_condition():
            # a2 # r45071
            jump zm965_s2

        '시체에게 당신의 뼈들이 해주는 이야기 능력을 사용한다.' if zm965Logic.r45072_condition():
            # a3 # r45072
            jump zm965_s3
