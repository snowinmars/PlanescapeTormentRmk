...
label zm985_s0: # - # IF ~  Global("Topple_985","GLOBAL",0)
    nr '이 시체 "985"번은 갑자기 멈추어 섰다. 왼쪽 다리의 상태로 미루어 볼 때, 곰팡이나 세균 때문에 무릎 부분이 썩은 듯하다. 시체는 균형을 유지하려고 앞뒤로 불안정하게 비틀거리고 있다.{#zm985_s0_1}'
...
label zm985_s1: # from 0.4 5.0 5.1 5.2
    nr '시체는 아무 것도 느끼지 못하고 앞만 바라본다. 당신이 말을 걸어도 아무런 반응을 나타내지 않는다.{#zm985_s1_1}'
...
label zm985_s2: # from 0.5 5.3
    nr '시체는 움직이지 않는다. 당신의 질문에 대답할 정도의 정신은 이제 그것에게는 남아있지 않은 것 같다.{#zm985_s2_1}'
...
label zm985_s3: # from 0.1 6.0
    nr '시체의 왼쪽 다리에서 *우두둑* 소리가 들린다, 그리고 시체는 죽은 나무처럼 쓰러진다. 동체는 판석과 충돌하여 썩은 멜론처럼 산산조각이 나고, 그 안에서 오물과 농장이 콸콸 흘러나온다. 놀랍게도 아무도 시체가 쓰러진 것을 눈치채지 못한 듯하다… 더욱 이상한 것은 왼쪽 다리가 시체가 서 있던 자리에 마치 차렷을 하는 듯 그대로 서 있다는 점이다. 하지만 다리도 잠시 후 *쿵* 소리와 함께 쓰러진다.{#zm985_s3_1}'
...
label zm985_s4: # from 0.2
    nr '당신은 시체를 안정시키려고 그것의 왼쪽 팔을 잡으려 한다. 하지만 당신이 팔을 잡자 시체는 갑자기 오른쪽으로 기운다. 때문에 당신은 시체를 안정시키기는커녕 잡아당긴 꼴이 되었다…{#zm985_s4_1}'
...
label zm985_s5: # - # IF ~  GlobalGT("Topple_985","GLOBAL",0)
    nr '누군가가 이 시체의 왼쪽 다리와 왼쪽 팔을 다른 시체로부터 가져온 여분의 팔다리로 교체한 것 같다. 왼쪽 다리는 전의 다리보다 짧다, 하지만 적어도 이제 시체는 설 수가 있다.{#zm985_s5_1}'
...
label zm985_s6: # from 0.3
    nr '당신은 시체를 안정시키려고 그것의 왼쪽 팔을 잡으려 한다. 하지만 당신이 팔을 잡자 시체는 갑자기 오른쪽으로 기운다. 때문에 당신은 시체를 안정시키기는커녕 잡아당긴 꼴이 되었다…{#zm985_s6_1}'
...
label zm985_s7: # from 3.0
    nr '당신은 시체의 썩은 잔해를 살펴보다가 그것의 왼쪽 팔은 아직도 멀쩡하다는 사실을 발견한다. 그것은 낙하 도중에 동체로부터 떨어져 나갔으며, 신체의 다른 부위와는 달리 부패가 별로 진행되지 않은 듯하다.{#zm985_s7_1}'
