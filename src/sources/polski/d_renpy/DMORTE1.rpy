...
label morte1_s0: # - # IF WEIGHT #1 /* Triggers after states #: 26 even though they appear after this state */ ~  !InParty("Morte") Global("Morte","GLOBAL",0)
    nr '"Hej, szefie. Dobrze się czujesz? Udajesz martwego, czy chcesz wpędzić Grabarzy w ślepaki? Myślałem, że jesteś stuprocentowym truposzem."~ [MRT001]'
...
label morte1_s1: # from 0.0
    nr '"Co… kim *ja* jestem? A może byś *ty* zaczął. Ktoś ty?"'
...
label morte1_s2: # from 1.0 3.0 4.0
    nr '"Nie pamiętasz swojego *imienia*, co? NASTĘPNYM razem, jak będziesz chciał spędzić noc w tej dziurze, to nie przeginaj z bełtem. Nazywam się Morte. Ja też tu jestem uwięziony."'
...
label morte1_s4: # from 3.1
    nr '"Ech… jesteś spięty, jakby ci coś w tyłek włożyli. Niech ci będzie. W takim razie to *ja* będę nawijał. Moje imię brzmi Morte. Jak się nazywasz?"'
...
label morte1_s8: # from 7.0
    nr '"No tak… rany na klatce nie są AŻ TAK tragiczne… ale te na plecach…" Morte waha się. "Powiedzmy, że wygląda to tak, jakby ktoś zamienił cię w galerię tatuażu, szefie. Coś mi to przypomina…"'
...
label morte1_s10: # from 9.0
    nr '"Jeszcze coś tu jest…" Morte zastanawia się. "Zobaczmy… to chyba ciąg dalszy…"  „Nie zgub dziennika, bo znowu popłyniemy w górę Styksu. I cokolwiek robisz, NIE MÓW nikomu KIM jesteś, ani CO się z tobą dzieje, bo inaczej wylądujesz w krematorium. Rób co ci każę: PRZECZYTAJ dziennik, a potem ODSZUKAJ Faroda.“'
...
label morte1_s13: # from 12.0
    nr '"Wszystkie drzwi są pozamykane, więc będziemy potrzebowali klucza. Może jakiś zombie ma go przy sobie."'
...
label morte1_s14: # from 13.0
    ...
    menu:
        ...
        '"Więc mam zaatakować jednego z tych zombie i zakosić mu klucz?"':
            # a17 # r39825
            jump morte1_s16
...
label morte1_s15: # from 14.0
    nr '"A co, uważasz, że to urazi ich uczucia? Przecież są MARTWI. Ale ma to też swoje plusy - jak ich zabijesz, to przynajmniej odpoczną sobie przez chwilę zanim ich pracodawcy znowu ich nie zapędzą do pracy."'
...
label morte1_s16: # from 14.1 15.0
    nr '"Zanim to zrobisz, znajdź sobie jakąś broń. Na jednej z tych półek widziałem chyba skalpel."  ^NNOTE: Przeszukaj półki w pokoju, aby znaleźć broń przeciw zombie. <SEARCH_WEAPON>^-'
...
label morte1_s18: # - # IF WEIGHT #2 /* Triggers after states #: 26 even though they appear after this state */ ~  Global("Mortuary_Walkthrough","GLOBAL",0) !PartyHasItem("Scalpel") Global("ZM782_Dead_KAPUTZ","GLOBAL",0)
    nr '"Na jednej z półek powinien być skalpel." Na twoim miejscu poszukałbym go, zanim się zmierzę z ożywionymi trupami."  ^NNOTE: Przeszukaj półki w pokoju, aby znaleźć broń przeciw zombie. <SEARCH_WEAPON>^-'
...
label morte1_s23: # from 19.1 20.1 21.1 22.0
    nr '"Czas zabić te trupy drugi raz…"  ^NNOTE: <ATTACKNOTE>^-'
...
label morte1_s24: # - # IF WEIGHT #4 /* Triggers after states #: 26 even though they appear after this state */ ~  Global("Mortuary_Walkthrough","GLOBAL",0) !PartyHasItem("KeyPr") Global("ZM782_Dead_KAPUTZ","GLOBAL",1)
    nr '"Widzę, że udało ci się załatwić właściwego trupa. Teraz musisz tylko znaleźć klucz. Powinien mieć go przy sobie. On pozwoli nam się stąd wydostać."  ^NNOTE: <DEADPILE>^-'
...
label morte1_s25: # - # IF WEIGHT #5 /* Triggers after states #: 26 even though they appear after this state */ ~  Global("Mortuary_Walkthrough","GLOBAL",0) PartyHasItem("KeyPr")
    ...
    menu:
        '"W takim razie spróbuję wszystkich drzwi."':
            # a31 # r39850
            jump morte1_dispose
...
label morte1_s26: # - # IF WEIGHT #0 ~  !InParty("Morte") GlobalGT("Morte","GLOBAL",0)
    nr '"Wiedziałem, że wrócisz, szefie! W końcu zorientowałeś się, że mnie potrzebujesz, co?"~ [MRT516]'
...
label morte1_s27: # from 26.1
    nr '"Hmmmm. Nie wiem, czy mnie tu jeszcze zastaniesz, ale dam ci jeszcze OSTATNIĄ szansę. Jesteś pewien, że nie potrzebujesz moich wskazówek i mojego sprytu?"'

    menu:
        '"Przecież nie masz ŻADNEJ z tych rzeczy."':
            # a34 # r39855
            jump morte1_s28

        '"No dobra, zmieniłem zdanie. No dalej, ruszajmy."':
            # a35 # r39856
...
label morte1_s28: # from 27.0 27.2
    ...
    menu:
        ...
        '"Nic z tych rzeczy. Po prostu w tej chwili nie potrzebuję towarzystwa. Żegnaj, Morte."':
            # a38 # r39860
            jump morte1_s29
...
label morte1_s29: # from 28.1
    ...
    menu:
        '"Masz to jak w banku. Żegnaj, Morte."':
            # a39 # r39862
            jump morte1_dispose
...
label morte1_s30: # - # IF WEIGHT #6 ~  Global("Mortuary_Walkthrough","GLOBAL",1)
    ...
    menu:
        '"Nic takiego, Morte. Po prostu sprawdzam, czy wciąż ze mną jesteś."':
            # a40 # r39864
            jump morte1_dispose
...
label morte1_s31: # externs zm825_s3 zm825_s0 zm569_s3 zm569_s0
    nr '"Szefie… one cię nie słyszą. Przecież są martwe."'

    menu:
        '"Ale przecież ty też nie żyjesz, a ze mną rozmawiasz."':
            # a41 # r42299
            jump morte1_s32
...
label morte1_s32: # from 31.0
    nr '"Tak, ale ja *jestem* inny. Śmierć nie zabiła we mnie chęci życia. A te tu…" Morte wskazuje na żywe trupy. "Pewnie nie miały wystarczająco dużo charakteru."'
...
label morte1_s33: # from 32.0
    nr '"Wiesz, szefie… coś mnie trafia, jak próbujesz wymienić śpiewkę z tymi truposzami. Zostawmy to świrom, dobra?"'
...
label morte1_s34: # externs zm782_s0
    nr '"Wygląda na to, że mamy drania, szefie. Patrz… ma w ręce klucz."'
