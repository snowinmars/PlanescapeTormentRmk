class MorteLogic:
    ...
    def j64512_s85_r4676_action(self):
        ...
        #$% .register('64512', 'Najprawdopodobniej człowiek przebrany za zombie był członkiem Anarchistów, pewnej frakcji, która chce przejrzeć zamiary wszelkich organów władzy i zniszczyć wszystko, co nawet z daleka pachnie urzędowym ładem.') %$#
    ...
    def r20612_action(self):
        ...
        #$%.register('20538', 'Zadaniem Marty Szwaczki jest chyba zajmowanie się przyniesionymi do niej zwłokami. Wyrywa im zęby, rozpruwa szwy, szuka wszelakich cennych rzeczy jakie mogłyby znajdować się w ciele, a następnie zszywa zwłoki z powrotem. %$#')
    ...
    def r24932_action(self):
        ...
        #$%.register('24933', 'Zacząłem spisywać moje doświadczenia, żeby uporządkować myśli, a także w nadziei, że znajdę jakiś sens w tym co dzieje się wokół mnie. Obudziłem się w miejscu zwanym "Kostnicą"... wygląda to na jakiś dom pogrzebowy. Nie wiem kim jestem, co tutaj robię, ani jak się tu pojawiłem. Pierwszą rzeczą jaką znalazłem po przebudzeniu był latający czerep zwący się Morte. Powiedział, że jesteśmy zamknięci i że klucz jest w posiadaniu jednego z trzęsących się trupów. Zaproponował, żebym poszukał jakiegoś skalpela na półkach, a następnie tak długo dźgał trupy, aż "zginą". %$#')
    ...
    def r41921_action(self):
        ...
        #$%.register('67862', 'W Miejskim Gmachu Rozrywki spotkałem Mieszacza Mordsensa, człowieka, który rzucił klątwę smrodu na niejakiego Śmierdziwiatra. %$#')
    ...
    def r43910_action(self):
        ...
        #$%.register('39490', 'Powiedziałem Nemelle, że szuka jej Aelwina. Nagrodziła mnie magicznym pocałunkiem, który zwiększył moją maksymalną liczbę punktów życia o trzy. %$#')
    ...
    def j39477_s478_r45093_action(self):
        ...
        #$% .register('39477', 'Morte wspominał coś o 'Regule Trójek'. Najwidoczniej niektórzy ludzie wierzą, że wszystko zdarza się potrójnie i przywiązują do tej liczby ogromne znaczenie.') %$#
    ...
    def r53630_action(self):
        ...
        #$%.register('53661', 'Okazuje się, że Morte pochodzi z czegoś pod nazwą Słupa Czaszek, stosu żyjących, odrąbanych głów, znajdującego się na Avernusie, pierwszej warstwie Sfery Baator. Słup Czaszek ponoć składa się z głów łgarzy, którzy swoimi kłamstwami pchnęli innych ku śmierci. %$#')
    ...
    def r65569_action(self):
        ...
        #$%.register('65573', 'Zagadnąłem Mortego o dodatkową linijkę tekstu wypisaną na moich plecach, którą "zapomniał" przeczytać. Tłumaczył, że to nie było nic istotnego, więc dlatego ominął ten fragment. Jestem pewien, że kłamie, ale nie wiem dlaczego. %$#')
    ...
    def r65621_action(self):
        ...
        #$%.register('65625', 'Morte powiedział mi, że posiada pewną specjalną "moc", zwaną "Litanią przekleństw, która sprawiają, że ludzie wpadają w złość. Widocznie zgromadził w czasie swojego przed- i pozagrobowego życia tak wiele złośliwości i obelg, że po prostu stał się specjalistą w prowokowaniu ludzi wszelkiego autoramentu. Kiedy zaś wprawi takich delikwentów w pasję, takich osobników znacznie łatwiej jej sięgnąć, ich siła ataku jest osłabiona a jedyną żądzą, jaka ich opanowuje, jest doścignięcie Mortego i spranie go na kwaśne jabłko. Wygląda na to, że zjawisko takie może być dla mnie korzystne, szczególnie wówczas, kiedy na moje drodze pojawią się nieprzyjacielscy czarnoksiężnicy. A poza tym ten talent, jako, że nie bierze swojego źródła w magii, może się okazać przydatny w przypadku innych konfrontacji z rozmaitymi stworami, które są w rozmaity sposób odporne na czary. Podejrzewam, że im więcej obelg i przekleństw Morte słyszy, tym szybciej jego "moc" rośnie w siłę. %$#')


    def j65637_s667_r65631_action(self):
        self.state_manager.journal_manager.update_journal('65637')
        #$% .register('65637', 'Morte poinformował mnie, że podczas pobytu na półce Lotara, posiadł nową umiejętność, którą określił mianem "Hordy Czaszek". Z tego, co mi opowiedział zrozumiałem, że kilka razy w ciągu dnia jest w stanie wezwać swoich pobratymców z gabinetu Lotara, a oni zaatakują wskazanego przez Mortego przeciwnika, po czym znikną. Rozmiar szkód, jakich dokonają, będzie zależał od poziomu doświadczenia Mortego.') %$#


    def j65669_s675_r65666_action(self):
        self.state_manager.journal_manager.update_journal('65669')
        #$% .register('65669', 'Morte poradził mi, abym, w poszukiwaniu informacji o nocnej wiedźmie, Raveli, udał się do Miejskiego Gmachu Rozrywki w Dzielnicy Urzędników.') %$#


    def j65678_s677_r65674_action(self):
        self.state_manager.journal_manager.update_journal('65678')
        #$% .register('65678', 'Morte zasugerował mi, że ktoś z Dzielnicy Urzędników może wiedzieć coś o tym, jak dotrzeć do Raveli. A gdyby to się nie udało, zawsze możemy spróbować poszukać czegoś w Miejskim Gmachu Rozrywki i obejrzeć sobie tamtejsze kamienie doznań. Ja mogę dodać od siebie, że jeżeli *i to* nie przyniesie spodziewanych rezultatów, to możemy zawsze udać się jeszcze w odwiedziny do ladacznic... Może to zbyt wiele nie pomoże w naszych poszukiwaniach, ale przynajmniej czas tam spędzony będzie się nam milej kojarzył, niż wspomnienia bezsensownej tułaczki i tak samo bezsensownych dyskusji z mniej czy bardziej rozmownymi przechodniami.') %$#


    def j65712_s686_r65710_action(self):
        self.state_manager.journal_manager.update_journal('65712')
        #$% .register('65712', 'Spróbowałem wybadać Mortego co do jego statusu mimira, ale sprytnie się wywinął, a i ja nie miałem specjalnie na czym się oprzeć w polemice z pyskatą czaszką. Myślę, że dopiero kiedy przyłapię go na kłamstwie, będę w stanie wydusić od niego całą prawdę,.') %$#
    ...
    def r65821_action(self):
        ...
        #$%.register('65825', 'Udało mi się ustalić, dlaczego Morte towarzyszył mi w wędrówce przez cały ten czas. Okazało się, że Morte miał wyrzuty sumienia po tym, jak kiedyś mnie zabił, dawno temu, kiedy jeszcze był człowiekiem. Kiedy wypytałem go dokładnie o to, co czuje w związku z tamtym zabójstwem, on, wyjawiając mi tę tajemnicę, zrzucił z siebie brzemię winy i w ten sposób dodatkowo się wzmocnił. %$#')
