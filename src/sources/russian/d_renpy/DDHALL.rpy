...
label dhall_s1: # externs morte_s104
    ...
    dhall '«Бремя прожитых лет лежит на мне тяжелым грузом, Неугомонный».{#dhall_s1_1}'
    nr 'Он откладывает перо,{#dhall_s1_1}'
    dhall '«Но глухотой я еще не страдаю».{#dhall_s1_1}'
...
label dhall_s2: # from 21.0
    ...
    nr 'Дхолл начинает кашлять, но ему удается перевести дыхание.{#dhall_s2_1}'
    dhall '«Я ошибаюсь?»{#dhall_s2_1}'
...
label dhall_s4: # from 3.0 3.1
    dhall '«Я…»{#dhall_s4_1}'
    nr 'У Дхолла начинается очередной приступ кашля. Спустя минуту или две его дыхание становится достаточно спокойным, чтобы он смог продолжить.{#dhall_s4_1}'
    dhall '«Я… незамедлительно предупрежу стражу».{#dhall_s4_1}'
...
label dhall_s6: # from 5.2
    ...
    dhall '«Подозреваю, что это ты в ответе за нападения в Морге. Это…»{#dhall_s6_1}'
    nr 'Они тихо кашляет, затем тяжело хрипит.{#dhall_s6_1}'
    dhall '«Это не позволит тебе обрести следующую жизнь».{#dhall_s6_1}'
...
label dhall_s7: # from 5.1
    ...
    dhall '«Итак…»{#dhall_s7_1}'
    nr 'Его голос уставший, как будто он повторял это уже много раз.{#dhall_s7_1}'
    dhall '«Ты пробудился ото сна и вернулся в свои грезы».{#dhall_s7_1}'
    nr 'Он продолжает более уважительным голосом.{#dhall_s7_1}'
    dhall '«Приятно встретиться… снова, Неугомонный».{#dhall_s7_1}'
...
label dhall_s10: # from 9.1
    ...
    nr 'Не успев закончить, Дхолл заходится в приступе кашля. Спустя некоторое время он берет себя в руки, и его дыхание снова становится неровным хрипом.{#dhall_s10_1}'
    dhall '«…это зал ожидания для тех, кто собирается покинуть тень этой жизни».{#dhall_s10_1}'
...
label dhall_s11: # from 2.6 2.7 4.1 8.3 9.7 10.6 12.2 14.5 15.3 16.4 19.3 20.3 21.2 22.3 23.3 24.2 25.3 26.3 27.1 28.2 29.4 30.1 31.3 32.7 33.4 34.3 35.3 36.3 37.2 38.3 41.4 42.5 43.4 47.5 48.3 49.3 51.3 52.3 53.2
    ...
    dhall '«Знай: я не завидую тебе, Неугомонный. Твои возрождения — проклятье, которого я бы не смог вынести. Ты должен смириться с этим. Когда-нибудь твой путь вернет тебя сюда…»{#dhall_s11_1}'
    nr 'Дхолл кашляет, из его горла доносятся хрипы.{#dhall_s11_1}'
    dhall '«Это путь всех существ из плоти и костей».{#dhall_s11_1}'
...
label dhall_s12: # from 2.3 2.4 42.2 42.3 43.1 43.2
    ...
    nr 'Дхолл обводит вокруг тебя рукой.{#dhall_s12_1}'
    dhall '«Все умершие прибывают сюда. Кое-кто из них, должно быть, путешествовал с тобой».{#dhall_s12_1}'
...
label dhall_s13: # from 9.3
    dhall '«Хм-м… Главные ворота»{#dhall_s13_1}'
    nr 'Самый очевидный выход, но они не выпустят никого, кроме тленных…{#dhall_s13_1}'
    dhall '«Дхолл заходится в кашле, затем продолжает. — У одного из проводников у главных ворот есть ключ к ним, но он вряд ли откроет их для тебя, разве что ты будешь весьма убедительным».{#dhall_s13_1}'
...
label dhall_s14: # from 10.3
    ...
    nr 'Он вздыхает.{#dhall_s14_1}'
    dhall '«Когда же ты откажешься от своих страстей и покинешь эту тень жизни?»{#dhall_s14_1}'
...
label dhall_s15: # from 9.2 10.4 32.5
    ...
    dhall '«К Моргу тебя доставила твоя ветхая карета, Неугомонный. Увидев ее, ты мог бы возомнить себя аристократом, учитывая количеств подданных, лежащих в ней зловонной разлагающейся кучей».{#dhall_s15_1}'
...
label dhall_s16: # from 15.0
    ...
    nr 'Дхолл снова заходится в приступе кашля, который ему удается побороть только несколько минут спустя.{#dhall_s16_1}'
    dhall '«Твоим „сенешалем“, как всегда, был Фарод, который был рад доставить твое тело к вратам Морга за несколько медяков».{#dhall_s16_1}'
...
label dhall_s17: # from 16.0
    ...
    nr 'Дхолл переводит дыхание, затем продолжает:{#dhall_s17_1}'
    dhall '«У нас в городе есть такие люди, которые собирают тела тех, кто вступил на путь Истинной Смерти, и приносят их нам для подобающего погребения».{#dhall_s17_1}'
...
label dhall_s19: # from 17.4 18.0 18.3 29.3 31.2
    ...
    dhall '«Не ищи Фарода, Неугомонный. Я уверен, что ты попросту пройдешь еще один полный круг, и не станешь от этого мудрее, зато обогатишь Фарода на несколько медяков. Прими смерть, Неугомонный. Не повторяй свой круг страданий».{#dhall_s19_1}'
...
label dhall_s20: # from 19.0
    ...
    dhall '«Я не знаю, в каком притоне находится логово Фарода в данный момент, но могу предположить, что оно где-то за воротами Морга, в Улье. Возможно, кто-то из местных знает, где его найти».{#dhall_s20_1}'
...
label dhall_s21: # from 9.4
    ...
    nr 'Дхолл вздыхает.{#dhall_s21_1}'
    dhall '«Прошу тебя, Неугомонный, больше не проси никого идти с тобой: за тобой остается только несчастье. Не разделяй свою ношу с другими».{#dhall_s21_1}'
...
label dhall_s22: # from 47.0
    ...
    dhall '«Говорят, что есть души, которые не могут достичь Истинной Смерти. Смерть отказалась от них, и их имена никогда не попадут в книгу мертвых. Подняться из мертвых, как это сделал ты… Это наводит на мысль, что ты — одна из таких душ. Само твое существование неприемлемо для нашей фракции».{#dhall_s22_1}'
...
label dhall_s23: # from 8.0
    ...
    nr 'Дхолл, кажется, готов разразиться очередным приступом кашля, но с некоторыми усилиями ему удается сдержаться.{#dhall_s23_1}'
    dhall '«И покуда я не покину свою должность, я буду защищать твое право искать свою правду».{#dhall_s23_1}'
...
label dhall_s24: # from 25.0
    ...
    nr 'Дхолл начинает кашлять, затем успокаивается.{#dhall_s24_1}'
    dhall '«Только я вижу лица тех, кто лежит на наших плитах. Тайна твоего существования со мной в безопасности».{#dhall_s24_1}'
...
label dhall_s25: # from 9.5 23.0
    ...
    nr 'Дхолл снова кашляет, затем глубоко вздыхает.{#dhall_s25_1}'
    dhall '«И пока поток трупов идет через Морг, я буду при своей должности».{#dhall_s25_1}'
...
label dhall_s26: # from 9.6
    ...
    nr 'Дхолл тяжело вздыхает.{#dhall_s26_1}'
    dhall '«Для такого, как я, на планах больше нет чудес».{#dhall_s26_1}'
...
label dhall_s28: # from 2.2 42.1
    ...
    nr 'Дхолл переходит на шепот.{#dhall_s28_1}'
    dhall '«Ты случаем не *бредишь*, Неугомонный? Она достигла Истинной Смерти и ушла туда, куда тебе не добраться».{#dhall_s28_1}'
...
label dhall_s29: # from 36.0
    ...
    dhall '«Скорее всего. Ты что-то потерял… что-то особенно ценное?»{#dhall_s29_1}'
    nr 'Он хмурится, понижая тон.{#dhall_s29_1}'
    dhall '«Вряд ли Фарод будет делать исключения для чего либо, кроме вживленных в плоть вещей, но порой даже этого недостаточно, чтобы остановить его жадность».{#dhall_s29_1}'
...
label dhall_s30: # from 28.0
    ...
    dhall '«Это дурное знамение, Неугомонный. Я молюсь о том, чтобы это оказалось твоим сном… и все же боюсь, это не так».{#dhall_s30_1}'
...
label dhall_s32: # from 8.1 10.0 14.3 15.1 16.2 17.2 18.1 19.1 20.1 22.1 23.1 24.0 25.1 33.2 34.1 37.0 38.1 41.2 47.3 48.1 49.1 51.1 52.1 53.0
    ...
    nr 'Дхолл от волнения понижает тон.{#dhall_s32_1}'
    dhall '«Должно быть, ты был тяжело ранен, раз не узнаешь это место. Это практически твой дом».{#dhall_s32_1}'
...
label dhall_s33: # from 10.2 14.2 32.0 41.0 47.2 49.0
    dhall '«Да, тень. Видишь ли, Неугомонный, эта жизнь… она не настоящая. Наши с тобой жизни»{#dhall_s33_1}'
    nr 'Всего лишь тени, жалкое подобие от того, что было однажды жизнью. Эта 'жизнь'{#dhall_s33_1}'
    dhall '«То, куда мы попадаем *после* того, как умираем. А здесь мы… в западне. В клетке. До тех пор, пока не достигнем Истинной Смерти».{#dhall_s33_1}'
...
label dhall_s34: # from 14.1 32.4
    ...
    nr 'Дхолл надрывно кашляет, затем успокаивается.{#dhall_s34_1}'
    dhall '«Но это всего лишь поверхностные раны».{#dhall_s34_1}'
...
label dhall_s35: # from 16.1 17.1 20.0
    ...
    nr 'Дхолл успокаивает свое неровное дыхание.{#dhall_s35_1}'
    dhall '«Фарод не входит в их число. Он носит свою дурную репутацию как знак гордости и свободно распоряжается имуществом умерших. Он рыцарь легкой наживы, подлая мразь самого низкого пошиба».{#dhall_s35_1}'
...
label dhall_s36: # from 35.0
    ...
    nr 'Дхолл кашляет,{#dhall_s36_1}'
    dhall '«Вор. Все, кого Фарод приносит к нашим стенам, лишены всего имущества, которым они обладали при жизни. Фарод присваивает себе все, что ему удается вырвать из их окоченевших пальцев».{#dhall_s36_1}'
...
label dhall_s38: # from 10.1 14.0
    dhall '«Неугомонный»{#dhall_s38_1}'
    nr 'Имя не хуже других…{#dhall_s38_1}'
    dhall '«Дхолл переводит дыхание. — Ведь что-то держит тебя здесь, не так ли? Какое-то незаконченное дело, какая-то страсть, которая должна быть подавлена, прежде чем ты сможешь достигнуть Истинной Смерти?»{#dhall_s38_1}'
...
label dhall_s40: # - # IF ~  Global("Dhall","GLOBAL",1)
    ...
    dhall '«Итак. Ты вернулся…»{#dhall_s40_1}'
    nr 'Дхолл начинает хрипло дышать, затем у него начинается удушливый кашель. Спустя минуту кашель прекращается, и он, хрипло дыша, продолжает:{#dhall_s40_1}'
    dhall '«…приветствую тебя снова, Неугомонный».{#dhall_s40_1}'
...
label dhall_s44: # from 1.0 6.2 7.0
    ...
    nr 'В голосе писца звучит горечь.{#dhall_s44_1}'
    dhall '«Я *никогда* не знал тебя, Неугомонный. Не больше, чем ты знал о себе сам».{#dhall_s44_1}'
    nr 'Он умолкает на секунду.{#dhall_s44_1}'
    dhall '«Ты ведь все забыл, не так ли?»{#dhall_s44_1}'
...
label dhall_s45: # from 44.0
    ...
    nr 'Он делает легкий поклон, но движение вызывает у него неожиданный приступ кашля.{#dhall_s45_1}'
    dhall '«Я…»{#dhall_s45_1}'
    nr 'Он умолкает на минуту, переводя дыхание.{#dhall_s45_1}'
    dhall '«Я… Дхолл».{#dhall_s45_1}'
...
label dhall_s46: # from 45.1
    ...
    nr 'Дхолл кивает.{#dhall_s46_1}'
    dhall '«Но, боюсь, в данном вопросе время тебе не враг».{#dhall_s46_1}'
    nr 'Он снова берется за перо.{#dhall_s46_1}'
    dhall '«Когда ты снова захочешь поговорить, я буду здесь».{#dhall_s46_1}'
...
label dhall_s48: # from 32.1 33.0 38.0 41.1 47.1
    ...
    nr 'Дхолл кашляет, затем восстанавливает неровное дыхание.{#dhall_s48_1}'
    dhall '«Состояние чистоты».{#dhall_s48_1}'
...
label dhall_s50: # from 33.1
    ...
    nr 'Дхолл качает головой.{#dhall_s50_1}'
    dhall '«Это чистилище. Здесь только горе. Несчастье. Страдание. Они не являются элементами „жизни“. Они — часть клетки, в которой мы заперты в этой тени».{#dhall_s50_1}'
...
label dhall_s51: # from 50.0 50.1 50.2
    ...
    dhall '«Страсти — тяжелый груз. Многих они приковали к этой тени жизни. Цепляясь за эмоции, ты будешь без конца возрождаться в этой „жизни“, в постоянных муках, так никогда и не познав чистоты Истинной Смерти».{#dhall_s51_1}'
...
label dhall_s52: # from 51.0
    ...
    nr 'Дхолл делает вздох… как будто смерть клокочет в его глотке.{#dhall_s52_1}'
    dhall '«За нашими бренными телами, за Границей Вечности, находится покой, к которому стремится каждая душа».{#dhall_s52_1}'
...
label dhall_s53: # from 34.0
    ...
    nr 'Дхолл снова кашляет,{#dhall_s53_1}'
    dhall '«Но только ты можешь знать это точно».{#dhall_s53_1}'
