...
label dustfem_s4: # from 3.0 3.1 3.2 3.3 40.2 40.3
    ...
    dustfem '«Ты потерялся?»{#dustfem_s4_1}'
...
label dustfem_s7: # from 6.0 9.0 20.0
    ...
    nr 'Тленная делает шаг назад, кажется, она собирается позвать стражников.{#dustfem_s7_1}'
...
label dustfem_s8: # from 6.1 16.0 20.1
    ...
    nr 'Тленная отступает на шаг; кажется, она собирается позвать стражников.{#dustfem_s8_1}'
...
label dustfem_s23: # from 22.0
    ...
    dustfem '«Приветствую тебя, посвященный».{#dustfem_s23_1}'
...
label dustfem_s25: # from 23.1
    ...
    nr 'Тленная какое-то время смотрит на тебя, как бы оценивая только что тобою сказанное.{#dustfem_s25_1}'
    dustfem '«Как, ты сказал, тебя зовут?»{#dustfem_s25_1}'
...
label dustfem_s28: # from 26.1
    ...
    nr 'Тленная на секунду умолкает.{#dustfem_s28_1}'
    dustfem '«Это имя *звучит* знакомо… Кажется, я припоминаю сборщика с таким именем. Писец Дхолл может знать о нем больше».{#dustfem_s28_1}'
...
label dustfem_s29: # from 28.0
    ...
    nr 'Тленная умолкает, хмуря брови.{#dustfem_s29_1}'
    dustfem '«Ты нездешний. Кто ты?»{#dustfem_s29_1}'
...
label dustfem_s31: # from 30.0 32.0 33.0
    ...
    dustfem '«Дхолл болен. Он стар, даже по меркам гитцераев. Несомненно, смерть последует за болезнью, которую он подхватил. Ему повезло».{#dustfem_s31_1}'
...
label dustfem_s35: # from 31.1 34.0
    ...
    nr 'Это…{#dustfem_s35_1}'
    dustfem '«Тленная умолкает, пристально глядя на тебя. — Ты ведь нездешний. Кто ты?»{#dustfem_s35_1}'
...
label dustfem_s37: # from 36.0
    ...
    dustfem '«Да».{#dustfem_s37_1}'
    nr 'Она хмурится, затем пристально смотрит на тебя.{#dustfem_s37_1}'
    dustfem '«Ты нездешний. Кто ты?»{#dustfem_s37_1}'
...
label dustfem_s38: # -
    ...
    nr 'Тленная отступает на шаг.{#dustfem_s38_1}'
    dustfem '«Стража! Стража!»{#dustfem_s38_1}'
...
label dustfem_s45: # from 19.5
    ...
    dustfem '«Шепнуть»{#dustfem_s45_1}'
    nr 'Ей что-то на ухо, тленная тоже наклоняется. Как только она оказывается на расстоянии вытянутой руки, ты хватаешь ее за виски и резко сворачиваешь голову влево.{#dustfem_s45_1}'
...
label dustfem_s47: # from 24.2 25.2 29.1 29.2 35.1 35.2 37.1 37.2 49.1 49.2
    ...
    dustfem '«Ты не из наших. Что ты здесь делаешь? Ты из анархистов? Или шпион другой фракции? Кажется, здесь есть дело для стражников…»{#dustfem_s47_1}'
...
label dustfem_s49: # from 24.0 24.1 25.0 25.1
    ...
    dustfem '«Это имя мне незнакомо».{#dustfem_s49_1}'
...
label dustfem_s50: # from 29.0 35.0 37.0 49.0
    ...
    dustfem '«Ну хорошо. Что я могу сделать для тебя, посвященный?»{#dustfem_s50_1}'
...
label dustfem_s51: # - # IF ~  Global("Appearance","GLOBAL",0)
    ...
    dustfem '«Ты потерялся?»{#dustfem_s51_1}'
...
label dustfem_s52: # from 51.0
    ...
    dustfem '«Хорошо. Если тебе нужна помощь, дай мне знать».{#dustfem_s52_1}'
