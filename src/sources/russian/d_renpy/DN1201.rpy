init 10 python:
    from game.engine.runtime import (runtime)
    from game.dlgs.n1201_logic import N1201Logic
    n1201Logic = N1201Logic(runtime.global_state_manager)


# ###
# Original:  DLG/DN1201.DLG
# ###


# s0 # say44993
label n1201_s0: # from 1.6 3.0 # IF ~  True()
    'n1201_s0{#n1201_s0}'
    # nr 'На этой вонючей записке под текстом изображена странная диаграмма. Кажется, она указывает, что тебе следует загнуть уголки записки к центру. На каждом из уголков есть ряд странных отметок: одна отметка в верхнем правом, две — в нижнем правом, три — в нижнем левом и ни одной в верхнем левом.{#n1201_s0_1}'

    menu:
        'n1201_s0_r44994{#n1201_s0_r44994}': # 'Загнуть верхний правый уголок.{#n1201_s0_r44994}'
            # a0 # r44994
            $ n1201Logic.r44994_action()
            jump n1201_s1

        'n1201_s0_r44995{#n1201_s0_r44995}': # 'Загнуть нижний правый уголок.{#n1201_s0_r44995}'
            # a1 # r44995
            $ n1201Logic.r44995_action()
            jump n1201_s1

        'n1201_s0_r44996{#n1201_s0_r44996}': # 'Загнуть нижний левый уголок.{#n1201_s0_r44996}'
            # a2 # r44996
            $ n1201Logic.r44996_action()
            jump n1201_s1

        'n1201_s0_r44997{#n1201_s0_r44997}': # 'Загнуть верхний левый уголок.{#n1201_s0_r44997}'
            # a3 # r44997
            $ n1201Logic.r44997_action()
            jump n1201_s1

        'n1201_s0_r44998{#n1201_s0_r44998}': # 'Оставить записку в покое.{#n1201_s0_r44998}'
            # a4 # r44998
            $ n1201Logic.r44998_action()
            jump n1201_dispose


# s1 # say44999
label n1201_s1: # from 0.0 0.1 0.2 0.3 1.0 1.1 1.2 1.3 1.4
    'n1201_s1{#n1201_s1}'
    # nr 'Ты загибаешь уголок таким образом, чтобы он касался центра.{#n1201_s1_1}'

    menu:
        'n1201_s1_r45000{#n1201_s1_r45000}' if n1201Logic.r45000_condition(): # 'Загнуть верхний правый уголок.{#n1201_s1_r45000}'
            # a5 # r45000
            $ n1201Logic.r45000_action()
            jump n1201_s1

        'n1201_s1_r45001{#n1201_s1_r45001}' if n1201Logic.r45001_condition(): # 'Загнуть нижний правый уголок.{#n1201_s1_r45001}'
            # a6 # r45001
            $ n1201Logic.r45001_action()
            jump n1201_s1

        'n1201_s1_r45002{#n1201_s1_r45002}' if n1201Logic.r45002_condition(): # 'Загнуть нижний правый уголок.{#n1201_s1_r45002}'
            # a7 # r45002
            $ n1201Logic.r45002_action()
            jump n1201_s1

        'n1201_s1_r45003{#n1201_s1_r45003}' if n1201Logic.r45003_condition(): # 'Загнуть нижний левый уголок.{#n1201_s1_r45003}'
            # a8 # r45003
            $ n1201Logic.r45003_action()
            jump n1201_s1

        'n1201_s1_r45004{#n1201_s1_r45004}' if n1201Logic.r45004_condition(): # 'Загнуть верхний левый уголок.{#n1201_s1_r45004}'
            # a9 # r45004
            $ n1201Logic.r45004_action()
            jump n1201_s1

        'n1201_s1_r45005{#n1201_s1_r45005}' if n1201Logic.r45005_condition(): # 'Загнуть верхний левый уголок.{#n1201_s1_r45005}'
            # a10 # r45005
            jump n1201_s2

        'n1201_s1_r45006{#n1201_s1_r45006}': # 'Разложить записку, начать заново.{#n1201_s1_r45006}'
            # a11 # r45006
            $ n1201Logic.r45006_action()
            jump n1201_s0

        'n1201_s1_r45008{#n1201_s1_r45008}': # 'Разложить записку, оставить ее в покое.{#n1201_s1_r45008}'
            # a12 # r45008
            $ n1201Logic.r45008_action()
            jump n1201_dispose


# s2 # say45015
label n1201_s2: # from 1.5
    'n1201_s2{#n1201_s2}'
    # nr 'Как только ты загибаешь верхний левый уголок, ты видишь, что верхний правый уголок разгибается сам по себе, возвращаясь в прежнее положение.{#n1201_s2_1}'

    menu:
        'n1201_s2_r45016{#n1201_s2_r45016}': # 'Снова загнуть правый верхний уголок.{#n1201_s2_r45016}'
            # a13 # r45016
            jump n1201_s4

        'n1201_s2_r45017{#n1201_s2_r45017}': # 'Загнуть нижний правый уголок.{#n1201_s2_r45017}'
            # a14 # r45017
            $ n1201Logic.r45017_action()
            jump n1201_s3

        'n1201_s2_r45018{#n1201_s2_r45018}': # 'Разложить записку, оставить ее в покое.{#n1201_s2_r45018}'
            # a15 # r45018
            $ n1201Logic.r45018_action()
            jump n1201_dispose


# s3 # say45019
label n1201_s3: # from 2.1
    'n1201_s3{#n1201_s3}'
    # nr 'Как только ты загибаешь нижний левый уголок, на мгновение он остается в таком положении, но затем другие уголки разгибаются. Ничего не происходит.{#n1201_s3_1}'

    menu:
        'n1201_s3_r45020{#n1201_s3_r45020}': # 'Снова осмотреть записку.{#n1201_s3_r45020}'
            # a16 # r45020
            jump n1201_s0

        'n1201_s3_r45021{#n1201_s3_r45021}': # 'Отложить записку.{#n1201_s3_r45021}'
            # a17 # r45021
            jump n1201_dispose


# s4 # say45022
label n1201_s4: # from 2.0
    'n1201_s4{#n1201_s4}'
    # nr 'Как только ты снова загибаешь верхний правый уголок, нижний левый уголок повторяет то же действие. Теперь все уголки касаются центра. Какое-то время ты наблюдаешь за тем, как уголки бумаги поднимаются, превращая записку в небольшую четырехугольную бумажную пирамидку.{#n1201_s4_1}'

    menu:
        'n1201_s4_r45023{#n1201_s4_r45023}': # 'Разогнуть пирамидку.{#n1201_s4_r45023}'
            # a18 # r45023
            $ n1201Logic.r45023_action()
            jump n1201_s5


# s5 # say45024
label n1201_s5: # from 4.0
    'n1201_s5{#n1201_s5}'
    # nr 'Ты отгибаешь стороны пирамидки, и бумага превращается в пыль. Внутри находится треугольная серьга. Она блестит и играет на свету.{#n1201_s5_1}'

    menu:
        'n1201_s5_r45025{#n1201_s5_r45025}': # 'Взять треугольную серьгу…{#n1201_s5_r45025}'
            # a19 # r45025
            $ n1201Logic.r45025_action()
            jump n1201_dispose
