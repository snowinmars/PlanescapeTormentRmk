...
label vaxis_s0: # - # IF ~  Global("Vaxis","GLOBAL",0)
    nr 'Неуклюжий труп смотрит на тебя пустым взглядом. На его лбу вырезан номер{#vaxis_s0_}'
    vaxis '«821»{#vaxis_s0_}'
    nr ', а его губы крепко зашиты. От тела исходит легкий запах формальдегида.{#vaxis_s0_}'
...
label vaxis_s1: # from 0.3 # IF ~  False()
    nr 'Довольно странно, но твоя способность не работает с этим трупом.{#vaxis_s1_}'
...
label vaxis_s2: # from 1.0
    nr 'После твоего тычка в глаз труп, рефлекторно закрыв руками лицо, издает нечленораздельный вопль. Он начинает что-то невнятно бормотать, сыпля проклятиями в твой адрес.{#vaxis_s2_}'
...
label vaxis_s3: # from 2.2 5.2
    nr 'Ты уже почти отвернулся, как{#vaxis_s3_}'
    vaxis '«Зомби»{#vaxis_s3_}'
    nr 'Начинает что-то бормотать… кажется, он пытается что-то сказать, но с зашитым ртом это сделать трудно.{#vaxis_s3_}'
    vaxis '«Фто ТЫ? Фто тее нао?»{#vaxis_s3_}'
...
label vaxis_s4: # from 3.5 6.5 7.8 8.5 10.4 11.4 12.2 13.5 14.4 15.2 16.4 17.2 18.1 19.3 20.1 25.6 27.6 31.6 32.5 34.2 35.6 59.1 74.1 75.3 76.1
    nr 'Ты уже почти отвернулся, как зомби начинает издавать низкое протяжное ворчание:{#vaxis_s4_}'
    vaxis '«Никоу ничео не говои пво МЕЯ. Моучи. Не говои НИФЕО твуфявым».{#vaxis_s4_}'
    nr 'Он прикладывает палец к губам.{#vaxis_s4_}'
    vaxis '«Фффф».{#vaxis_s4_}'
    nr 'После этого он проводит пальцем по горлу.{#vaxis_s4_}'
    vaxis '«Или ты уфнефь нафегда. ПОЯЛ мея?»{#vaxis_s4_}'
...
label vaxis_s5: # from 0.0 0.1 0.2 0.4
    nr 'Зомби от неожиданности моргает при твоем обращении.{#vaxis_s5_}'
    vaxis '«А? Фто?»{#vaxis_s5_}'
...
label vaxis_s6: # from 2.0 2.1 5.0 5.1
    vaxis '«Зомби»{#vaxis_s6_}'
    nr 'Пытается что-то сказать сквозь зашитые губы. На его лице странная смесь испуга и злобы.{#vaxis_s6_}'
    vaxis '«Фто ТЫ? Фего тее нао?»{#vaxis_s6_}'
...
label vaxis_s7: # from 3.0 3.1 3.2 3.4 6.0 6.1 6.2 6.4
    nr 'Кажется, зомби не расслышал тебя. Он осматривает тебя с ног до головы, затем хмурится.{#vaxis_s7_}'
    vaxis '«Фто ты фдефь делаефь?»{#vaxis_s7_}'
    nr 'Его глаза недоверчиво сужаются.{#vaxis_s7_}'
    vaxis '«Фпионифь за Мертфяками?»{#vaxis_s7_}'
...
label vaxis_s8: # from 7.2
    nr 'Он изучает тебя более пристально.{#vaxis_s8_}'
    vaxis '«Ты фпион? Ты иф яфейки?»{#vaxis_s8_}'
...
label vaxis_s9: # from 7.3
    nr 'Он изучает тебя более пристально.{#vaxis_s9_}'
    vaxis '«Ты фпион? Ты иф яфейки?»{#vaxis_s9_}'
...
label vaxis_s10: # from 8.0 8.1
    nr 'Хмурясь, он шипит на тебя.{#vaxis_s10_}'
    vaxis '«Ты не фпион!»{#vaxis_s10_}'
    nr 'Он гонит тебя прочь.{#vaxis_s10_}'
    vaxis '«Профь! Профь!»{#vaxis_s10_}'
...
label vaxis_s11: # externs morte_s86
    nr 'Зомби согласно кивает. Кажется, ты замечаешь, что под слоем маскировки его распирает от гордости.{#vaxis_s11_}'
...
label vaxis_s12: # from 7.0 11.0
    nr 'Зомби выглядит заинтересованным.{#vaxis_s12_}'
    vaxis '«Пвоулемы? Фево ты натвовил?»{#vaxis_s12_}'
...
label vaxis_s13: # from 12.0
    nr 'Зомби смотрит на тебя как на умалишенного.{#vaxis_s13_}'
    vaxis '«Ты фпятил?»{#vaxis_s13_}'
...
label vaxis_s14: # from 13.0 13.3 15.0
    nr 'Он смотрит на тебя, затем начинает шипеть и отмахиваться от тебя.{#vaxis_s14_}'
    vaxis '«Ты фпятил! Профь от мея!»{#vaxis_s14_}'
...
label vaxis_s15: # externs morte_s88
    nr 'Фальшивый зомби недоверчиво смотрит на вас обоих.{#vaxis_s15_}'
...
label vaxis_s16: # from 13.1
    nr 'Он смотрит на тебя, затем начинает шипеть и отмахиваться от тебя.{#vaxis_s16_}'
    vaxis '«Пуфтогоовый! Приурок! Профь от мея, пей! Профь!»{#vaxis_s16_}'
...
label vaxis_s17: # from 7.5 10.1 14.1 16.1 25.3 27.3
    nr 'На миг он кажется испуганным, потом осматривает тебя, и по его лицу расползается ухмылка.{#vaxis_s17_}'
    vaxis '«Ты подеиффя фо мной фвоим фекветом, я подеюфь фвоим ф *тоой*. Фдефь у мея пряфуффа друфья, у тея фдефь *никоо*. Тее фдефь не мефто. Твуфьявые тея уют. Я фбегу».{#vaxis_s17_}'
...
label vaxis_s18: # from 17.0
    nr 'Глаза зомби превращаются в щелочки, он шипит на тебя:{#vaxis_s18_}'
    vaxis '«Ты ПЫТАЕФФЯ фапифать мея ф кьигу мертфых? У мея фдефь пвяфуффа двуфья, у тея фдефь *никоо*. Твониф мея — они тея пвиконфят».{#vaxis_s18_}'
...
label vaxis_s19: # from 7.6 10.2 14.2 16.2 25.4 27.4
    nr 'На миг он кажется испуганным, потом осматривает тебя, и по его лицу расползается ухмылка.{#vaxis_s19_}'
    vaxis '«ТЫ пытаеффя фапифать МЕЯ ф кьигу мертфых? У мея фдефь пвячуффа двуфья, у тея фдефь *никоо*. Твонеф мея — они тея пвиконфят».{#vaxis_s19_}'
...
label vaxis_s20: # from 19.2
    nr 'Глаза зомби превращаются в щелочки, он шипит на тебя:{#vaxis_s20_}'
    vaxis '«Ты подеилфя фо мной фвоим фекретом, я подеюфь фоим ф *тоой*. Фдефь у мея прячуффа друфья, у тея фдефь *никоо*. Твуфьявые тея уют. Я фбегу».{#vaxis_s20_}'
...
label vaxis_s21: # from 7.4 10.0 14.0 16.0 19.1 25.2 27.2
    nr 'Недобрый блеск в твоих глазах не оставляет от его самонадеянности и следа.{#vaxis_s21_}'
    vaxis '«Не-не-не! Не наа фвать фтражу!»{#vaxis_s21_}'
    nr 'Он явно напуган.{#vaxis_s21_}'
    vaxis '«Я-я-я фпионю фа твуфявыми, говою, фео увиву. Ни-нифео больфе».{#vaxis_s21_}'
...
label vaxis_s22: # from 7.7 10.3 14.3 16.3 17.1 25.5 27.5
    vaxis '«Не-не-не! Не твогаи мея!»{#vaxis_s22_}'
    nr 'Факт того, что ты явно превосходишь зомби в грубой силе, очевидно, повлиял на его решение, и от его самонадеянности не осталось и следа.{#vaxis_s22_}'
    vaxis '«Я-я-я фпионю за твуфьявыми, говою, фео увиву. Ни-нифео больфе».{#vaxis_s22_}'
...
label vaxis_s23: # from 21.0 22.0
    nr 'Зомби в страхе умолкает, ничего не желая говорить.{#vaxis_s23_}'
...
label vaxis_s24: # from 3.3 6.3 8.2
    vaxis '«Иффефь мея? Вафем?»{#vaxis_s24_}'
    nr 'Он искоса смотрит на тебя.{#vaxis_s24_}'
    vaxis '«У тея соофение двя мея?»{#vaxis_s24_}'
...
label vaxis_s25: # from 24.2
    nr 'Яростно шепчет.{#vaxis_s25_}'
    vaxis '«Тада фео тее *надо*, пей?!»{#vaxis_s25_}'
...
label vaxis_s26: # from 24.0
    vaxis '«Какое фоофение?»{#vaxis_s26_}'
...
label vaxis_s27: # from 24.1 26.3 26.4
    nr 'Его глаза в ярости сужаются.{#vaxis_s27_}'
    vaxis '«Ты не фьяфной. Фто ты?»{#vaxis_s27_}'
...
label vaxis_s28: # from 8.3 8.4 11.1 11.2 11.3 26.0 30.0 43.5
    vaxis '«Я фпионю фа твуфьявыми. Говою, чео вифу. Нифео больфе».{#vaxis_s28_}'
...
label vaxis_s29: # from 21.1 22.1 23.4 28.0 70.1 71.2
    vaxis '«Нифео. Они нифео ни деают. Нифео не нафол. Твупы, твупы, пвофто твупы. Твуфьявые *нифео* ни деают».{#vaxis_s29_}'
    nr 'Его глаза деловито сужаются.{#vaxis_s29_}'
    vaxis '«Буу дальфе фледить».{#vaxis_s29_}'
...
label vaxis_s30: # from 26.1 26.2
    nr 'Он сужает глаза, будто пытаясь тебя вычислить.{#vaxis_s30_}'
    vaxis '«Какие вафповявения?»{#vaxis_s30_}'
...
label vaxis_s31: # from 7.1 12.1 13.4 15.1 25.0 27.0 50.0
    nr 'Он на секунду умолкает, затем медленно, будто бы понимающе кивает.{#vaxis_s31_}'
    vaxis '«Пофеу я доувен помоать тее?»{#vaxis_s31_}'
...
label vaxis_s32: # from 31.0
    nr 'Зомби едва усмехается.{#vaxis_s32_}'
    vaxis '«Фем *нао* фео-то, но нифто нифео *не дает*. *Дай* мне фео-нить, и, *мовет*, я помоу».{#vaxis_s32_}'
...
label vaxis_s33: # from 31.2 31.5 32.1 32.4 34.1 35.2 35.5 75.0
    nr 'Он оглядывает тебя с ног до головы, как бы примериваясь, сможет ли он с тобой справиться, останавливается на шрамах и решает не делать этого.{#vaxis_s33_}'
    vaxis '«Хм-м. Ты мовефь убевать фееф поуталы».{#vaxis_s33_}'
...
label vaxis_s34: # from 31.3 32.2 35.3
    nr 'На миг он кажется испуганным, потом осматривает тебя, и по его лицу расползается ухмылка.{#vaxis_s34_}'
    vaxis '«Ты подеиффя фо мной фвоим фекветом, я подеюфь фвоим ф *тоой*. Фдефь у мея пряфуффа друфья, у тея фдефь *никоо*. Тее фдефь не мефто. Твуфьявые тея уют. Я фбегу».{#vaxis_s34_}'
...
label vaxis_s35: # from 30.3 31.1 32.0
    vaxis '«Тее нуно найти *кьюч* дья мея. Нуен желефный кьюч к байвамовофной комуафе».{#vaxis_s35_}'
...
label vaxis_s36: # from 35.1 58.2
    vaxis '«Он у твуфьявой».{#vaxis_s36_}'
    nr 'Он показывает на свои глаза.{#vaxis_s36_}'
    vaxis '«У нее воутые глафифи…»{#vaxis_s36_}'
    nr 'Затем он делает движение руками, похожее на стрижку ножницами.{#vaxis_s36_}'
    vaxis '«Лефвия на пайфах».{#vaxis_s36_}'
...
label vaxis_s37: # from 36.3
    nr 'Зомби моргает. Кажется, он тебя не понял.{#vaxis_s37_}'
...
label vaxis_s38: # from 36.1 36.2 37.0 37.1
    nr 'Зомби косо на тебя смотрит.{#vaxis_s38_}'
    vaxis '«Ефи тея поимают, не говои никоу обо мне, или я добеусь до тея, когда ты буешь фпать».{#vaxis_s38_}'
...
label vaxis_s39: # from 43.12
    vaxis '«Я ховоф ф мафкироуке. У мея ефть фрамы. Я наил на сея мноо байфамируфеи фидкофти. Иф мея поуфифя ХООФЫЙ фомби».{#vaxis_s39_}'
    nr 'Зомби хихикает через зашитые губы, потом стучит себя по голове.{#vaxis_s39_}'
    vaxis '«Твуфяки тууупые».{#vaxis_s39_}'
...
# s40 # say4514
label vaxis_s40: # -
    vaxis '«Я фду тея фдефь. Найди кьюч».{#vaxis_s40_}'
    nr 'Зомби улыбается мертвецким оскалом.{#vaxis_s40_}'
    vaxis '«Потом я помоу тее».{#vaxis_s40_}'
...
# s41 # say4517
label vaxis_s41: # -
    nr 'Глаза зомби расширяются, он протягивает руку и прищелкивает пальцами.{#vaxis_s41_}'
    vaxis '«Дай его мие».{#vaxis_s41_}'
...
label vaxis_s42: # from 35.0 36.0 58.0 58.1
    nr 'Глаза зомби расширены, он выхватывает ключ из твоей руки. Затем он поворачивается, все время кивая.{#vaxis_s42_}'
    vaxis '«Хорофо… хорофо».{#vaxis_s42_}'
...
label vaxis_s43: # from 21.2 22.2 23.5 25.1 27.1 28.1 29.0 42.1 44.2 45.1 46.2 47.2 48.0 51.1 52.0 53.0 54.0 56.0 58.3 59.0 60.3 61.4 62.3 63.1 64.0 70.2 71.3 77.0
    vaxis '«Фто ты хофефь уфнать?»{#vaxis_s43_}'
...
label vaxis_s44: # from 43.6
    vaxis '«Фа-ОД?»{#vaxis_s44_}'
    nr 'Нахмурившись, зомби напряженно думает.{#vaxis_s44_}'
    vaxis '«Я… фвыфал, фто он фиет де-то ф Уле».{#vaxis_s44_}'
    nr 'Он качает головой.{#vaxis_s44_}'
    vaxis '«Нефнаю где».{#vaxis_s44_}'
    nr 'Он снова хмурится.{#vaxis_s44_}'
    vaxis '«Твуфявые фодят ф ума, они не ЛЮЯТ Фа-ода».{#vaxis_s44_}'
...
label vaxis_s45: # from 44.0
    vaxis '«Твуффобы фнауви».{#vaxis_s45_}'
...
label vaxis_s46: # from 44.1 45.0
    vaxis '«Он фбофик. Пвинофит твупы ф Моуг, пводает их твуфвым. Пвинофит МУОГО твупоф. Твуфявые ненают, откуда он их беет. Дуают, он пифет пьей в кьигу мертфых».{#vaxis_s46_}'
...
label vaxis_s47: # from 43.7
    vaxis '«Ненают. Какой-то пей обфифтил тея?»{#vaxis_s47_}'
...
label vaxis_s48: # from 46.0 47.0
    nr 'Зомби пытается сказать, умолкает, пытается снова, затем вздыхает. Очевидно, что доходчивей он сказать не сможет.{#vaxis_s48_}'
...
label vaxis_s49: # from 30.1 42.0 43.0 43.1
    nr 'Зомби ворчит.{#vaxis_s49_}'
    vaxis '«Ты мовешь убевафь феев поуталы».{#vaxis_s49_}'
    nr 'Он взмахивает руками.{#vaxis_s49_}'
    vaxis '«Пуф».{#vaxis_s49_}'
...
label vaxis_s50: # from 33.0 49.0
    vaxis '«Поуталы…»{#vaxis_s50_}'
    nr 'Зомби окидывает широким жестом пространство вокруг себя.{#vaxis_s50_}'
    vaxis '«Поуталы вефде».{#vaxis_s50_}'
...
label vaxis_s51: # from 50.1 50.2 50.3 72.0
    nr 'Зомби кивает.{#vaxis_s51_}'
    vaxis '«Тее надо выити, пойти ф арку на певом этаже, февео-фападный фал… Тее нувна кофть паица, фогнутоо в кьюк…»{#vaxis_s51_}'
    nr 'Он поднимает указательный палец и сгибает его в крюк.{#vaxis_s51_}'
    vaxis '«Ефли у тея кьюч, иди в авку и пуыгай ф тайную гуобицу. И ты выбралфя отфюда. Тайный выфод».{#vaxis_s51_}'
    nr 'Он энергично кивает.{#vaxis_s51_}'
    vaxis '«Там ты мовефь ОДОФНУТЬ».{#vaxis_s51_}'
...
label vaxis_s52: # from 43.2
    vaxis '«Фуфай! Фапомиай!»{#vaxis_s52_}'
    nr 'В голосе зомби слышна раздраженность.{#vaxis_s52_}'
    vaxis '«Арка, пеувый этаж, феверо-фападная коуната…»{#vaxis_s52_}'
    nr 'Он поднимает указательный палец и сгибает его.{#vaxis_s52_}'
    vaxis '«Тее нувна кофть паица, фогнутая. Ты попаефь ф тайную гуобицу. Тайный выфод. Там ты мовефь ОДОФНУТЬ».{#vaxis_s52_}'
...
label vaxis_s53: # from 43.8
    vaxis '«Пифец».{#vaxis_s53_}'
    nr 'Пожимает плечами.{#vaxis_s53_}'
    vaxis '«Фтарый. Вовтый».{#vaxis_s53_}'
...
label vaxis_s54: # from 43.9
    vaxis '«Э?»{#vaxis_s54_}'
    nr 'Хмурится.{#vaxis_s54_}'
    vaxis '«Фто она?»{#vaxis_s54_}'
...
label vaxis_s55: # from 43.10
    vaxis '«Пвоводник. На пеувом этаже. Фто ты хофефь фнать о нем?»{#vaxis_s55_}'
...
label vaxis_s56: # from 55.0
    vaxis '«Фтвааный. Не твуфявый, не анавфифт. Гуава двугие…»{#vaxis_s56_}'
    nr 'Пожимает плечами.{#vaxis_s56_}'
    vaxis '«Как у квыфы. Фтванно».{#vaxis_s56_}'
...
label vaxis_s57: # - # IF ~  GlobalGT("Vaxis","GLOBAL",0)
    nr 'Ты видишь фальшивого зомби. Ты удивлен его маскировке… его дыхание едва заметно, ты практически не можешь уловить его.{#vaxis_s57_}'
...
label vaxis_s58: # from 57.0 57.1 57.3
    nr 'Зомби быстро оглядывается вокруг, высматривая соглядатая, затем поворачивается к тебе:{#vaxis_s58_}'
    vaxis '«Фто?»{#vaxis_s58_}'
...
label vaxis_s59: # from 57.2
    nr 'Зомби быстро оглядывается вокруг, высматривая соглядатая, затем шипит на тебя.{#vaxis_s59_}'
    vaxis '«Ухои! Вон!»{#vaxis_s59_}'
...
label vaxis_s60: # from 43.11
    vaxis '«Я ховоф ф мафкироуке. У мея ефть фрамы. Я наил на сея мноо байфамируфеи фидкофти. Иф мея поуфифя ХООФЫЙ фомби».{#vaxis_s60_}'
    nr 'Зомби хихикает через зашитые губы, потом стучит себя по голове.{#vaxis_s60_}'
    vaxis '«Твуфяки тууупые».{#vaxis_s60_}'
...
label vaxis_s61: # from 60.0
    nr 'Зомби определенно не понял сарказма, энергично кивая словам.{#vaxis_s61_}'
    vaxis '«Тупие твуфявые. Ив мея ХОВОФЫЙ фомби».{#vaxis_s61_}'
...
label vaxis_s62: # from 60.1 61.0
    nr 'Он смотрит на твои шрамы.{#vaxis_s62_}'
    vaxis '«А как ты думаефь? По мие, нет, не офень».{#vaxis_s62_}'
    nr 'Бьет себя по груди.{#vaxis_s62_}'
    vaxis '«Я КИЕПКИЙ».{#vaxis_s62_}'
...
label vaxis_s63: # from 43.3 43.4 60.2 61.1 62.0 64.1 64.2
    nr 'Что-то бормоча, он несколько раз оглядывает тебя с ног до головы, затем кивает.{#vaxis_s63_}'
    vaxis '«Уху. Мие нуна банка баифама».{#vaxis_s63_}'
    nr 'Показывает на шрамы на твоей груди.{#vaxis_s63_}'
    vaxis '«И игоука ф ниткой».{#vaxis_s63_}'
...
label vaxis_s64: # from 61.2 61.3 62.1 62.2
    nr 'Странным взглядом он осматривает тебя с ног до головы.{#vaxis_s64_}'
    vaxis '«Ты будефь ХОВОФЫМ фомби. Мовно фделать из тея фомби? ХОВОФАЯ мафкиофка».{#vaxis_s64_}'
...
label vaxis_s65: # from 63.0
    nr 'Зомби берет у тебя предметы и приступает к работе…{#vaxis_s65_}'
...
label vaxis_s66: # from 65.0 65.2 65.3
    nr 'Зомби обильно натирает твое тело бальзамирующей жидкостью, затем широкими стежками зашивает несколько шрамов. Начав с ног, он медленно поднимается наверх, зашивая в конце концов твои губы.{#vaxis_s66_}'
...
label vaxis_s67: # from 66.0 66.2
    nr 'Зомби держит тебя за руку.{#vaxis_s67_}'
    vaxis '«Офтововно! Вафгоов вафофет фвы, науфив мафкивку. Фомби не говоят. Поняу? Говои медвенно, офтовоно».{#vaxis_s67_}'
...
label vaxis_s68: # from 67.0
    nr 'Зомби хмурится.{#vaxis_s68_}'
    vaxis '«Мафкирофка не деужится доуго… бальфам выфыхает, фвы рвутфя».{#vaxis_s68_}'
    nr 'Он жмет плечами.{#vaxis_s68_}'
    vaxis '«Фнаружи Морга от нее нифего не офтанетфя. Не бегай! Ефли ты побежифь, то науфифь вфю мафкирофку».{#vaxis_s68_}'
...
# s69 # say4641
label vaxis_s69: # -
    vaxis '«Зомби»{#vaxis_s69_}'
    nr 'Хмурится.{#vaxis_s69_}'
    vaxis '«Фде-то я тея виел. Мы не виелифь раньфе?»{#vaxis_s69_}'
...
label vaxis_s70: # from 23.0 23.2 71.0 71.1
    nr 'К твоему удивлению, зомби отступается от тебя… Он начинает со страхом озираться.{#vaxis_s70_}'
...
label vaxis_s71: # externs morte_s90
    nr 'Зомби смотрит на вас обоих с явной опаской, продолжая молчать… но что-то в его выражении говорит тебе, что предположение Морта верно.{#vaxis_s71_}'
...
label vaxis_s72: # from 30.2
    nr 'Зомби выглядит сбитым с толку, но затем пожимает плечами, начиная копаться в своей заляпанной тунике.{#vaxis_s72_}'
    vaxis '«Вфе тихо, твувявыя не февелятся, нифего новоо с пофледнего отфета».{#vaxis_s72_}'
    nr 'Спустя несколько секунд он протягивает тебе какие-то предметы, затем ворчит:{#vaxis_s72_}'
    vaxis '«Вот»{#vaxis_s72_}'
    nr 'Судя по запаху, они прятались очень глубоко, чтобы их невозможно было найти в случае обыска.{#vaxis_s72_}'
    vaxis '«Я фкоро уйду».{#vaxis_s72_}'
...
# s73 # say4658
label vaxis_s73: # -
    nr 'Зомби ворчит:{#vaxis_s73_}'
    vaxis '«Поутал в арке»{#vaxis_s73_}'
    nr 'Пеувый этав, феверо-фападная коумата, нувен кофтяной паиец дья откуывания,{#vaxis_s73_}'
    vaxis '«Он кивает. — Удафи».{#vaxis_s73_}'
...
label vaxis_s74: # from 34.0
    nr 'Глаза зомби превращаются в щелочки, он шипит на тебя:{#vaxis_s74_}'
    vaxis '«Ты ПЫТАЕФФЯ фапифать мея ф кьигу мертфых? У мея фдефь пвяфуффа двуфья, у тея фдефь *никоо*. Твониф мея — они тея пвиконфят».{#vaxis_s74_}'
...
label vaxis_s75: # from 31.4 32.3 35.4
    nr 'На миг он кажется испуганным, потом осматривает тебя, и по его лицу расползается ухмылка.{#vaxis_s75_}'
    vaxis '«ТЫ пытаеффя фапифать МЕЯ ф кьигу мертфых? У мея фдефь пвячуффа двуфья, у тея фдефь *никоо*. Твонеф мея — они тея пвиконфят».{#vaxis_s75_}'
...
label vaxis_s76: # from 75.1
    nr 'Глаза зомби превращаются в щелочки, он шипит на тебя:{#vaxis_s76_}'
    vaxis '«Ты подеилфя фо мной фвоим фекретом, я подеюфь фоим ф *тоой*. Фдефь у мея прячуффа друфья, у тея фдефь *никоо*. Твуфьявые тея уют. Я фбегу».{#vaxis_s76_}'
...
label vaxis_s77: # from 51.0
    nr 'Он пожимает плечами.{#vaxis_s77_}'
    vaxis '«Доувен быть де-то фдефь… Ифи на фкладах навеуху. Мовет быть, там».{#vaxis_s77_}'
